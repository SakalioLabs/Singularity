...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has twenty-one failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 21 live-validated `m4-ready-task-goal-verifier-success-criteria-v1` with 61 fail-closed reports (`37` retain, `24` suppress, `0` allow), then exposed `m4_readiness_recovery_inventory_family_root_completion_disconnect`: events 1512/1513 machine-completed a recovery child from `dark_oak_log:4`, but event 1519 kept its exact-item root active and six stale crafting tasks remained at dusk. The bounded offline fix now passes as `m4-readiness-recovery-inventory-family-root-completion-v1`: strict M4 binds child/root identity and a canonical family/exact requirement fingerprint, closes a machine-satisfied root before Planner, cancels only matching siblings frozen at binding time, preserves later consumers, and emits idempotent evidence plus bounded inventory context. Five focused cases, all 743 definitions in 35 non-live Python files (744 repository definitions total), and six Node suites with 52 internal cases pass; protocol, task contract, generic GoalVerifier, and capability thresholds are unchanged. No live episode ran. Exactly one fresh BM-012 Probe 22 is authorized only after this gate commit is pushed; BM-013/BM-014 remain locked. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
