...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has thirteen failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 13 live-validates `m4-pickup-collection-completion-grounding-v1`: direct `GoalNear` completion at 1.014 blocks was rejected, one standable-cell fallback reached 0.876 blocks under the remaining shared budget, and the outer postcondition observed `oak_log:+1`. The first unrecovered transition moved to action event 974: the requested crafting table was equipped and the target block was air, but the target cell `(103,136,-31)` was also the player's floored feet cell. Mineflayer waited five seconds for a `blockUpdate` that never arrived; six same-goal attempts and one later shelter retry made no placement, and no iron was acquired despite an observed ore block. A bounded offline `place_target_player_occupancy_grounding` gate is now required; no fresh live episode is authorized until it passes and is pushed. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
