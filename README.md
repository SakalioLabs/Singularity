...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has twelve failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 12 live-validates `m4-expected-drop-pickup-postcondition-v1`: all five strict digs emitted the policy, three acquired their expected drops, two correctly failed closed, and no removed block was falsely reported as resource success. Its event-169 pickup collector nevertheless reported inner success 2.092 blocks from the drop with no inventory delta. The bounded `m4-pickup-collection-completion-grounding-v1` gate now passes offline: strict M4 accepts collection completion only from measured acquisition distance or the expected inventory delta, and a false direct completion receives at most one machine-validated standable-cell `GoalBlock` fallback under the same 6000 ms budget. Unsupported or still-distant fallbacks fail closed, while fixed M1/M2 behavior is unchanged. Exactly one fresh BM-012 Probe 13 is authorized only after this gate commit is pushed. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
