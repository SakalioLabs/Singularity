...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has thirteen failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 13 live-validates `m4-pickup-collection-completion-grounding-v1`: direct `GoalNear` completion at 1.014 blocks was rejected, one standable-cell fallback reached 0.876 blocks under the remaining shared budget, and the outer postcondition observed `oak_log:+1`. Its first unrecovered transition was action event 974, where target `(103,136,-31)` matched the player's floored feet cell and seven placement attempts made no mutation. The offline `m4-place-target-player-occupancy-v1` gate now passes: strict M4 uses a fixed 0.6-by-1.8 player collision box, rejects the exact Probe 13 feet/head target before equip or `placeBlock`, preserves machine coordinates, and exposes at most four adjacent references for one next-cycle replan. The exact replay, boundary/missing-position controls, adjacent strict control, non-M4 compatibility control, 722 Python tests, and all six Node suites with 45 internal cases pass. No live episode ran in this offline round; exactly one fresh BM-012 Probe 14 is authorized only after this gate commit is pushed. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
