...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has eight failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 8 live-validated `m4-place-target-occupancy-v1`: an occupied target was rejected before execution, the target-aware replan selected air, and the crafting table was placed. The resulting `m4_task_world_state_reconciliation_grounding` gate now passes offline under `m4-task-world-state-reconciliation-v1`: exact nearby-block machine evidence retires the satisfied Probe 8 task before ready-task selection, while ambiguous evidence and non-M4 pre-goal reconciliation remain fail closed. Exactly one fresh Probe 9 is authorized after this gate commit is pushed. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
