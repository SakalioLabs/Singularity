...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has eleven failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 11 live-validates `m4-subtask-opportunity-trigger-type-grounding-v1`: all 110 accepted real Planner responses carried passing trigger reports, all 117 trigger values were valid strings, and the Probe 10 object-trigger crash did not recur. Its new earliest blocker was a successful `dig` that removed an `oak_log` without acquiring its expected drop. The bounded `m4-expected-drop-pickup-postcondition-v1` gate now passes offline: strict M4 waits for a delayed drop, collects with a reachable 1-block goal, and fails closed unless the expected item reaches inventory, while fixed M1/M2 behavior remains unchanged. Exactly one fresh Probe 12 is authorized only after this gate commit is pushed. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
