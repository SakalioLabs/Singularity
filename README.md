...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has nineteen failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 19's earliest unrecovered transition is `post_place_crafting_table_machine_observation_grounding`: event 621 machine-confirmed a crafting table at `(106,136,-36)`, event 623 omitted it, and fourteen later actions repeated against the occupied target. The bounded offline `m4-post-place-crafting-table-machine-observation-v1` gate now passes. Strict M4 projects only a successful crafting-table result whose requested item, reference, placed target, before/after names, and exact integer coordinates all agree; the evidence reaches the post-action and next planner observation, closes machine-state tasks, then expires. Unsuccessful, mismatched, malformed, non-table, and non-M4 controls remain fail-closed, and Observer range is unchanged. Two exact gate cases, 43 M4 deadline definitions, 735 Python regression definitions across all 35 non-live scripts, and six Node suites with 52 internal PASS cases succeed. No live episode ran in this offline round; exactly one fresh BM-012 Probe 20 is authorized only after this gate commit is pushed. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
