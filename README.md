...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has eighteen failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 18 did not exercise the deadline-bound readiness branch: no transport timeout, bridge recovery, navigation-recovery report, or `PathStopped` error occurred; 8/10 pre-critical navigations succeeded and each bounded completion failure was followed by a successful move. The Runner's placement `empty_plan` at event 303 recovered through the event-379 crafting-table placement. The earliest unrecovered transition is `critical_health_survival_action_precondition_deadlock`: after a hostile interrupt moved the player to safety but health fell to 2.33, a real schema-valid escape plan and ActionVerifier acceptance reached action event 817, where ActionController rejected `move_to` at zero duration solely because health was below 4. It then rejected fourteen survival-recovery actions, while thirteen critical-health goals consumed the remaining goal budget. Probe 18 completed 7/24 goals and 28/47 actions, acquired no iron, and passed 67/74 eligibility checks. No code changed and no second episode ran; another live run is blocked pending a bounded offline critical-health survival-action precondition gate. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
