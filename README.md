...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has fifteen failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 15 live-validates `m4-dig-required-tool-equip-v1`: all 13 stone digs selected, equipped, and machine-confirmed `wooden_pickaxe`; 12 succeeded and acquired 12 cobblestone, while one expected-drop failure was recovered by a later successful dig and the cobblestone goal completed. Its earliest blocker is `deadline_bound_navigation_bridge_recovery`: action event 981 timed out after 60 seconds while approaching observed coal, did not replay, left `bridge_reconnected=false` with 281.859 seconds still available, and all 16 later actions failed. The bounded offline `m4-deadline-bound-bridge-recovery-v1` gate now passes. A timed-out deadline-bound command remains single-attempt, reconnect is deferred outside the expired action, and the next strict-M4 observation establishes a fresh deadline-bounded socket and confirms machine player state before planning can resume; expired or unconfirmed recovery fails closed. Exact Probe 15 replay controls, 727 Python tests, and all six Node suites with 50 internal cases pass. No live episode ran in this offline round; exactly one fresh BM-012 Probe 16 is authorized only after this gate commit is pushed. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
