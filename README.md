...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has twenty failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 20 live-activated `m4-post-place-crafting-table-machine-observation-v1`: event 269 accepted exact machine evidence, event 273 closed the placement task, event 278 verified the goal, and event 288 carried the second bounded observation into wooden-pickaxe planning. Observer directly saw the table in both observations, so the omission-projection branch remains offline-only, but Probe 19's same-target loop did not recur. Independent review also overrides the event-820 torch `empty_plan`, because progression recovered through a second table, stone pickaxe, and action event 1172 `raw_iron:+1`. The earliest unrecovered transition is `m4_ready_task_goal_verifier_success_criteria_bypass`: plan event 1189 required `raw_iron:2` and supplied a valid dig, but pre-plan verification event 1193 accepted generic `Mine iron ore` from the existing `raw_iron:1` with no new delta. Fourteen `ready_task_selected` roots repeated that false completion with zero further actions, exhausting the 24-goal limit while 190.516 seconds remained. The bounded offline fix now passes as `m4-ready-task-goal-verifier-success-criteria-v1`: strict M4 binds the exact selected task ID and criteria snapshot, suppresses generic completion until that task is machine-completed by state reconciliation or action feedback, and rejects same-title replacement, malformed binding, unproven completion, and expired deadlines without changing global GoalVerifier, M1, or M2 behavior. Four exact gate tests, 47 M4 deadline definitions, 739 Python regression definitions, all 35 non-live Python scripts, and six Node suites with 52 internal cases pass. No live episode ran in this gate round; exactly one fresh BM-012 Probe 21 is authorized only after the gate commit is pushed. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
