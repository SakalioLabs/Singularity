...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has eleven failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 11 live-validates `m4-subtask-opportunity-trigger-type-grounding-v1`: all 110 accepted real Planner responses carried passing trigger reports, all 117 trigger values were valid strings, and the Probe 10 object-trigger crash did not recur. The new earliest blocker is a successful `dig` that removed an `oak_log` without acquiring its expected drop; a second pickup attempt timed out, inventory never exceeded one `dark_oak_log`, and a later navigation deadline disconnected the bot bridge. No further live episode is authorized until a bounded expected-drop pickup/postcondition gate passes offline and is pushed. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
