...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has sixteen failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 16 did not exercise `m4-deadline-bound-bridge-recovery-v1`: there were no deadline-bound action failures, bridge-recovery events, or `Not connected to bot bridge` action errors, all 10 navigation actions succeeded, and the terminal bot remained connected. Its earliest blocker is `planner_place_replan_feedback_grounding`: after event 283 rejected reference `(106,135,-29)`, supplied four adjacent candidates, and explicitly prohibited retry, the real schema-valid replan at events 292/294 repeated that exact reference and action event 304 was rejected again with unchanged state. The bounded offline `m4-place-replan-feedback-grounding-v1` gate now passes. Agent forwards the rejected reference and verifier-supplied candidates as structured one-shot feedback; the next strict-M4 replan must select exactly one candidate, while a repeated, missing, multiple, malformed, or candidate-external place reference rejects the plan before task creation or action execution. Exact Probe 16 replay, all four adjacent positives, fail-closed controls, 729 Python regression definitions, and six Node suites with 50 internal cases pass; the real M2 integration test remains skipped without `OPENAI_API_KEY`. No live episode ran in this offline round, and exactly one fresh BM-012 Probe 17 is authorized only after this gate commit is pushed. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
