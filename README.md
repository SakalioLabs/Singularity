...
Current status: M1, M2, and M3 are `repeat_verified`. M1 has BM-001..005 at 3/3 (`15/15` distinct eligible live successes). M2 has BM-006 and BM-007 at three eligible baseline/candidate pairs each, BM-008..010 at 3/3 independent successes each, and two eligible recovery sessions. M3 has three later-session skill retrieval/outcome cases plus approved held-out transfer evidence. M4 is `failing`: BM-011 has three independently eligible fresh survival-to-dawn episodes, BM-012 has nineteen failed attempts and remains 0/3, and BM-013..014 remain unverified. Probe 19 never entered critical health, so `m4-critical-health-survival-action-precondition-v1` emitted no live policy report and remains live-unverified. The Runner's coal `empty_plan` recovered through a verified coal dig. The earliest unrecovered transition is `post_place_crafting_table_machine_observation_grounding`: action event 621 machine-confirmed a crafting table at `(106,136,-36)`, but observation event 623 omitted it, task reconciliation did not close the placement prerequisite, and event 686 began fourteen zero-duration repeats against the occupied target. Forty schema-valid Planner calls exhausted the torch goal's 40 cycles and consumed 209.688 seconds; moving adjacent later exposed the same table and recovered the machine objective, but not the lost dusk budget. Stone-pickaxe progression resumed, yet shelter preparation started at world time 10039 and the final move crossed the absolute deadline by 0.015 seconds. Probe 19 passed 64/74 eligibility checks, acquired no iron, and is ineligible. No code fix, second episode, or new live authorization exists; the next gate is offline-only. The sole authoritative audit is `workspace/evals/capability_evidence_current.json`.
