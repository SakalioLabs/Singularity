"""Run one representative SP-003 planner throughput probe without Minecraft."""
...
PHASE = 134
POLICY_ID = "sp003-representative-provider-throughput-gate-v1"
PROBE_EPISODE_ID = "sp003-provider-throughput-probe-phase134"
DEFAULT_SOURCE = Path(
    "workspace/evals/sp003_runs/"
    "sp003_baseline_20260720_111853_3cd46332/session_78921484-8ab.jsonl"
)
DEFAULT_OUTPUT = Path(
    "workspace/evals/stone_pickaxe_sp003_representative_provider_throughput_probe.json"
)
SOURCE_CALL_ID = "llm-e0ace62ed2294be7"
SOURCE_ROOT_PLAN_ID = "root-8241bfa38f44473e"
SOURCE_PARENT_CALL_ID = "llm-08f6383b6b16423a"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one no-Minecraft representative SP-003 provider probe"
    )
...
def repo_path(value: str | Path) -> Path:
    ...
    try:
        ...
        raise RuntimeError(f"path must stay inside repository: {value}") from exc
    return resolved
...
def retained_observation_before_call(source: Path, call_id: str) -> tuple[dict, dict]:
    ...
    for line_number, line in enumerate(source.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if (
            event.get("type") == "llm_planner_call"
            and isinstance(event.get("data"), dict)
            and event["data"].get("call_id") == call_id
        ):
            if not latest_observation:
                raise RuntimeError(f"no retained observation precedes planner call {call_id}")
            return latest_observation, event
    raise RuntimeError(f"retained planner call not found: {call_id}")
...
def exact_probe_messages(planner: Planner, world_state: dict) -> list[dict]:
    ...
    return [
        ...
        {
            "role": "user",
            "content": planner._build_planning_prompt(SP003_GOAL, world_state, ""),
        },
    ]


def request_payload_metadata(messages: list[dict]) -> dict:
    ...
    return {
        ...
        "request_payload_byte_count": len(payload.encode("utf-8")),
        "system_message_byte_count": len(messages[0]["content"].encode("utf-8")),
        "user_message_byte_count": len(messages[1]["content"].encode("utf-8")),
    }
...
def run_probe(source: Path) -> dict:
    ...
    messages = exact_probe_messages(planner, world_state)
    request_shape = request_payload_metadata(messages)
    compact_state = planner._compact_stone_pickaxe_state(world_state)
    ...
    actual_source_state = {
        "runtime_mode": compact_state.get("runtime_mode"),
        "arm": compact_state.get("sp003_arm"),
        "stage": compact_state.get("sp003_stage"),
        "inventory": compact_state.get("inventory"),
        "log_source_removal_count": (
            compact_state.get("sp003_progress", {}).get("log_source_removal_count")
            if isinstance(compact_state.get("sp003_progress"), dict)
            else None
        ),
        "target_count": len(compact_state.get("sp003_targets", [])),
    }
    ...
    criteria = {
        ...
        "request_payload_matches_precomputed_sha256": (
            provider_metadata.get("request_sha256") == request_shape["request_sha256"]
        ),
        "representative_request_size": (
            request_shape["request_payload_byte_count"]
    ...
    return {
        "type": "stone_pickaxe_sp003_representative_provider_throughput_probe",
        "schema_version": 1,
        "phase": PHASE,
        "policy_id": POLICY_ID,
        "task_id": "SP-003",
        "purpose": "gate_a_new_strict_live_baseline_after_phase_133_external_planner_latency",
        ...
        "decision": (
            ...
            if passed
            else "hold_new_authorization_provider_throughput_ineligible"
        ),
        "thresholds": {
        ...
        "source": {
            ...
            "predecessor_call_index": predecessor_call["data"]["call_index"],
        ...
        "request": {
            **request_shape,
            "request_message_count": provider_metadata.get("request_message_count"),
