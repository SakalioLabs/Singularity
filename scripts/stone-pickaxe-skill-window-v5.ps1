[CmdletBinding()]
param(
    [ValidateSet("RunSP001SkillCandidateV5", "AuditFixture")]
    [string]$Mode = "AuditFixture",
    [ValidateSet("candidate")]
    [string]$SkillArm = "candidate",
    [string]$ReplicateId = "",
...
$pairedPolicyPath = Join-Path $repoRoot "workspace\evals\sp001_skill_evaluation_v5\stone_pickaxe_paired_evaluation_policy_v5.json"
...
try {
    ...
    if ($Mode -eq "PrepareFixture") {
        throw "The v5 recovery launcher cannot prepare or replace the frozen fixture."
    ...
    if ($Mode -eq "RunSP001SkillCandidateV5") {
    ...
    $isSkillArm = $Mode -eq "RunSP001SkillCandidateV5"
    if ($isSkillArm) {
        ...
        $allowedReplicates = [ordered]@{
            "candidate" = @("r13", "r14", "r15")
        }
        if (-not $allowedReplicates.Contains($normalizedArm)) {
            throw "The v5 recovery window only permits the candidate arm."
        }
    ...
    if ($isSkillArm) {
        ...
        & python scripts/stone_pickaxe_skill_evaluation_v5.py write-authorization --arm $normalizedArm --replicate-id $normalizedReplicate --episode-id $episodeId --git-head $gitHead --output $authorizationRelative
    ...
    Start-ControlledRuntime $episodeId $levelName $serverJarSha256
    if ($isSkillArm) {
        & python scripts/stone_pickaxe_skill_evaluation_v5.py run-arm --host $MinecraftHost --port $MinecraftPort --username $Username --bridge-host 127.0.0.1 --bridge-port $BridgePort --arm $normalizedArm --replicate-id $normalizedReplicate --episode-id $episodeId --level-name $levelName --git-head $gitHead --output-dir $outputRelative --fixture "workspace/evals/stone_pickaxe_fixture.json" --authorization $authorizationRelative --restoration $restorationRelative --server-jar-sha256 $serverJarSha256
    }
    else {
    ...
    if ($isSkillArm) {
        & python scripts/stone_pickaxe_skill_evaluation_v5.py refresh-report
