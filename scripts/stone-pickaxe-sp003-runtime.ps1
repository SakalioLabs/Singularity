...
try {
    ...
    & python scripts/stone_pickaxe_sp003_phase122_episode_runner.py audit-authorization --arm $Arm --replicate-id $ReplicateId --episode-id $EpisodeId --git-head $gitHead --git-parent $gitParent --authorization $authorizationRelative
    if ($LASTEXITCODE -ne 0) { throw "SP-003 authorization audit failed." }

    Set-EpisodeServerProperties
    Start-ControlledRuntime $jarSha256
    & python scripts/stone_pickaxe_sp003_phase122_episode_runner.py run --arm $Arm --replicate-id $ReplicateId --episode-id $EpisodeId --level-name $levelName --output-dir $outputRelative --authorization $authorizationRelative --git-head $gitHead --git-parent $gitParent --host $MinecraftHost --port $MinecraftPort --username $Username --bridge-host 127.0.0.1 --bridge-port $BridgePort --fresh-level
