...
- Phase 0/1, the controlled Phase 2 runtime, and the Phase 3/4 candidate-to-advisory lifecycle are implemented under `stone-pickaxe-skill-fixed-v1`: the repository, existing skill history, authorization state, and Probe 21/22/23 evidence hashes are frozen; the fixed protocol and 34-case offline harness are present.
...
- `learned:acquire_cobblestone@1.0.0` exists only as `advisory`; its queue record is `advisory/approved`, all three support arms ran without executing it, candidates `r1`, `r4`, and `r7` are retained failed, and v4 candidates `r10` and `r11` passed. R12 is a consumed infrastructure-ineligible startup failure. Fresh v5 is 3/3 after `candidate/r13`, `candidate/r14`, and `candidate/r15` each machine-verified the goal with one bounded skill completion and no failures or fallback. The new-version promotion gate is approved, but no executable record or normal-runtime permission exists until a separate promotion review after r15 evidence is pushed. `learned:craft_stone_pickaxe` does not exist. Five controlled SP-001 failures, three paired candidate failures, five paired candidate passes, and one paired infrastructure failure remain retained; evaluation still grants no capability or M4 credit.
- Offline verifier fixtures do not count toward a skill gate, capability evidence, or M4. The canonical capability report is unchanged. One new advisory record was appended to `custom_skills.jsonl`; all eight pre-existing skill records remain protected by individual canonical hashes.
...
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 32/32 runtime cases. Four failed preparations, one successful preparation, five failed SP-001 episodes, and three eligible SP-001 successes each ran once and stopped. Eighty SP-001 artifacts are retained and hash-bound.
...
- Learned skills: `learned:acquire_cobblestone@1.0.0` is advisory and cannot execute; `learned:craft_stone_pickaxe` is not created. Existing wooden-pickaxe history is unchanged record-for-record.
...
- Paired evaluation: v1 `r1`, v2 `r4`, and v3 `r7` are consumed failures. V4 inherits all three support arms, binds the exact immutable failure chain and route-scope repair commit, and is `retain_advisory` at 2/3 after candidates `r10` and `r11` passed every evaluation check.
- Runtime boundary: r11 machine-verified in 15.750 seconds with one Planner call, four successful actions, three completed attributed skill digs, zero failures/rejects/fallbacks, and confidence 1.0. Evidence commit `ef40eec7` freezes its eleven artifacts and report hash; Paper/Bridge are stopped and ports are clear. Exactly one `candidate/r12` run becomes authorized only after this separate ledger commit is pushed. Superseded IDs, support reruns, retries, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: `workspace/evals/acquire_cobblestone_promotion.json` records exact `candidate -> advisory` history, three source identities, and capability/M4 credit false. Executable review still requires shadow, advisory, fallback, and three independent eligible candidate pairs, then a separate new-version (`1.1.0`) transition; current `1.0.0` cannot be rewritten in place.
