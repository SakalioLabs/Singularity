...
- M4 is `partial` and still incomplete. BM-011, BM-012, and BM-013 are each repeat-verified at 3/3 after eligible Probe 50 closed BM-013. BM-014 has not run; its exact stick-goal verifier blocker is repaired and passing offline, but live execution remains locked until this repair is committed and pushed and a separate one-use authorization is committed and pushed.
...
- BM-014's exact `Ensure 2 sticks for crafting the iron pickaxe` goal now binds only `stick:2`, preserves the non-binding purpose suffix, preserves explicit multi-action followups, records the real stick-craft delta, and advances through both direct-plank and log-to-plank paths. The bounded repair passes 147 selected Python cases plus 19 Node protocol/smelt cases and is committed/pushed at `ee6f972a`, tree `39b4c719`. Probe 51 now has one zero-retry, skills-off authorization bound to that repair; it may be consumed only after this separate authorization commit is pushed. BM-014 remains 0/3 and Probe 52 is unauthorized.
...
| M4 | Autonomous Survival | **Partial (`partial`)** | BM-011, BM-012, and BM-013 repeat_verified 3/3; BM-014 offline repair passing, live not run and locked |
