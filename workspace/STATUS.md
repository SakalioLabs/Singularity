...
- BM-012 is the next target at 0/3 after six failed live attempts. Probe 6 live-validated the placement-criterion prompt path, then exposed `place_backend_requested_item_equip_grounding`. The generic bridge now applies `m4-place-requested-item-equip-v1`: it finds the exact positive inventory stack, equips it to `hand`, confirms the actual held item, and requires the exact requested block delta. Missing items and ineffective equip fail before `placeBlock` or any world mutation. The offline gate passes and authorizes exactly one fresh Probe 7 after this commit is pushed. BM-013 and BM-014 remain locked.
