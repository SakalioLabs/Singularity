...
- Phases 0-34 are retained under `stone-pickaxe-skill-fixed-v1`; Phase 35 added the SP-002 offline harness, Phases 36-37 retain the first one-time fixture transport failure, and Phases 38-40 retain the second run's progress plus its terminal-action contract failure. The repository, existing skill history, authorization state, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
...
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history. Fresh v5 is 3/3 after `candidate/r13`, `candidate/r14`, and `candidate/r15`; the separate promotion appended executable `1.1.0` with parent and rollback target 1.0.0 plus an approved fingerprint-bound runtime gate. `learned:craft_stone_pickaxe` does not exist. The SP-002 supplemental policy binds those exact promotion artifacts, enforces one exact craft action and one backend attempt, and requires a delayed stable observation. Its 13/13 dedicated offline cases pass. Two fixture authorizations are consumed: the first failed before action on transport, while the second reached exact three cobblestone in four successful actions before a terminal action-bearing plan was rejected. No SP-002 fixture or reusable authorization exists yet.
...
- Runtime boundary: SP-002 fixture preparation and SP-002 live evidence use separate parent-bound one-time authorization scopes. Each authorization must be committed and pushed from clean synchronized `main`; output reuse and automatic retry are forbidden. Both fixture-preparation authorizations are permanently consumed. The next gate is the second failure-evidence commit followed by an offline-only prompt-contract fix and retained-response regression; no new authorization exists. SP-002 live evidence, SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
