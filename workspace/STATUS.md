...
- Runtime boundary: r10 machine-verified in 13.718 seconds with one Planner call, four successful actions, three completed attributed skill digs, zero failures/rejects/fallbacks, and confidence 1.0. Evidence commit `90e4e230` freezes its eleven artifacts and report hash; preflight fix `1f5a4061` is synchronized, Paper/Bridge are stopped, and ports are clear. Exactly one `candidate/r11` run becomes authorized only after this separate ledger commit is pushed. `r12`, superseded IDs, support reruns, retries, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
