...
- `learned:acquire_cobblestone@1.0.0` exists only as `advisory`; its queue record is `advisory/approved`, it has no executable promotion gate, all three support arms ran without executing it, and candidates `r1` and `r4` are retained failed. `learned:craft_stone_pickaxe` does not exist. Five controlled SP-001 failures plus two paired candidate failures remain retained, and the advisory grants no capability or M4 credit.
...
- Paired evaluation: v1 `r1` and v2 `r4` are consumed failures. Isolated v3 policy `stone-pickaxe-sp001-paired-evaluation-v3` binds both prior windows and failures, inherits all three support arms by exact immutable bindings, excludes `r1..r6`, and permits only fresh `r7/r8/r9`. Its initial report is `retain_advisory` at 0/3 with inherited support 3/3 and no errors.
- Runtime boundary: fixes `473b05a` and `36f03ee7` address nearest-observed ordering/re-observation and guard provenance respectively. V3 window `2fc4e04b` is pushed; exactly one `candidate/r7` run becomes authorized only after this separate ledger commit is pushed. `r8/r9`, superseded IDs, support reruns, retries, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
