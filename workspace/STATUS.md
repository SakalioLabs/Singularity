...
- Phases 0-83 retain the verified SP-001/SP-002 history, SP-003 definition, four gameplay failures, one pre-Bridge infrastructure failure, and their bounded repairs. Phase 84 authorizes one new baseline; Phase 85 retains its canopy pickup-reachability failure; Phase 86 verifies a bounded offline egress repair. Earlier skill records, consumed authorizations, and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: six baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. Five produced gameplay failures; one created no episode because Node rejected the bare preload module specifier before Bridge readiness.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. The retained gameplay failures exposed reset-audit interpretation, a Runner API mismatch, craft-progress reconciliation, Pathfinder hidden mutation, and leaf-canopy pickup reachability. Phase 86 now binds an empty leaf-canopy start to the nearest observed distant lower ground block with a clear stand/head cell, rejects all pre-egress log digs, and fails closed without such a target. This repair must be pushed before any new authorization.
...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with retained advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support/candidate pairs both 3/3, and executable `1.0.1` under its approved runtime gate. SP-003 Phase 86 is baseline 0/1 and candidates 0/3; six baseline authorizations are consumed and no SP-003 live authorization exists.
...
- Runtime boundary: SP-002 fixture preparation, source evidence, and paired-evaluation arms use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine source-phase SP-002 authorizations, v1 `shadow/shadow-1`, all six v2 arms, and all six SP-003 baseline authorizations are consumed. No further live arm is authorized. Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: the acquire 1.1.0 and craft 1.0.1 executable transitions are complete and rollback-safe. Phase 86 preserves both gates, the inventory-preserving preload, and the frozen shared bridge while adding only the observation-bound egress selector, guard, and Planner instruction. The next gate is to push this repair, then create and push one fresh parent-bound baseline authorization; candidate execution and automatic retry remain forbidden.
