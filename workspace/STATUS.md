...
- Phases 0-95 retain the verified SP-001/SP-002 history, SP-003 definition, offline surface-clearance and deferred-injection repairs, consumed authorizations, and immutable failures. Phase 96 offline-verifies bounded delayed-log-pickup reconciliation against the retained Phase 95 Agent episode. Earlier skill records and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: ten baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. Eight produced gameplay failures; two created no Agent episode before Bridge readiness.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains the bounded Phase 92 surface-clearance and 1000ms interactive-craft settlement contracts. Phase 96 now reconciles only exact fingerprinted removed-log failures covered by a later same-family pickup surplus, preserving raw failures while the retained replay reaches `3/3` exact sources and unblocks planks. Dedicated tests pass 46/46, all stone tests 227/227, full Python 982/982, and Node 65/65. The full chain is still not live-proven.
