...
- Phase 0/1, the controlled Phase 2 runtime, and the Phase 3 candidate command are implemented under `stone-pickaxe-skill-fixed-v1`: the repository, existing skill lifecycle, authorization state, and Probe 21/22/23 evidence hashes are frozen; the fixed protocol and 33-case offline harness are present.
...
- `learned:acquire_cobblestone` and `learned:craft_stone_pickaxe` do not exist. No candidate, advisory, paired evaluation, or executable promotion has been created. Five failed SP-001 episodes remain retained: three behavioral failures and two zero-action external transport failures. Three independent eligible successes satisfy only the acquisition extraction gate without granting capability or M4 credit.
...
- Current authorization: no further SP-001 live episode. The third success evidence is pushed; candidate extraction becomes available only after the exact-contract lifecycle fix is committed and pushed. SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: retained-session generic extraction now preserves its source path, while the stone-specific builder independently rechecks all source hashes and machine outcomes before emitting exact `learned:acquire_cobblestone` preconditions, observations, postconditions, and bounded template. Candidate creation and advisory promotion are separate commands; neither artifact exists yet. The complete non-live gate passes, including a deterministic runtime-profile marker regression that prevents parent-directory text from satisfying required-suite coverage.
