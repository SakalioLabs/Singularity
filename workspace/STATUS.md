...
- Phases 0-96 retain the verified SP-001/SP-002 history, SP-003 definition, offline surface-clearance, deferred-injection, and delayed-log reconciliation repairs, consumed authorizations, and immutable failures. Phase 97 retains the first post-Phase-96 baseline and isolates a pickup navigation completion-metric mismatch. Earlier skill records and Probe 21/22/23 hashes remain protected.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains bounded surface clearance, interactive-craft settlement, and exact delayed-log reconciliation. Phase 97 live-proves all stages through the wooden pickaxe and one stone. Its second stone drop remains a detected entity because Pathfinder's horizontal `GoalNear(1)` completes while the strict three-dimensional pickup distance is still `1.252876`; the fixed fallback set is empty. The full chain is still not live-proven, and no further live authorization exists before an offline repair is pushed.
