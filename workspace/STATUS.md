...
- Phases 0-51 retain the path through the verified SP-002 fixture, two failed live attempts, and the bounded authoritative-inventory repair. Phases 52/54/56 record separately pushed live authorizations, Phases 53/55/57 retain three eligible SP-002 live successes, and Phase 58 repairs the task-specific candidate source gate offline. The repository, existing skill history, consumed authorizations, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol is unchanged and its harness now has 36 cases.
...
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history and executable `1.1.0` remains approved. `learned:craft_stone_pickaxe` does not exist yet. Four fixture authorizations and five live authorizations are consumed and nonreusable. The third, fourth, and fifth live episodes each completed one exact craft with authoritative `-3 cobblestone`, `-2 stick`, and `+1 stone_pickaxe`, satisfying the three-distinct-success extraction gate.
...
- Current gate: SP-001 3/3; SP-002 3/3 extraction-ready; SP-003 locked.
...
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Craft candidate source reconstruction now validates three task-specific 17-file bundles, exact machine/planner/task-graph transitions, and candidate/advisory separation offline. Extraction is unlocked only after the Phase 58 tooling commit is pushed and synchronized; candidate and advisory stages remain nonexecutable, and no current skill record may be rewritten in place.
