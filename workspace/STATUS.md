...
- Phases 0-51 retain the path through the verified SP-002 fixture, two failed live attempts, and the bounded authoritative-inventory repair. Phases 52/54 record separately pushed live authorizations, and Phases 53/55 retain the first two eligible SP-002 live successes. The repository, existing skill history, consumed authorizations, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
- SP-001 Acquire Cobblestone is 3/3, SP-002 Craft Stone Pickaxe is 2/3, and SP-003 Composite Stone Pickaxe Chain is 0/3 and locked.
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history and executable `1.1.0` remains approved. `learned:craft_stone_pickaxe` does not exist. Four fixture authorizations and four live authorizations are consumed and nonreusable. The third and fourth live episodes each completed one exact craft with authoritative `-3 cobblestone`, `-2 stick`, and `+1 stone_pickaxe`, yielding two distinct eligible SP-002 successes.
...
- Added a bounded authoritative crafting-table inventory refresh and isolated SP-002 post-action machine completion. Two independent revalidations each made exactly one stone pickaxe from the sealed fixture in one backend attempt, passed all machine and evidence gates, and advance SP-002 to 2/3 without changing capability or M4 status.
...
- Current gate: SP-001 3/3; SP-002 2/3; SP-003 locked.
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 32/32 runtime cases; the supplemental SP-002 harness passes 18/18. Four SP-002 live episodes have run under separate authorizations. The first two remain immutable failures. The third and fourth each made one schema-valid zero-retry Planner call and one accepted Mineflayer craft, completed both task nodes, and passed exact material, stable observation, deadline, and evidence-eligibility gates in 5.453 and 5.515 seconds. Paper/Bridge are stopped and controlled ports are clear.
...
- Runtime boundary: SP-002 fixture preparation and live evidence use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All eight SP-002 authorizations are consumed. A fifth live authorization may be created only after the second-success evidence commit is pushed and `main` is clean and synchronized. SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
