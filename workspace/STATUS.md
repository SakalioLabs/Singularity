...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with retained advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support/candidate pairs both 3/3, and executable `1.0.1` under its approved runtime gate. SP-003 Phase 82 is baseline 0/1 and candidates 0/3; five baseline authorizations are consumed and no SP-003 live authorization exists.
...
- Candidate lifecycle: the acquire 1.1.0 and craft 1.0.1 executable transitions are complete and rollback-safe. Phase 80 preserves both gates and the frozen shared bridge. The next gate is to push the Phase 82 infrastructure evidence, then fix and push only the preload specifier; one new parent-bound baseline authorization may follow only after those commits synchronize.
