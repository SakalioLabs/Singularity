...
- Runtime boundary: SP-002 fixture preparation, source evidence, and paired-evaluation arms use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine source-phase SP-002 authorizations and v1 `shadow/shadow-1` are consumed. Recovery v2 currently authorizes no arm; SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Phase 59 candidate extraction is pushed at `eba9c82f`; Phase 60 adds only the reviewed SP-002 advisory record and related ledgers. Phase 61 binds three immutable skill-off baselines. Phase 62 proves the one-action root-plan ordering defect while preserving a valid machine success and zero-retry failure record. Phase 63 binds that failure into isolated policy `stone-pickaxe-sp002-paired-evaluation-v2`, repairs first-cycle routing without modifying v1, and reserves fresh support/candidate IDs. Its initial report is 0/3 and `retain_advisory`; after the definition commit is pushed, the next gate is one separate `shadow/shadow-2` authorization. Executable review and SP-003 remain disabled.
