...
- Phases 0-45 retain the complete path through the verified SP-002 fixture. Phase 46 records the first separately pushed live authorization, Phase 47 retains its zero-action root-plan contract failure, Phase 48 completes the exact offline repair, and Phase 49 is the fresh reauthorization gate. The repository, existing skill history, consumed authorizations, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
...
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history and executable `1.1.0` remains approved. `learned:craft_stone_pickaxe` does not exist. Four fixture authorizations and one live authorization are consumed and nonreusable. The live Planner returned the correct single craft action but omitted root subtasks, so the unchanged validator rejected the plan before execution and SP-002 remains 0/3. Offline replay now preserves those exact rejects, while the repaired prompt supplies the required two-node graph without changing the validator or action limit.
...
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 32/32 runtime cases; the supplemental SP-002 harness passes 16/16. The first SP-002 live episode ran once, made one successful zero-retry Planner transport, executed zero actions, and retained seventeen evidence artifacts. Its inventory stayed at exact three cobblestone, two sticks, and no stone pickaxe. The root-graph prompt repair is offline-verified; no second live episode has run. Paper/Bridge are stopped and controlled ports are clear.
...
- Runtime boundary: SP-002 fixture preparation and live evidence use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All five SP-002 authorizations are consumed. A new authorization may be created only after the offline fix is committed and pushed, the exact fixture tree is re-audited, and `main` is clean and synchronized. SP-002 evidence credit, SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
