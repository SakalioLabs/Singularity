...
- BM-014's exact `Ensure 2 sticks for crafting the iron pickaxe` goal now binds only `stick:2`, preserves the non-binding purpose suffix, preserves explicit multi-action followups, records the real stick-craft delta, and advances through both direct-plank and log-to-plank paths. The bounded repair passes 147 selected Python cases plus 19 Node protocol/smelt cases. Audit `workspace/evals/m4_bm014_stick_goal_verifier_repair_audit.json` grants no live or capability credit; Probe 51 remains unauthorized until this repair is independently committed and pushed.
