...
- `learned:acquire_cobblestone@1.0.0` exists only as `advisory`; its queue record is `advisory/approved`, it has no executable promotion gate, all three support arms ran without executing it, candidates `r1`, `r4`, and `r7` are retained failed, and v4 candidates `r10` and `r11` passed. R12 is a consumed infrastructure-ineligible startup failure. Fresh v5 is 2/3 after `candidate/r13` and `candidate/r14` each machine-verified the goal with one bounded skill completion and no failures or fallback. R14 evidence is pushed at `fe15719a`, and exactly one r15 episode becomes authorized only after this separate ledger commit is pushed. `learned:craft_stone_pickaxe` does not exist. Five controlled SP-001 failures, three paired candidate failures, four paired candidate passes, and one paired infrastructure failure remain retained; the advisory still grants no capability or M4 credit.
