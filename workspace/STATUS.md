...
- Phases 0-78 retain the verified SP-001/SP-002 history, SP-003 definition, three baseline failures, and their bounded repairs. Phase 79 retains the fourth SP-003 baseline failure; Phase 80 isolates inventory-preserving navigation. Earlier skill records, consumed authorizations, and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: four baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. The fourth run reached a placed table, wooden pickaxe, and exactly three stone removals before return navigation consumed all three cobblestone as hidden scaffolding.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. The first retained baseline exposed reset-audit interpretation, the second exposed one nonexistent Runner method call, the third exposed craft-progress reconciliation, and the fourth exposed Pathfinder's default hidden digging/scaffolding. The Phase 80 preload disables those mutations only in the SP-003 process and fails closed on any post-move inventory loss. All four fixes are bounded and tested. No SP-003 live authorization is currently present.
...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with retained advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support/candidate pairs both 3/3, and executable `1.0.1` under its approved runtime gate. SP-003 Phase 80 is offline-verified at baseline 0/1 and candidates 0/3; four baseline authorizations are consumed and no SP-003 live authorization exists.
...
- Runtime boundary: SP-002 fixture preparation, source evidence, and paired-evaluation arms use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine source-phase SP-002 authorizations, v1 `shadow/shadow-1`, all six v2 arms, and all four SP-003 baseline authorizations are consumed. No further live arm is authorized. Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: the acquire 1.1.0 and craft 1.0.1 executable transitions are complete and rollback-safe. Phase 80 preserves both gates and the frozen shared bridge. After this repair commit is pushed and `main` is synchronized, the next gate is one separate parent-bound SP-003 baseline authorization, never a candidate or automatic retry.
