...
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history and executable `1.1.0` remains approved. `learned:craft_stone_pickaxe@1.0.0` exists only as pending candidate `a488cd61`; no advisory or executable skill record exists. Four fixture authorizations and five live authorizations are consumed and nonreusable. The third, fourth, and fifth live episodes each completed one exact craft with authoritative `-3 cobblestone`, `-2 stick`, and `+1 stone_pickaxe`.
...
- Current gate: SP-001 3/3; SP-002 3/3 with candidate `a488cd61` pending review; SP-003 locked.
...
- Learned skills: `learned:acquire_cobblestone@1.0.0` is retained advisory history and `1.1.0` is executable under its approved runtime-default gate. Craft candidate `a488cd61` is nonexecutable and pending; `custom_skills.jsonl` has no craft-stone-pickaxe record. Existing wooden-pickaxe history is unchanged record-for-record.
...
- Runtime boundary: SP-002 fixture preparation and live evidence use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine SP-002 authorizations are consumed, and no further SP-002 evidence episode is required or authorized. SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Phase 58 tooling is pushed at `48000e10`; Phase 59 appended craft candidate `a488cd61` and updated only the candidate queue, learning ledger, and candidate promotion artifact. Advisory review must be a later isolated transition, and SP-002 executable promotion remains disabled.
