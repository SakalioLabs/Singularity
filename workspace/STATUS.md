...
- Phases 0-101 retain the verified SP-001/SP-002 history, SP-003 definition, consumed authorizations, offline repairs, and immutable failures. Phase 101 live-proves the empty-hand chain through one exact stone pickup, then retains one stage-drift failure and one terminal compact-context overflow. Earlier skill records and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: thirteen baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. Eleven produced Agent episodes; two created no Agent episode before Bridge readiness.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains bounded surface clearance, interactive-craft settlement, and exact delayed-log reconciliation. Phase 101 executes 19 actions, completes three of five graph nodes, ends with `wooden_pickaxe:1,cobblestone:1`, and preserves three guard failures. The terminal Planner call carries 7,512 prompt tokens because the nominal compact state includes 2,031 characters of history and 11,687 characters of target proofs; its otherwise parsed response is rejected on `reasoning_too_long`. The full chain is still not live-proven, and the next live authorization remains false pending evidence push plus a separate offline fix.
