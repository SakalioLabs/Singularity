...
- BM-012 is the next target at 0/3 after twenty failed live attempts. Probe 20 live-activated `m4-post-place-crafting-table-machine-observation-v1`: exact placement evidence closed the task at event 273 and remained available at planner observation event 288; the prior same-target loop did not recur. The Runner's event-820 torch `empty_plan` recovered through table, stone-pickaxe, and iron progression. The earliest historical layer is `m4_ready_task_goal_verifier_success_criteria_bypass`: plan event 1189 required `raw_iron:2`, but event 1193 accepted selected `Mine iron ore` from existing `raw_iron:1`; fourteen false one-cycle completions then executed zero actions and exhausted the 24-goal budget with 190.516 seconds remaining. `m4-ready-task-goal-verifier-success-criteria-v1` now passes offline by binding the exact ready-task ID and criteria snapshot and allowing root completion only after that task is machine-completed by state reconciliation or action feedback. Same-title replacement, criteria mutation, status-only completion, malformed binding, and deadline controls fail closed; generic GoalVerifier and M1/M2 behavior are unchanged. No live episode ran in this gate round. Exactly one fresh BM-012 Probe 21 is authorized only after the gate commit is pushed; BM-013/BM-014 remain locked.
