...
- Phases 0-34 are retained under `stone-pickaxe-skill-fixed-v1`; Phase 35 added the SP-002 offline harness, Phases 36-37 retain the first one-time fixture transport failure, Phases 38-39 retain the second run's progress and terminal-action contract failure, Phase 40 fixes that contract offline, Phases 41-42 retain the third authorization plus its root-plan contract failure, and Phase 43 fixes the root graph offline. Phase 44 is a reauthorization gate. The repository, existing skill history, authorization state, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
...
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history. Fresh v5 is 3/3 after `candidate/r13`, `candidate/r14`, and `candidate/r15`; the separate promotion appended executable `1.1.0` with parent and rollback target 1.0.0 plus an approved fingerprint-bound runtime gate. `learned:craft_stone_pickaxe` does not exist. The SP-002 supplemental policy binds those exact promotion artifacts, enforces one exact craft action and one backend attempt, and requires a delayed stable observation. Its 15/15 dedicated offline cases pass. Three fixture authorizations are consumed: the first failed before action on transport, the second reached exact three cobblestone in four successful actions before a terminal action-bearing plan was rejected, and the third was rejected at its zero-subtask root plan before any action. The mode-specific completion and root-graph gates are now fixed offline without relaxing validation. No SP-002 fixture or reusable authorization exists.
