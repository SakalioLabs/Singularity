...
- Phases 0-102 retain the verified SP-001/SP-002 history, SP-003 definition, consumed authorizations, offline repairs, and immutable failures. Phase 101 live-proves the empty-hand chain through one exact stone pickup; Phase 102 repairs its stage drift and compact-context overflow entirely offline. Earlier skill records and Probe 21/22/23 hashes remain protected.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains bounded surface clearance, interactive-craft settlement, and exact delayed-log reconciliation. Phase 102 adds policy `sp003-bounded-planner-state-v1`: 16 progress fields, 15 target fields, one target, no clearance-proof body, and 13 machine-derived action stages. The retained first drift now resolves to `craft_crafting_table`; the terminal state resolves to `acquire_cobblestone` with 1,614 compact characters and a 2,724-character user prompt. SP-003 passes 69/69 and all stone-pickaxe tests pass 250/250. No live episode ran, so the full chain is still not live-proven.
...
- Current gate: SP-001 and SP-002 are independently 3/3 with executable `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains baseline 0/1 and candidates 0/3 after thirteen consumed baseline authorizations; no live authorization exists.
- Phase 102 is an offline-only planner-state repair. It preserves the action guard, frozen protocol, shared Bridge, retained runs, skill records, capability report, and Probe 21/22/23 hashes. Dedicated SP-003 is 69/69, all stone-pickaxe tests are 250/250, and the expected repository Python gate is 1005/1005.
- The next transaction is to push the Phase 102 repair and then push one separate parent-bound, one-use baseline authorization. Candidate execution remains locked until a baseline completes the full empty-hand-to-stone-pickaxe chain.
