...
- Phases 0-117 retain the verified SP-001/SP-002 history, SP-003 definition, consumed authorizations, offline repairs, and immutable failures. Phase 116 stages the single crafting table beside machine-proven stone access without changing the shared Bridge, vendored Pathfinder, or base protocol. Phase 117 live-confirms the carry strategy but exposes rejection of an exact navigation target when the Planner includes its matching y coordinate. Earlier skill records and Probe 21/22/23 hashes remain protected.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Phase 116 is pushed at `c5f120cc`; Phase 117 then completes the wood chain and carries the table, but rejects five exact `(121,137,-33)` targets solely because they include y. Two later x/z-only moves succeed after the budget is spent, and the run terminates at 300 seconds before table placement. Manifest `a7580b5b...` retains the 13-file failure. The full chain is still not live-proven.
...
- Phase 117's exact 13-file failure preserves five exact-target y rejections, three wrong-y rejections, two late successful x/z-only moves, and the 300-second deadline boundary. The consumed authorization cannot be reused.
- The next transaction is the Phase 117 evidence commit and push, followed by an offline-only exact-target parameter-normalization repair. A matching y may be stripped only after exact xyz binding; wrong-y inputs must remain rejected. Candidate execution remains locked until a baseline completes the full chain.
