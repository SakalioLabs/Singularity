...
- `learned:acquire_cobblestone@1.0.0` exists only as `advisory`; its queue record is `advisory/approved`, it has no executable promotion gate, all three support arms ran without executing it, candidates `r1`, `r4`, and `r7` are retained failed, and v4 candidates `r10` and `r11` passed. `learned:craft_stone_pickaxe` does not exist. Five controlled SP-001 failures, three paired candidate failures, and two paired candidate passes remain retained; the advisory still grants no capability or M4 credit.
