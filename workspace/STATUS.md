...
- `learned:acquire_cobblestone@1.0.0` exists only as `advisory`; its queue record is `advisory/approved`, it has no executable promotion gate, all three support arms ran without executing it, and candidates `r1`, `r4`, and `r7` are retained failed. `learned:craft_stone_pickaxe` does not exist. Five controlled SP-001 failures plus three paired candidate failures remain retained, and the advisory grants no capability or M4 credit.
...
- Paired evaluation: v1 `r1`, v2 `r4`, and v3 `r7` are consumed failures. V3 inherited all three support arms and preserved exact controls/state, but its only run failed the lifecycle completion check after all world actions and machine verification passed; the window remains `retain_advisory` at 0/3 and can no longer reach 3/3.
- Runtime boundary: fixes `473b05a` and `36f03ee7` address nearest-observed ordering/re-observation and guard provenance. The r7 finalization defect is now reproduced and fixed offline: selection freezes the routed subtask goal/fingerprint/family, finalization validates them, and the root family remains separate evidence. Exact r7 replay succeeds as `mining`; wrong-family and tampered routes still fail closed. `r8/r9`, superseded IDs, support reruns, retries, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized until a fresh non-overlapping window is committed and pushed.
