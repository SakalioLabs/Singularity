...
- Runtime boundary: SP-002 fixture preparation, source evidence, and paired-evaluation arms use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine source-phase SP-002 authorizations and v1 `shadow/shadow-1` are consumed. No further paired arm is authorized; SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Phase 59 candidate extraction is pushed at `eba9c82f`; Phase 60 adds only the reviewed SP-002 advisory record and related ledgers. Phase 61 binds three immutable skill-off baselines. Phase 62 proves the one-action root-plan ordering defect while preserving a valid machine success and zero-retry failure record. The next gate is an offline non-overlapping v2 policy and first-cycle skill-routing repair; executable review and SP-003 remain disabled.
