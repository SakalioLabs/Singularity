...
- Phases 0-45 retain the complete path through the verified SP-002 fixture. Phase 46 records the first separately pushed live authorization, Phase 47 retains its zero-action root-plan contract failure, Phase 48 completes that exact offline repair, Phase 49 records the second authorization, Phase 50 retains its target-created but ineligible result, and Phase 51 is an offline-only repair gate. The repository, existing skill history, consumed authorizations, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
...
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history and executable `1.1.0` remains approved. `learned:craft_stone_pickaxe` does not exist. Four fixture authorizations and two live authorizations are consumed and nonreusable. The second live episode produced one real stone pickaxe with one accepted craft attempt, but stick consumption remained zero and the terminal continuation was schema-invalid; SP-002 therefore remains 0/3.
...
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 32/32 runtime cases; the supplemental SP-002 harness passes 16/16. Two SP-002 live episodes have run under separate authorizations. The first executed zero actions. The second made two successful zero-retry Planner transports and one successful craft action; its terminal inventory has one stone pickaxe, zero cobblestone, and two sticks. Exact stick-delta verification and terminal plan controls failed, so neither episode is eligible. Paper/Bridge are stopped and controlled ports are clear.
...
- Runtime boundary: SP-002 fixture preparation and live evidence use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All six SP-002 authorizations are consumed. The next gate is offline-only replay and repair of exact material-delta binding plus post-action terminal completion; a fix must be committed and pushed before any new authorization. SP-002 evidence credit, SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
