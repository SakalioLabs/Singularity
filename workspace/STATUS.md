...
- Phases 0-51 retain the path through the verified SP-002 fixture, two failed live attempts, and the bounded authoritative-inventory repair. Phase 52 records the third separately pushed live authorization, and Phase 53 retains the first eligible SP-002 live success. The repository, existing skill history, consumed authorizations, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
- SP-001 Acquire Cobblestone is 3/3, SP-002 Craft Stone Pickaxe is 1/3, and SP-003 Composite Stone Pickaxe Chain is 0/3 and locked.
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history and executable `1.1.0` remains approved. `learned:craft_stone_pickaxe` does not exist. Four fixture authorizations and three live authorizations are consumed and nonreusable. The third live episode completed one exact craft with authoritative `-3 cobblestone`, `-2 stick`, and `+1 stone_pickaxe`, so it is the first eligible SP-002 success.
...
- Added a bounded authoritative crafting-table inventory refresh and isolated SP-002 post-action machine completion. The first revalidation made exactly one stone pickaxe from the sealed fixture in one backend attempt, passed all machine and evidence gates, and advances SP-002 to 1/3 without changing capability or M4 status.
...
- Current gate: SP-001 3/3; SP-002 1/3; SP-003 locked.
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 32/32 runtime cases; the supplemental SP-002 harness passes 18/18. Three SP-002 live episodes have run under separate authorizations. The first two remain immutable failures. The third made one schema-valid zero-retry Planner call and one accepted Mineflayer craft, completed both task nodes, and passed exact material, stable observation, deadline, and evidence-eligibility gates in 5.453 seconds. Paper/Bridge are stopped and controlled ports are clear.
...
- Runtime boundary: SP-002 fixture preparation and live evidence use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All seven SP-002 authorizations are consumed. A fourth live authorization may be created only after the first-success evidence commit is pushed and `main` is clean and synchronized. SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
