...
- Phase 0/1, the controlled Phase 2 runtime, and the Phase 3 candidate lifecycle are implemented under `stone-pickaxe-skill-fixed-v1`: the repository, existing skill lifecycle, authorization state, and Probe 21/22/23 evidence hashes are frozen; the fixed protocol and 33-case offline harness are present.
...
- Candidate `e1534e57` for `learned:acquire_cobblestone` now exists at `candidate/pending` with three distinct hash-verified live sources and an `advisory_ready` review. Neither `learned:acquire_cobblestone` nor `learned:craft_stone_pickaxe` exists in the skill library; no advisory, paired evaluation, or executable promotion has been created. Five failed SP-001 episodes remain retained, and the candidate grants no capability or M4 credit.
...
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 33/33 protocol and 30/30 runtime cases. Four failed preparations, one successful preparation, five failed SP-001 episodes, and three eligible SP-001 successes each ran once and stopped. Eighty SP-001 artifacts are retained and hash-bound.
...
- Learned skills: `learned:acquire_cobblestone` and `learned:craft_stone_pickaxe` are not created. Candidate `e1534e57` is pending and cannot execute. Existing wooden-pickaxe history is unchanged.
...
- Current authorization: no further SP-001 live episode. Advisory promotion is permitted only after candidate `e1534e57` and its candidate-stage artifacts are committed and pushed. SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: `workspace/evals/acquire_cobblestone_promotion.json` records the candidate stage, exact contract, three source identities, and `promote_advisory` review while explicitly setting capability and M4 credit false. The queue and learning ledger contain the same candidate; `custom_skills.jsonl` is unchanged. Advisory promotion remains a separate command.
