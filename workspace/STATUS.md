...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Phase 114 is pushed at `ae43ecbc`; Phase 115 then reaches exact `cobblestone:3, stick:2` through 21 successful actions. It does not craft the stone pickaxe: action 20 removes the sole climbable egress anchor, action 21 descends into a feet-level enclosure, and eleven later table moves fail at an unchanged position. Manifest `77a95b8e...` retains the 13-file failure. The full chain is still not live-proven.
...
- Phase 115's exact 13-file failure reaches three cobblestone but destroys its only egress anchor before the terminal craft. Its replay, geometry, eleven failed return moves, and all hashes are now the evidence gate.
- The next transaction is the Phase 115 evidence commit and push, followed by an offline-only SP-003 egress-anchor repair. Only after that repair is separately committed and pushed may a fresh parent-bound one-use baseline authorization be created; candidate execution remains locked until a baseline completes the full empty-hand-to-stone-pickaxe chain.
