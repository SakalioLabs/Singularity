...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with retained advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support/candidate pairs both 3/3, and executable `1.0.1` under its approved runtime gate. SP-003 is unlocked for protocol definition and offline verification only; no SP-003 live authorization exists.
...
- Runtime boundary: SP-002 fixture preparation, source evidence, and paired-evaluation arms use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine source-phase SP-002 authorizations, v1 `shadow/shadow-1`, and all six v2 arms are consumed. No further SP-002 arm is authorized. SP-003 protocol work may proceed offline after the promotion push, while SP-003 live execution, Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: the acquire 1.1.0 and craft 1.0.1 executable transitions are complete and rollback-safe. Phase 70 supplies the tested task-specific append-only path; Phase 71 applies it exactly once, preserves every prior record, verifies idempotent reexecution, and proves the runtime gate selects only craft version `1.0.1`. The next gate is the SP-003 composite protocol and offline verifier, not a live episode.
