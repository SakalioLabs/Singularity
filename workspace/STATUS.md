...
- Runtime boundary: fixes `473b05a` and `36f03ee7` address nearest-observed ordering/re-observation and guard provenance. R7 exposed a separate finalization defect: the legally selected `mining` subtask route was overwritten by the outer root's inferred `gathering` family, producing a false `routing_error` with postconditions and root success both true. `r8/r9`, superseded IDs, support reruns, retries, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized while the offline route-scope fix and a new non-overlapping window are prepared.
