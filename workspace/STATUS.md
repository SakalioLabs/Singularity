...
- Phases 0-86 retain the verified SP-001/SP-002 history, SP-003 definition, five gameplay failures, one pre-Bridge infrastructure failure, and their bounded repairs. Phase 87 authorizes one new baseline; Phase 88 retains its stone-drop vertical pickup failure after live-validating canopy egress. Earlier skill records, consumed authorizations, and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: seven baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. Six produced gameplay failures; one created no episode because Node rejected the bare preload module specifier before Bridge readiness.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Phase 88 proves Phase 86's leaf-canopy egress and the complete prefix through equipped wooden pickaxe. The remaining blocker is stone geometry: selected surface-visible stone sits three blocks below the player, can be removed, but its drop cannot be reached by the no-dig/no-scaffold Pathfinder. A bounded grounded pre-dig approach is required before any new authorization.
...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with retained advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support/candidate pairs both 3/3, and executable `1.0.1` under its approved runtime gate. SP-003 Phase 88 is baseline 0/1 and candidates 0/3; seven baseline authorizations are consumed and no SP-003 live authorization exists.
...
- Runtime boundary: SP-002 fixture preparation, source evidence, and paired-evaluation arms use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine source-phase SP-002 authorizations, v1 `shadow/shadow-1`, all six v2 arms, and all seven SP-003 baseline authorizations are consumed. No further live arm is authorized. Probe 24, full BM-012, and iron mining remain unauthorized.
