...
- Phases 0-100 retain the verified SP-001/SP-002 history, SP-003 definition, consumed authorizations, offline repairs, and immutable failures. Phase 98 passes retained replays for the Phase 97 table-reference, movement-metric, and pickup-goal mismatches. Phase 99 retains a zero-action external Planner HTTP 500; Phase 100 re-establishes the same fixed provider with one zero-retry exact-response probe and no Minecraft process. Earlier skill records and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: twelve baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. Ten produced Agent episodes; two created no Agent episode before Bridge readiness.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains bounded surface clearance, interactive-craft settlement, and exact delayed-log reconciliation. Phase 97 live-proves all stages through the wooden pickaxe and one stone, then exposes a detected second drop at strict distance `1.252876`. Phase 98 keeps the shared Bridge byte-identical, maps only SP-003 process-local three-dimensional `GoalNear(1)` to range 0, aligns stone-approach block-cell and continuous metrics, and repairs at most one observed unsafe table reference. Phase 99's only root Planner call fails with HTTP 500 before any plan or action, so none of those repairs is exercised. Phase 100 proves provider recovery in one zero-retry request. Offline gates pass at 50/50 SP-003, 231/231 stone-prefixed Python, 986/986 full Python, and 67/67 Node assertions. The full chain is still not live-proven; the next authorization remains false until the Phase 100 health proof is pushed.
...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with retained advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support/candidate pairs both 3/3, and executable `1.0.1` under its approved runtime gate. SP-003 Phase 92 is offline-verified at Python 969/969 and Node 61/61, but remains baseline 0/1 and candidates 0/3; eight baseline authorizations are consumed and no SP-003 live authorization exists.
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 36/36 protocol and 32/32 runtime cases; the supplemental SP-002 harness passes 18/18. Five SP-002 live episodes have run under separate authorizations. The first two remain immutable failures. The last three each made one schema-valid zero-retry Planner call and one accepted Mineflayer craft, completed both task nodes, and passed exact material, stable observation, deadline, and evidence-eligibility gates in 5.453, 5.515, and 6.250 seconds. Paper/Bridge are stopped and controlled ports are clear.
- Fixture state: `workspace/evals/stone_pickaxe_fixture.json` remains sealed at 45 files and 14,684,703 bytes. New `workspace/evals/stone_pickaxe_sp002_fixture.json` is independently verified at 48 files and 14,962,077 bytes with exact `cobblestone:3`, `stick:2`, `stone_pickaxe:0`, an interactive table at 1.414 blocks, and tree `e5201d33a5eb2b9a0d52c0c8be5165363c95e61ef5fc3e88644b3c3d70e2dc0c`. Controlled ports are clear and Paper/Bridge are stopped.
- Offline repair: held-tool state and redundant-equip guards remain enforced. The pickup backend admits one safe adjacent standable candidate within a fixed 0.5-block selection margin when `GoalNear(1)` resolves outside the measured envelope. If that candidate aliases the player's current block cell, it performs at most one 100ms forward nudge toward the safe expected position before the existing `GoalBlock` fallback. The one-fallback limit is unchanged, and completion still requires measured one-block distance or observed inventory delta. Positive and negative grounded tests, retained evidence replay, and the third eligible live success verify the path; protocol identity is unchanged.
- Learned skills: `learned:acquire_cobblestone@1.0.0` is retained advisory history and `1.1.0` is executable under its approved runtime-default gate. `learned:craft_stone_pickaxe@1.0.0` is retained advisory history and append-only `1.0.1` is executable under approved gate fingerprint `47840e03...`, with rollback to `1.0.0`. Existing gather-wood, table, and wooden-pickaxe histories are unchanged record-for-record.
...
- Paired evaluation: v1 `r1`, v2 `r4`, and v3 `r7` are consumed failures. V4 is closed at 2/3 after `r10/r11` passed and r12 failed before bot readiness. Isolated v5 is complete at 3/3 after `r13/r14/r15`; every retained policy, report, and run remains hash-bound.
- Runtime boundary: SP-002 fixture preparation, source evidence, and paired-evaluation arms use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine source-phase SP-002 authorizations, v1 `shadow/shadow-1`, all six v2 arms, and all eight SP-003 baseline authorizations are consumed. No further live arm is authorized. Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: the acquire 1.1.0 and craft 1.0.1 executable transitions are complete and rollback-safe. Phase 92 preserves both gates, the frozen shared bridge, the base protocol, and all retained evidence. The next gate is to push the Phase 92 offline repair, then create and push one separate fresh parent-bound baseline authorization; candidate execution and automatic retry remain forbidden.
