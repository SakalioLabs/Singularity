...
- `learned:acquire_cobblestone@1.0.0` exists only as `advisory`; its queue record is `advisory/approved`, it has no executable promotion gate, all three support arms ran without executing it, candidates `r1`, `r4`, and `r7` are retained failed, and v4 candidates `r10` and `r11` passed. R12 is a consumed infrastructure-ineligible startup failure. Fresh v5 is 1/3 after `candidate/r13` machine-verified the goal with one bounded skill completion and no failures or fallback; r13 evidence is pushed at `24f28911`, and exactly one r14 episode becomes authorized only after this separate ledger commit is pushed. R15 remains unauthorized. `learned:craft_stone_pickaxe` does not exist. Five controlled SP-001 failures, three paired candidate failures, three paired candidate passes, and one paired infrastructure failure remain retained; the advisory still grants no capability or M4 credit.
