...
- Phases 0-34 are retained under `stone-pickaxe-skill-fixed-v1`; Phase 35 added the SP-002 offline harness, Phase 36 committed its first one-time fixture authorization, and Phase 37 retains that run's external Planner transport failure. The repository, existing skill history, authorization state, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
...
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history. Fresh v5 is 3/3 after `candidate/r13`, `candidate/r14`, and `candidate/r15`; the separate promotion appended executable `1.1.0` with parent and rollback target 1.0.0 plus an approved fingerprint-bound runtime gate. `learned:craft_stone_pickaxe` does not exist. The SP-002 supplemental policy binds those exact promotion artifacts, enforces one exact craft action and one backend attempt, and requires a delayed stable observation. Its 13/13 dedicated offline cases pass. Authorization `6664082c` was consumed exactly once, but the sole Planner request failed at transport before any action, so no SP-002 fixture or reusable authorization exists yet.
...
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 32/32 runtime cases; the supplemental SP-002 harness passes 13/13. Four earlier failed preparations, one successful preparation, the first SP-002 fixture-preparation transport failure, five failed SP-001 episodes, and three eligible SP-001 successes each ran once and stopped. Eighty SP-001 artifacts and eleven SP-002 failure-evidence files are hash-bound. Paper/Bridge are stopped and controlled ports are clear.
...
- Runtime boundary: SP-002 fixture preparation and SP-002 live evidence use separate parent-bound one-time authorization scopes. Each authorization must be committed and pushed from clean synchronized `main`; output reuse and automatic retry are forbidden. The first fixture-preparation authorization is permanently consumed. The next gate is its failure-evidence commit and push, followed by one fresh fixture-preparation authorization only. SP-002 live evidence, SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
