...
- Phases 0-89 retain the verified SP-001/SP-002 history, SP-003 definition, prior failures, and bounded repairs. Phase 90 authorizes one baseline against the grounded stone-approach repair; Phase 91 retains its buried-stone clearance failure. Earlier skill records, consumed authorizations, and Probe 21/22/23 hashes remain protected.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Phase 91 again proves the complete prefix through equipped wooden pickaxe and exercises Phase 89's horizontal stone-column probe. The complete radius-one scan returned 29 non-air cells, but both exact stone supports at y=139 had dirt in their stand and head cells, so the fail-closed runtime exposed no dig target. The same run also repeated a first-attempt wooden-pickaxe craft rollback before a later craft succeeded, making it independently ineligible. A bounded SP-003-only craft-settlement repair and machine-proven surface-clearance contract are required before any new authorization.
...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with retained advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support/candidate pairs both 3/3, and executable `1.0.1` under its approved runtime gate. SP-003 Phase 91 is baseline 0/1 and candidates 0/3; eight baseline authorizations are consumed and no SP-003 live authorization exists.
...
- Candidate lifecycle: the acquire 1.1.0 and craft 1.0.1 executable transitions are complete and rollback-safe. Phase 88 preserves both gates, the inventory-preserving preload, the frozen shared bridge, and the live-proven canopy egress. The next gate is to push the retained stone-pickup failure and implement a bounded offline grounded approach; candidate execution and automatic retry remain forbidden.
