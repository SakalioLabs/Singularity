...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support 3/3, and candidate pairs 2/3; SP-003 locked.
...
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Phase 63 repairs first-cycle routing in isolated policy v2, and Phases 64-66 prove all three support arms. Phases 67/68 prove `candidate/r4` and `candidate/r5` against distinct paired baselines with matching fixed controls and initial state, one verified and reobserved skill action each, and attribution confidence 1.0. Support is 3/3 and candidates are 2/3; after this evidence commit is pushed, the next gate is one separate `candidate/r6` authorization. Executable review and SP-003 remain disabled.
