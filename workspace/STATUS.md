...
- Phases 0-45 retain the complete path through the verified SP-002 fixture. Phase 46 records the first separately pushed live authorization, Phase 47 retains its zero-action root-plan contract failure, and Phase 48 is an offline-only repair gate. The repository, existing skill history, consumed authorizations, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
...
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 32/32 runtime cases; the supplemental SP-002 harness passes 15/15. The first SP-002 live episode ran once, made one successful zero-retry Planner transport, executed zero actions, and retained seventeen evidence artifacts. Its inventory stayed at exact three cobblestone, two sticks, and no stone pickaxe. Paper/Bridge are stopped and controlled ports are clear.
...
- Runtime boundary: SP-002 fixture preparation and live evidence use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All five SP-002 authorizations are consumed. The next gate is offline replay and a mode-specific live root-graph prompt repair, followed by a pushed fix before any fresh authorization. SP-002 evidence credit, SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
