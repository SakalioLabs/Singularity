...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with advisory `1.0.0`, fixed paired baselines 3/3, v1 shadow 0/1 after one consumed failure, and candidate pairs 0/3; SP-003 locked.
...
- Runtime boundary: SP-002 fixture preparation, source evidence, and paired-evaluation arms use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine source-phase SP-002 authorizations, v1 `shadow/shadow-1`, and v2 `shadow/shadow-2` are consumed. Recovery v2 currently authorizes no further arm; SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Phase 59 candidate extraction is pushed at `eba9c82f`; Phase 60 adds only the reviewed SP-002 advisory record and related ledgers. Phase 61 binds three immutable skill-off baselines. Phase 62 proves the one-action root-plan ordering defect. Phase 63 repairs first-cycle routing in isolated policy v2, and Phase 64 proves the repair live: `shadow-2` passed with one shadow plan before the real root Planner call and zero direct skill execution. Support is 1/3 and candidates are 0/3; after this evidence commit is pushed, the next gate is one separate `advisory/advisory-2` authorization. Executable review and SP-003 remain disabled.
