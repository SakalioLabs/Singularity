...
- Phases 0-104 retain the verified SP-001/SP-002 history, SP-003 definition, consumed authorizations, offline repairs, and immutable failures. Phase 103 live-exercises bounded planner state through an equipped wooden pickaxe; Phase 104 repairs its exact horizontal-navigation schema contradiction offline. Earlier skill records and Probe 21/22/23 hashes remain protected.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains bounded surface clearance, interactive-craft settlement, exact delayed-log reconciliation, and bounded planner state. Phase 104 adds `sp003-horizontal-move-envelope-v1`: strict SP-003 `move_to` requires `x/z` and validates optional `y`, while `look_at` and non-SP-003 moves remain `x/y/z`. The retained Phase 103 call-12 state now validates without rewriting its action. SP-003 passes 72/72 and all stone-pickaxe tests pass 253/253. The full chain is still not live-proven.
...
- Phase 103's zero-action-failure evidence is pushed at `06a8327a`. Phase 104 preserves its 13-file bundle byte-for-byte and changes only the planner envelope, supplemental policy identity, tests, audit, and status records. Dedicated SP-003 is 72/72, all stone-pickaxe tests are 253/253, and the repository Python gate is 1008/1008.
- The next transactions are to push Phase 104 and only then push one separate parent-bound, one-use baseline authorization. Candidate execution remains locked until a baseline completes the full empty-hand-to-stone-pickaxe chain.
