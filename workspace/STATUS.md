...
- Phases 0-106 retain the verified SP-001/SP-002 history, SP-003 definition, consumed authorizations, offline repairs, and immutable failures. Phase 105 live-exercises the horizontal-navigation schema repair and exposes the covered-stone pickup boundary; Phase 106 adds the offline pre-dig pickup-access proof. Earlier skill records and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: fifteen baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. Thirteen produced Agent episodes; two created no Agent episode before Bridge readiness.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains bounded surface clearance, interactive-craft settlement, exact delayed-log reconciliation, bounded planner state, and `sp003-horizontal-move-envelope-v1`. Phase 105 made all 27 Planner calls schema-valid and completed through wooden-pickaxe equip, then exposed two covered-stone removals with no cobblestone pickup. Phase 106 adds `sp003-pre-dig-pickup-access-v1`: exact stone target/support, machine-proven post-dig head air, verifier-reconstructed complete or strict-priority visibility, stripped caller proofs, and clearance bounds of three per shaft and six per episode. The frozen Bridge and protocol remain unchanged. SP-003 passes 77/77, all stone-pickaxe tests pass 258/258, and full Python passes 1013/1013. The full chain is still not live-proven.
...
- Current gate: SP-001 and SP-002 are independently 3/3 with executable `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains baseline 0/1 and candidates 0/3 after fifteen consumed baseline authorizations; no live authorization exists.
- Phase 105 retains the exact 13-file failed bundle and a machine replay of both covered-stone pickup failures. Phase 106 offline repair audit `465a39fc...` binds the exact pre-dig pickup-access contract. Dedicated SP-003 is 77/77, all stone-pickaxe tests are 258/258, and the repository Python gate is 1013/1013.
- The next transaction is the Phase 106 offline-fix commit and push; only then may a separate fresh parent-bound baseline authorization be committed and pushed. Candidate execution remains locked until a baseline completes the full empty-hand-to-stone-pickaxe chain.
