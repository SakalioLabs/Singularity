...
- `learned:acquire_cobblestone@1.0.0` exists only as `advisory`; its queue record is `advisory/approved`, it has no executable promotion gate, all three support arms ran without executing it, and candidate `r1` is retained failed after one direct execution attempt. `learned:craft_stone_pickaxe` does not exist. Five controlled SP-001 failures plus one paired candidate failure remain retained, and the advisory grants no capability or M4 credit.
...
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 30/30 runtime cases. Four failed preparations, one successful preparation, five failed SP-001 episodes, and three eligible SP-001 successes each ran once and stopped. Eighty SP-001 artifacts are retained and hash-bound.
...
- Paired evaluation: supplemental policy `stone-pickaxe-sp001-paired-evaluation-v1` binds the exact advisory/candidate/promotion artifacts, fixture tree, and three retained baseline bundles without changing the base protocol. The baseline index is 3/3 with equal fixed-control and initial-state fingerprints. All support arms passed. Candidate `r1` matched controls/state but failed when the skill runtime and SP-001 guard disagreed on tied nearest-observed ordering; the report remains `retain_advisory` at 0/3 candidate pairs.
- Runtime boundary: candidate `r1` is consumed and cannot be retried. Its first skill dig was verifier-blocked before a world command, ordinary planning recovered the root goal, and all eleven artifacts are retained. The offline fix aligns observed-distance ordering and forces one acquire dig per observation; 40/40 non-live Python scripts and 6/6 Node suites pass. No live arm is currently authorized until this fix and a new non-overlapping evaluation window are committed and pushed. SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
