...
- Phases 0-80 retain the verified SP-001/SP-002 history, SP-003 definition, four baseline failures, and their bounded repairs. Phase 81 authorizes one new baseline; Phase 82 retains its pre-Bridge infrastructure failure; Phase 83 repairs only the preload module specifier. Earlier skill records, consumed authorizations, and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: five baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. Four produced gameplay failures; the fifth created no episode because Node rejected the bare preload module specifier before Bridge readiness.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. The first retained baseline exposed reset-audit interpretation, the second a Runner API mismatch, the third craft-progress reconciliation, and the fourth Pathfinder's hidden digging/scaffolding. The fifth authorization stopped at Node preload resolution with zero gameplay. Its world and authorization are closed; the one-line `./src/...` repair passes real resolution and full offline regression but must be pushed before any new authorization.
...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with retained advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support/candidate pairs both 3/3, and executable `1.0.1` under its approved runtime gate. SP-003 Phase 83 is baseline 0/1 and candidates 0/3; five baseline authorizations are consumed and no SP-003 live authorization exists.
...
- Runtime boundary: SP-002 fixture preparation, source evidence, and paired-evaluation arms use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine source-phase SP-002 authorizations, v1 `shadow/shadow-1`, all six v2 arms, and all five SP-003 baseline authorizations are consumed. No further live arm is authorized. Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: the acquire 1.1.0 and craft 1.0.1 executable transitions are complete and rollback-safe. Phase 83 preserves both gates, the preload implementation, and the frozen shared bridge. The next gate is to push this repair, then create and push one new parent-bound baseline authorization; candidate execution and automatic retry remain forbidden.
