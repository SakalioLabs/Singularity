...
- `learned:acquire_cobblestone@1.0.0` exists only as `advisory`; its queue record is `advisory/approved`, it has no executable promotion gate, and all three support arms have run without executing it. `learned:craft_stone_pickaxe` does not exist. Five failed SP-001 episodes remain retained, and the advisory grants no capability or M4 credit.
...
- Paired evaluation: supplemental policy `stone-pickaxe-sp001-paired-evaluation-v1` binds the exact advisory/candidate/promotion artifacts, fixture tree, and three retained baseline bundles without changing the base protocol. The baseline index is 3/3 with equal fixed-control and initial-state fingerprints. All support arms completed four verified actions with no direct skill selection/execution; fallback emitted exactly one fail-closed event and ordinary planning recovered. The report remains `retain_advisory` at 0/3 candidate pairs.
- Runtime boundary: one clean synchronized `main` invocation may consume one exact arm/replicate. Evaluation keeps the skill artifact read-only, recomputes run metrics from bound source evidence, rejects duplicate replicates, and grants neither normal runtime nor capability/M4 credit. After the fallback evidence commit is pushed, only `candidate/r1` is next; SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
