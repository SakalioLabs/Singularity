...
- Paired evaluation: v1 `r1`, v2 `r4`, and v3 `r7` are consumed failures. V4 inherits all three support arms, binds the exact immutable failure chain and route-scope repair commit, and starts at `retain_advisory` 0/3 with fresh candidates `r10/r11/r12`.
- Runtime boundary: fixes `473b05a`, `36f03ee7`, and `18525025` address nearest-observed ordering/re-observation, guard provenance, and routed-subtask finalization. Exact r7 replay succeeds as `mining`; wrong-family and tampered routes still fail closed. V4 policy identity and 11/11 cases pass, but no live arm is authorized until the window commit is pushed and a separate `r10` authorization is committed. Superseded IDs, support reruns, retries, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
