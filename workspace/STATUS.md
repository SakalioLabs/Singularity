...
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 31/31 runtime cases. Four failed preparations, one successful preparation, five failed SP-001 episodes, and three eligible SP-001 successes each ran once and stopped. Eighty SP-001 artifacts are retained and hash-bound.
...
- Paired evaluation: v1 remains `retain_advisory` at 0/3 after consumed candidate `r1` exposed the tied nearest-observed ordering disconnect. Isolated recovery policy `stone-pickaxe-sp001-paired-evaluation-v2` permits only `r4/r5/r6`, maps the three retained baselines to new pair IDs, inherits support arms by exact immutable bindings, and excludes all v1 candidate replicate IDs. Its initial report is `retain_advisory` at 0/3 with inherited support 3/3 and no errors.
- Runtime boundary: candidate `r1` is consumed and cannot be retried. Its first skill dig was verifier-blocked before a world command, ordinary planning recovered the root goal, and all eleven artifacts are retained. Runtime fix `473b05a` aligns observed-distance ordering and forces one acquire dig per observation. The v2 policy and its ten tests are ready, but no live arm is authorized until this window commit is pushed and a separate ledger update grants exactly one `candidate/r4` run. SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
