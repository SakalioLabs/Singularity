...
- Current authorization: no further SP-001 live episode. The paired baseline/candidate evaluation harness may be prepared offline only after the advisory artifacts are committed and pushed; no paired live run is authorized yet. SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: `workspace/evals/acquire_cobblestone_promotion.json` now records exact `candidate -> advisory` history, three source identities, and capability/M4 credit false. The queue, learning ledger, and sole advisory skill record agree on candidate `e1534e57`; executable selection remains blocked until three independent paired evaluations and a separate promotion gate pass.
