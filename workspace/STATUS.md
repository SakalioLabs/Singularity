...
- Paired evaluation: supplemental policy `stone-pickaxe-sp001-paired-evaluation-v1` binds the exact advisory/candidate/promotion artifacts, fixture tree, and three retained baseline bundles without changing the base protocol. The baseline index is 3/3 with equal fixed-control and initial-state fingerprints. Episode `sp001_skill_shadow_20260718_043048_bee49f0b` verified the shadow arm with four successful actions, three shadow plans, and no direct skill selection/execution; the report remains `retain_advisory` at 0/3 candidate pairs.
- Runtime boundary: one clean synchronized `main` invocation may consume one exact arm/replicate. Evaluation keeps the skill artifact read-only, recomputes run metrics from bound source evidence, rejects duplicate replicates, and grants neither normal runtime nor capability/M4 credit. After the shadow evidence commit is pushed, only `advisory/advisory-1` is next; SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
