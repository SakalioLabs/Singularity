# STATUS.md | Last updated: 2026-07-27
...
- M4 is `live_observed` but still incomplete. BM-011, BM-012, and BM-013 are each repeat-verified at 3/3 after eligible Probe 50 closed BM-013. BM-014 is live-observed at 1/3 after four authorized attempts: Probes 51, 53, and 54 are retained as three failures, while Probe 52 independently completed the full empty-inventory iron-pickaxe chain. Two independently eligible successes remain required.
...
- BM-012 is `repeat_verified` at 3/3 after Probe 46. Probe 38 episode `m4_episode_20260726_202651_d80a4946` completed the full empty-hand-to-stone-pickaxe loop and mined eight machine-proven iron-ore sources into `raw_iron:8`. Probe 45 episode `m4_episode_20260727_010123_3a9ca67f` live-validated the candidate-bound table-placement repair and passed in 256.391 seconds. Probe 46 episode `m4_episode_20260727_012016_7ef19ad6` independently replicated the full chain, mined eight distinct verified iron-ore sources into `raw_iron:8`, preserved health/hunger at 20, passed all 74 eligibility checks, and finished in 353.687 seconds with 246.313 seconds remaining. The forty-three ineligible attempts remain retained failure evidence. BM-013 is now `repeat_verified` at 3/3; BM-014 is the active sequential target.
- BM-013 is `repeat_verified` after eligible Probes 48, 49, and 50. Probe 50 episode `m4_episode_20260727_111149_e1794e58` consumed its one-use authorization at `autonomous_start` line 2, completed all nine goals in 59 cycles, and machine-smelted `iron_ingot:1` from episode-acquired raw iron and coal. It passed 74/74 checks in 271.922 seconds, sealed with 28.000 seconds remaining, kept health/hunger at 20, and had zero deaths. Its 18 failed actions were recovered noise, including one physical furnace placement observed after a backend block-update timeout. Probe 50 evidence is committed and pushed at `84b9817d`, tree `adf8399f`.
- BM-014's exact `Ensure 2 sticks for crafting the iron pickaxe` goal now binds only `stick:2`, preserves the non-binding purpose suffix, preserves explicit multi-action followups, records the real stick-craft delta, and advances through both direct-plank and log-to-plank paths. The bounded verifier repair passes 147 selected Python cases plus 19 Node protocol/smelt cases and is committed/pushed at `ee6f972a`, tree `39b4c719`.
- Probe 51 episode `m4_episode_20260727_121623_ce060317` legally consumed its one-use authorization at JSONL line 2 and failed eligibility 67/74 at `300.172s`. It completed eight goals through furnace crafting, retained `raw_iron:3`, `coal:1`, and `furnace:1`, then made 40 failed furnace-place actions against 36 stone and four clay target cells. The truncated non-air observation made every target appear merely `not_observed_occupied`; no smelt, stick goal, or iron-pickaxe action ran, so the stick repair was not live-exercised.
- The bounded offline follow-up derives furnace candidates only from the already-collected complete 36-cell Mineflayer local snapshot: both the solid reference and its replaceable cell above must be present in the same machine snapshot. ActionVerifier hard-requires the complete, at-most-five-second-old envelope and exact integral reference/target pair, unions snapshot-time and current player collision cells, and records `target:air`; missing, forged, stale, player-drifted, or non-matching evidence rejects instead of accepting `target:not_observed_occupied`. If no valid candidate remains, `_think` returns a zero-action bounded block before learned-skill, BM-012, LLM/rule, or visual-grounding fallback. Probe 51's exact state selects the observed crafting table at `(118,122,-49)` whose top `(118,123,-49)` is machine-proven air. BM-012 generic placement and the Node mutation-time occupancy/player-collision gates remain unchanged. The 30-blob Probe 51 evidence/repair commit is pushed at `0d97e431`, tree `cae2c41d`, with exact remote readback. Probe 52's separate authorization is pushed and read back at `8e2c04a2`, tree `63a6d0fb`; it was consumed exactly once at JSONL line 2.
- Probe 52 episode `m4_episode_20260727_153112_f28d04d6` completed all 11 goals in 67 cycles and 286.188 seconds. It machine-smelted three ingots, crafted the required sticks and `iron_pickaxe:1`, preserved health/hunger at 20, had zero deaths or respawns, used no skills, and passed 74/74 eligibility checks with 13.812 seconds of runtime margin. The repaired furnace path first encountered one backend block-update timeout, then selected another exact same-snapshot solid-reference/air-target pair and successfully placed the furnace. This is BM-014 success 1/3. Its 22-blob evidence transaction is committed, pushed, and independently read back at `0563d3f6`, tree `e95669ee`.
- Probe 53 episode `m4_episode_20260727_165036_f689d87b` consumed its separately pushed/read-back authorization exactly once, completed eight goals in 70 cycles, and failed at the fixed deadline in 300.015 seconds after 24 crafting-table placements targeted backend-observed stone. It passes 64/74 checks and is retained as attempt 3 / failure 2 with no success or capability credit. The failure evidence plus bounded owned-table snapshot repair are pushed/read back at `8e0c90e2`, tree `8377e7fb`, with 32/32 exact blobs. Portability commit `cf888555`, tree `a711e996`, has 19/19 exact remote readback and passes 279/279 clean-checkout M4 tests plus 38/38 capability/action tests.
- Probe 54 authorization commit `4aaf0011`, tree `20e44f90`, was pushed and independently read back with 6/6 exact blobs before episode `m4_episode_20260727_200833_e85a7757` consumed it exactly once at `autonomous_start` line 2. The zero-retry, skills-off run completed seven goals in 40 cycles and succeeded on all 38 attempted actions, but line 536 navigation depleted its portable-station materials and it reached the furnace frontier without a nearby or inventoried crafting table or a recoverable log. It ended `episode_deadline` at 300.031 seconds, passed 66/74 checks, and counts as attempt 4 / failure 3 with no success or capability credit. The retained classification is `capability_navigation_inventory_preservation_failure`; later Planner timeouts amplified rather than caused the recovery gap. Policy `m4-bm013-bm014-portable-crafting-table-recovery-v1` now reclaims a currently observed episode-owned table before remote raw-iron/coal travel, allows historical ownership only as a return target requiring current re-observation, permits only current machine-observed oak as material recovery, and otherwise fails closed. The repair leaves BM-012 and all fixed runtime controls unchanged. Probe 54 evidence and repair are committed, pushed, and independently read back at `19c495d5` / tree `2c2cf002` with 24/24 exact blobs; clean detached validation passes 298/298 M4 tests and 43/43 capability/action tests. Probe 55's one-use authorization is staged but unusable pending its own commit/push/readback; BM-014 remains globally unauthorized and locked, and Probe 56 is unauthorized.
- The current `m4-fixed-v1` ledger contains 30 live episodes, Probes 25–54. Probe 51's earlier Python 3.13.5 startup correction had no `autonomous_start` or session JSONL and counts as neither an episode, attempt, nor retry.
...
| M4 | Autonomous Survival | **Live observed (`live_observed`)** | BM-011, BM-012, and BM-013 repeat_verified 3/3; BM-014 live_observed 1/3 after 4 attempts / 3 failures; Probe 54 evidence/repair published at `19c495d5` with 24/24 exact readback; Probe 55 authorization pending its own push/readback and unusable; Probe 56 unauthorized |
