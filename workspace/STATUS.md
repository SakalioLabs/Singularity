...
- Paired evaluation: supplemental policy `stone-pickaxe-sp001-paired-evaluation-v1` binds the exact advisory/candidate/promotion artifacts, fixture tree, and three retained baseline bundles without changing the base protocol. The baseline index is 3/3 with equal fixed-control and initial-state fingerprints. Shadow and advisory each completed four verified actions with no direct skill selection/execution; advisory emitted three hints. The report remains `retain_advisory` at 0/3 candidate pairs.
- Runtime boundary: one clean synchronized `main` invocation may consume one exact arm/replicate. Evaluation keeps the skill artifact read-only, recomputes run metrics from bound source evidence, rejects duplicate replicates, and grants neither normal runtime nor capability/M4 credit. After the advisory evidence commit is pushed, only `fallback/fallback-1` is next; SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
