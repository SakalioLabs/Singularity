...
- Phases 0-92 retain the verified SP-001/SP-002 history, SP-003 definition, offline surface-clearance repair, consumed authorizations, and immutable failures. Phase 93 retains the second pre-Bridge infrastructure failure. Earlier skill records and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: nine baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. Seven produced gameplay failures; two created no Agent episode before Bridge readiness.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Phase 92 evolves only the supplemental policy to `stone-pickaxe-sp003-empty-hand-runtime-v2`: interactive table crafts receive a process-local 1000ms settlement delay without retry, while a stable complete radius-one scan may authorize at most three top-down `grass_block`/`dirt` removals above one exact stone support. The Phase 91 replay selects `grass -> dirt -> dirt`, then proves the full entry shaft and stand `(124,140,-38)`; incomplete, forged, wrong-order, wrong-family, player-column, and fourth-clearance cases fail closed. Phase 93 exercised none of that gameplay: Paper became ready, but the preload required `bot.craft` synchronously even though Mineflayer injects it after `createBot` returns. The full chain is still not live-proven.
