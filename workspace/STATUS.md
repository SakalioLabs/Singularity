...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. The retained gameplay failures exposed reset-audit interpretation, a Runner API mismatch, craft-progress reconciliation, Pathfinder hidden mutation, and now leaf-canopy pickup reachability. The latest run removed ten logs but acquired none; its world and authorization are closed. A bounded offline canopy-egress repair is required before any new authorization.
...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with retained advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support/candidate pairs both 3/3, and executable `1.0.1` under its approved runtime gate. SP-003 Phase 85 is baseline 0/1 and candidates 0/3; six baseline authorizations are consumed and no SP-003 live authorization exists.
...
- Candidate lifecycle: the acquire 1.1.0 and craft 1.0.1 executable transitions are complete and rollback-safe. Phase 85 preserves both gates, the inventory-preserving preload, and the frozen shared bridge. The next gate is to push the retained failure and implement a bounded offline canopy-egress repair; candidate execution and automatic retry remain forbidden.
