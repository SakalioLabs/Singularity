...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, v2 support 3/3, and candidate pairs 0/3; SP-003 locked.
...
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Phase 63 repairs first-cycle routing in isolated policy v2. Phases 64-66 prove `shadow-2`, `advisory-2`, and `fallback-2` live before the root Planner; all three directly execute zero skill actions and machine-verify the exact ordinary craft. Support is 3/3 and candidates are 0/3; after this evidence commit is pushed, the next gate is one separate `candidate/r4` authorization. Executable review and SP-003 remain disabled.
