...
- Phases 0-34 are retained under `stone-pickaxe-skill-fixed-v1`; Phase 35 adds only the SP-002 offline harness. The repository, existing skill history, authorization state, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
...
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history. Fresh v5 is 3/3 after `candidate/r13`, `candidate/r14`, and `candidate/r15`; the separate promotion appended executable `1.1.0` with parent and rollback target 1.0.0 plus an approved fingerprint-bound runtime gate. `learned:craft_stone_pickaxe` does not exist. The SP-002 supplemental policy binds those exact promotion artifacts, enforces one exact craft action and one backend attempt, and requires a delayed stable observation. Its 13/13 dedicated offline cases pass; no SP-002 fixture or live authorization exists yet.
...
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 32/32 runtime cases; the supplemental SP-002 harness passes 13/13. Four failed preparations, one successful preparation, five failed SP-001 episodes, and three eligible SP-001 successes each ran once and stopped. Eighty SP-001 artifacts are retained and hash-bound. No Minecraft process ran for Phase 35.
...
- Runtime boundary: SP-002 fixture preparation and SP-002 live evidence use separate parent-bound one-time authorization scopes. Each authorization must be committed and pushed from clean synchronized `main`; output reuse and automatic retry are forbidden. The next gate is one fixture-preparation authorization only. SP-002 evidence, SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
