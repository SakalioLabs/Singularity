...
- Phases 0-109 retain the verified SP-001/SP-002 history, SP-003 definition, consumed authorizations, offline repairs, and immutable failures. Phase 108 adds process-local post-resolve grounding for a false-resolved `GoalBlock`; Phase 109 exposes the corresponding exact-unit `GoalNear` completion disconnect before the first stone dig. Earlier skill records and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: seventeen baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. Fifteen produced Agent episodes; two created no Agent episode before Bridge readiness.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains bounded surface clearance, interactive-craft settlement, exact delayed-log reconciliation, bounded planner state, `sp003-horizontal-move-envelope-v1`, and `sp003-pre-dig-pickup-access-v1`. Phase 108 adds `sp003-goalblock-completion-grounding-v1` and is pushed at `81dbf714`. Phase 109 ran once from pushed authorization `4b5bfc25`, completed the wood/table/wooden-pickaxe chain plus four clearances, then repeated twelve exact-GoalNear moves that returned success without leaving `{124,141,-37}` for target cell `{124,140,-38}`. No stone was dug, so GoalBlock recovery was not exercised. SP-003 passes 80/80, all stone-pickaxe tests pass 261/261, and full Python passes 1016/1016. The full chain is still not live-proven.
