...
- Phases 0-105 retain the verified SP-001/SP-002 history, SP-003 definition, consumed authorizations, offline repairs, and immutable failures. Phase 104 repairs the horizontal-navigation schema contradiction; Phase 105 live-exercises that repair and exposes the next pickup-access boundary. Earlier skill records and Probe 21/22/23 hashes remain protected.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains bounded surface clearance, interactive-craft settlement, exact delayed-log reconciliation, bounded planner state, and `sp003-horizontal-move-envelope-v1`. Phase 105 made all 27 Planner calls schema-valid and completed through wooden-pickaxe equip. Its first two stone actions removed covered stone while dirt still occupied each target's head cell; direct pickup navigation resolved outside range, no standable fallback existed, and no cobblestone entered inventory. The run stopped at the absolute deadline with no post-deadline action and no retry. SP-003 passes 73/73 and all stone-pickaxe tests pass 254/254. The full chain is still not live-proven.
...
- Phase 105 retains the exact 13-file failed bundle and a machine replay of both covered-stone pickup failures. Dedicated SP-003 is 73/73, all stone-pickaxe tests are 254/254, and the repository Python gate is 1009/1009.
- The next transaction is an offline pre-dig pickup-access repair; any later live run requires a fresh parent-bound authorization after that fix is committed and pushed. Candidate execution remains locked until a baseline completes the full empty-hand-to-stone-pickaxe chain.
