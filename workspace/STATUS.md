...
- BM-012 is the next target at 0/3 after eight failed live attempts. Probe 8 live-validated `m4-place-target-occupancy-v1`: the occupied `dark_oak_log` target rejected at zero execution duration, a target-aware replan selected air, and the crafting table was placed successfully. The earliest failure is now `m4_task_world_state_reconciliation_grounding`: strict-M4 reselected a duplicate placement task after its machine-world criterion was already satisfied, and stale descendants consumed 80 cycles in two dirt roots. No further live episode is authorized until the bounded offline reconciliation gate passes; BM-013 and BM-014 remain locked.
