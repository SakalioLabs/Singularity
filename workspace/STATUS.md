...
- `learned:acquire_cobblestone@1.0.0` exists only as `advisory`; its queue record is `advisory/approved`, it has no executable promotion gate, all three support arms ran without executing it, candidates `r1`, `r4`, and `r7` are retained failed, and v4 candidate `r10` passed. `learned:craft_stone_pickaxe` does not exist. Five controlled SP-001 failures, three paired candidate failures, and one paired candidate pass remain retained; the advisory still grants no capability or M4 credit.
...
- Paired evaluation: v1 `r1`, v2 `r4`, and v3 `r7` are consumed failures. V4 inherits all three support arms, binds the exact immutable failure chain and route-scope repair commit, and is `retain_advisory` at 1/3 after candidate `r10` passed every evaluation check.
- Runtime boundary: r10 machine-verified in 13.718 seconds with one Planner call, four successful actions, three completed attributed skill digs, zero failures/rejects/fallbacks, and confidence 1.0. Evidence commit `90e4e230` freezes its eleven artifacts and report hash; Paper/Bridge are stopped and ports are clear. A separate offline preflight fix guarantees remedies for every nonpassing local command check and passes 93/93 focused cases plus the full 43-script and six-suite regression. `r11/r12`, superseded IDs, support reruns, retries, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain unauthorized until that fix is pushed and a separate next-arm decision is recorded.
