...
- Phases 0-94 retain the verified SP-001/SP-002 history, SP-003 definition, offline surface-clearance and deferred-injection repairs, consumed authorizations, and immutable failures. Phase 95 retains the first post-repair Agent episode. Earlier skill records and Probe 21/22/23 hashes remain protected.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains the bounded Phase 92 surface-clearance and 1000ms interactive-craft settlement contracts. Phase 95 live-proves deferred wrapper injection and reaches three oak logs in inventory, but a delayed first pickup leaves exact source progress at `2/3`; eight premature plank crafts are rejected and the run expires. The next gate is offline-only bounded source reconciliation. The full chain is still not live-proven.
