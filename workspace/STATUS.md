...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, and v2 support/candidate pairs both 3/3. Executable 1.0.1 review is approved but not yet applied; SP-003 remains locked.
...
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Phase 63 repairs first-cycle routing in isolated policy v2, Phases 64-66 prove all three support arms, and Phases 67-69 prove `candidate/r4/r5/r6` against distinct paired baselines with matching fixed controls and initial state, one verified and reobserved skill action each, and attribution confidence 1.0. Support and candidates are both 3/3; after this evidence commit is pushed, the next gate is a separate offline append-only promotion to `learned:craft_stone_pickaxe@1.0.1`. Normal runtime and SP-003 remain disabled until that promotion is pushed.
