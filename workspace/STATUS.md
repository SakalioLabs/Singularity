...
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Phase 63 repairs first-cycle routing in isolated policy v2. Phase 64 proves `shadow-2` live with one shadow plan before the root Planner; Phase 65 proves `advisory-2` live with one bounded hint before the root Planner. Both directly execute zero skill actions and machine-verify the exact ordinary craft. Support is 2/3 and candidates are 0/3; after this evidence commit is pushed, the next gate is one separate `fallback/fallback-2` authorization. Executable review and SP-003 remain disabled.
