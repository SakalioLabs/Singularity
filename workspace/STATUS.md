...
- Phases 0-76 retain the verified SP-001/SP-002 history, SP-003 definition, two baseline failures, and their bounded repairs. Phase 77 retains the third SP-003 baseline failure; Phase 78 repairs only successful-craft progress reconciliation. Earlier skill records, consumed authorizations, and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: three baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. The third run made 15 real Planner calls and four successful actions before the stale progress loop exhausted its deadline.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. The first retained baseline exposed reset-audit interpretation, the second exposed one nonexistent Runner method call, and the third proved empty-hand log acquisition plus exact plank crafting before exposing a craft-progress reconciliation disconnect. All three fixes are bounded and tested. No SP-003 live authorization is currently present.
