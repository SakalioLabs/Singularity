...
- Phases 0-93 retain the verified SP-001/SP-002 history, SP-003 definition, offline surface-clearance repair, consumed authorizations, and immutable failures. Phase 94 offline-verifies deferred craft-wrapper installation after Mineflayer plugin injection. Earlier skill records and Probe 21/22/23 hashes remain protected.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Runtime-v2 retains the bounded Phase 92 surface-clearance and 1000ms interactive-craft settlement contracts. Phase 94 now returns the original bot immediately and installs that process-local wrapper once on `inject_allowed`, after Mineflayer's internal craft plugin; a real loader smoke proves `craft` is absent at return and wrapped after injection. Missing craft still fails closed, and no backend retry is added. The full chain is still not live-proven.
