...
- Phases 0-116 retain the verified SP-001/SP-002 history, SP-003 definition, consumed authorizations, offline repairs, and immutable failures. Phase 114 repairs down-step transition headroom without changing the shared Bridge, vendored Pathfinder, or base protocol. Phase 115 live-confirms that repair and exposes destruction of the only climbable egress anchor after the first cobblestone pickup. Phase 116 offline-stages the single crafting table beside machine-proven stone access. Earlier skill records and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3 and SP-002 Craft Stone Pickaxe is 3/3. SP-003 is not live-verified: twenty baseline authorizations are consumed, baseline remains 0/1, and candidates remain 0/3. Eighteen produced Agent episodes; two created no Agent episode before Bridge readiness.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Phase 115 evidence is pushed at `14ce593e` and retains the 13-file egress failure. Phase 116 now passes offline with a 21-action local-table path: carry the sole table through the existing probes and five bounded clearances, place it once at `(123,142,-37)` after direct stone access exists, then craft both tools at that same table. The retained terminal pit is 4.297 blocks from the table. The full chain is still not live-proven.
...
- Current gate: SP-001 and SP-002 are independently 3/3 with executable `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains baseline 0/1 and candidates 0/3 after twenty consumed baseline authorizations; no live authorization exists.
- Phase 115's exact 13-file failure reaches three cobblestone but destroys its only egress anchor before the terminal craft. Its replay, geometry, eleven failed return moves, and hashes are frozen at evidence commit `14ce593e`.
- Phase 116's SP-003-only local-table staging repair passes offline and preserves every Phase 114 protected identity. The next transaction is its commit and push. Only afterward may a fresh parent-bound one-use baseline authorization be created; candidate execution remains locked until a baseline completes the full empty-hand-to-stone-pickaxe chain.
