...
- Phases 0-51 retain the path through the verified SP-002 fixture, two failed live attempts, and the bounded authoritative-inventory repair. Phases 52/54/56 record separately pushed live authorizations, Phases 53/55/57 retain three eligible SP-002 live successes, Phase 58 repairs the source gate, Phase 59 extracts candidate `a488cd61`, Phase 60 promotes it to advisory review status, Phase 61 defines its fail-closed paired evaluation, Phase 62 retains the consumed v1 shadow failure, Phase 63 defines an isolated first-cycle-routing recovery window, Phases 64-66 retain passing support evidence, and Phases 67-69 retain all three eligible candidate pairs `r4/r5/r6`. The repository, earlier skill records, consumed authorizations, and Probe 21/22/23 hashes remain protected.
- SP-001 Acquire Cobblestone is 3/3, SP-002 Craft Stone Pickaxe is 3/3, and SP-003 Composite Stone Pickaxe Chain is 0/3 and locked.
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history and executable `1.1.0` remains approved. `learned:craft_stone_pickaxe@1.0.0` is now an append-only advisory record sourced from candidate `a488cd61`; it is not executable. Four fixture authorizations and five live authorizations are consumed and nonreusable.
...
- Current gate: SP-001 3/3 executable; SP-002 3/3 with advisory `1.0.0`, fixed paired baselines 3/3, retained v1 shadow failure, and v2 support/candidate pairs both 3/3. Executable 1.0.1 review is approved and its offline tooling passes 43/43, but the promotion is not yet applied; SP-003 remains locked.
...
- Learned skills: `learned:acquire_cobblestone@1.0.0` is retained advisory history and `1.1.0` is executable under its approved runtime-default gate. `learned:craft_stone_pickaxe@1.0.0` is advisory only and has no normal-runtime permission. Existing gather-wood, table, and wooden-pickaxe histories are unchanged record-for-record.
...
- Runtime boundary: SP-002 fixture preparation, source evidence, and paired-evaluation arms use separate parent-bound one-time authorization scopes; output reuse and automatic retry are forbidden. All nine source-phase SP-002 authorizations, v1 `shadow/shadow-1`, and v2 `shadow-2/advisory-2` are consumed. Recovery v2 currently authorizes no further arm; SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Phase 63 repairs first-cycle routing in isolated policy v2, Phases 64-66 prove all three support arms, and Phases 67-69 prove `candidate/r4/r5/r6` against distinct paired baselines with matching fixed controls and initial state, one verified and reobserved skill action each, and attribution confidence 1.0. Support and candidates are both 3/3; Phase 70 supplies the tested task-specific append-only promotion path, while the actual `learned:craft_stone_pickaxe@1.0.1` append remains a separate post-push operation. Normal runtime and SP-003 remain disabled until that promotion is pushed.
