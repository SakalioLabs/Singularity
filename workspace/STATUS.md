# STATUS.md | Last updated: 2026-07-18
...
- Phases 0-34 are retained under `stone-pickaxe-skill-fixed-v1`; Phase 35 added the SP-002 offline harness, Phases 36-37 retain the first one-time fixture transport failure, Phases 38-39 retain the second run's progress and terminal-action contract failure, Phase 40 fixes that contract offline, Phases 41-42 retain the third authorization plus its root-plan contract failure, Phase 43 fixes the root graph offline, Phase 44 records the fourth authorization, and Phase 45 seals the verified SP-002 fixture. Phase 46 is the separate live-episode authorization gate. The repository, existing skill history, authorization state, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
- SP-001 Acquire Cobblestone is 3/3, SP-002 Craft Stone Pickaxe is 0/3, and SP-003 Composite Stone Pickaxe Chain is 0/3 and locked.
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history. Fresh v5 is 3/3 after `candidate/r13`, `candidate/r14`, and `candidate/r15`; the separate promotion appended executable `1.1.0` with parent and rollback target 1.0.0 plus an approved fingerprint-bound runtime gate. `learned:craft_stone_pickaxe` does not exist. The SP-002 supplemental policy binds those exact promotion artifacts, enforces one exact craft action and one backend attempt, and requires a delayed stable observation. Its 15/15 dedicated offline cases pass. Four fixture authorizations are consumed: the first failed before action on transport, the second reached exact three cobblestone before a terminal action-bearing plan was rejected, the third was rejected at its zero-subtask root plan, and the fourth completed six verified actions and sealed the SP-002 fixture. The consumed authorization is archived and cannot be reused; no SP-002 live authorization exists.
- Offline verifier fixtures do not count toward a skill gate, capability evidence, or M4. The canonical capability report is unchanged. Advisory 1.0.0 and executable 1.1.0 are append-only records; all older skill records remain protected by individual canonical hashes.
...
- Current gate: SP-001 3/3; SP-002 0/3; SP-003 locked.
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 32/32 runtime cases; the supplemental SP-002 harness passes 15/15. Four earlier failed preparations, one successful preparation, three SP-002 fixture-preparation failures, one SP-002 fixture-preparation success, five failed SP-001 episodes, and three eligible SP-001 successes each ran once and stopped. The fourth SP-002 preparation's fourteen evidence artifacts are hash-bound alongside retained prior evidence. Paper/Bridge are stopped and controlled ports are clear.
...
- Learned skills: `learned:acquire_cobblestone@1.0.0` is retained advisory history and `1.1.0` is executable under its approved runtime-default gate; `learned:craft_stone_pickaxe` is not created. Existing wooden-pickaxe history is unchanged record-for-record.
...
- Runtime boundary: SP-002 fixture preparation and SP-002 live evidence use separate parent-bound one-time authorization scopes. Each authorization must be committed and pushed from clean synchronized `main`; output reuse and automatic retry are forbidden. All four fixture-preparation authorizations are permanently consumed, and the fourth produced the sealed fixture. The next gate is a separately committed and pushed one-time `live_episode` authorization bound to that fixture. SP-002 live evidence, SP-003, Probe 24, full BM-012, and iron mining remain unauthorized until their respective gates open.
- Candidate lifecycle: the acquire 1.1.0 executable transition is complete and rollback-safe. Craft candidate extraction remains locked until three distinct eligible SP-002 live successes; no current skill record may be rewritten in place.
