...
- Phases 0-110 retain the verified SP-001/SP-002 history, SP-003 definition, consumed authorizations, offline repairs, and immutable failures. Phase 109 exposes an exact-unit `GoalNear` completion disconnect before the first stone dig; Phase 110 adds bounded process-local grounding for only that marked goal type. Earlier skill records and Probe 21/22/23 hashes remain protected.
...
- SP-003 starts in a fresh seed-12345 survival world with exact empty inventory and requires the full `logs -> table -> wooden pickaxe -> three stone -> stone pickaxe` machine-verified chain. Phase 109 evidence is pushed at `bc20b5a0` after twelve unchanged-position exact-GoalNear successes blocked the first stone dig. Phase 110 adds `sp003-exact-goalnear-completion-grounding-v1`: only marked transformed range-1 goals can use a machine-proven adjacent clear down-step, bounded to four 125ms pulses with no world mutation. Retained geometry, Bridge `move_to`, and direct pickup replays pass; unsupported goals and errors remain unchanged. SP-003 passes 81/81, all stone-pickaxe tests pass 262/262, full Python passes 1017/1017, and seven Node suites pass 75/75. The full chain is still not live-proven.
...
- Current gate: SP-001 and SP-002 are independently 3/3 with executable `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains baseline 0/1 and candidates 0/3 after seventeen consumed baseline authorizations; no live authorization exists.
- Phase 109 retains the exact 13-file failed bundle at pushed commit `bc20b5a0`. Phase 110 audit `db1372ee...` binds the SP-003-only exact-GoalNear completion repair. Dedicated SP-003 is 81/81, all stone-pickaxe tests are 262/262, the repository Python gate is 1017/1017, and Node is 75/75.
- The next transaction is the Phase 110 offline-fix commit and push; only afterward may a separate fresh parent-bound baseline authorization be committed and pushed. Candidate execution remains locked until a baseline completes the full empty-hand-to-stone-pickaxe chain.
