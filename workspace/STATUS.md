...
- Phases 0-34 are retained under `stone-pickaxe-skill-fixed-v1`; Phase 35 added the SP-002 offline harness, Phases 36-37 retain the first one-time fixture transport failure, Phases 38-39 retain the second run's progress and terminal-action contract failure, Phase 40 fixes that contract offline, and Phases 41-42 retain the third authorization plus its root-plan contract failure. Phase 43 is an offline-only repair gate. The repository, existing skill history, authorization state, and Probe 21/22/23 evidence hashes remain protected; the fixed protocol and its 34-case harness are unchanged.
...
- `learned:acquire_cobblestone@1.0.0` remains immutable advisory history. Fresh v5 is 3/3 after `candidate/r13`, `candidate/r14`, and `candidate/r15`; the separate promotion appended executable `1.1.0` with parent and rollback target 1.0.0 plus an approved fingerprint-bound runtime gate. `learned:craft_stone_pickaxe` does not exist. The SP-002 supplemental policy binds those exact promotion artifacts, enforces one exact craft action and one backend attempt, and requires a delayed stable observation. Its 14/14 dedicated offline cases pass. Three fixture authorizations are consumed: the first failed before action on transport, the second reached exact three cobblestone in four successful actions before a terminal action-bearing plan was rejected, and the third was rejected at its zero-subtask root plan before any action. The mode-specific completion gate remains fixed; the next offline fix must repeat the root graph contract without relaxing validation. No SP-002 fixture or reusable authorization exists.
...
- Runtime state: the controlled fixture/snapshot/SP-001 harness passes 34/34 protocol and 32/32 runtime cases; the supplemental SP-002 harness passes 14/14. Four earlier failed preparations, one successful preparation, three SP-002 fixture-preparation failures, five failed SP-001 episodes, and three eligible SP-001 successes each ran once and stopped. Eighty SP-001 artifacts and thirty-three SP-002 failure-evidence files are hash-bound. Paper/Bridge are stopped and controlled ports are clear.
...
- Runtime boundary: SP-002 fixture preparation and SP-002 live evidence use separate parent-bound one-time authorization scopes. Each authorization must be committed and pushed from clean synchronized `main`; output reuse and automatic retry are forbidden. All three fixture-preparation authorizations are permanently consumed. The terminal-action prompt repair passed offline and was exercised once; the retained third failure now requires an offline root-plan graph-contract repair before any fresh authorization can exist. No new authorization exists. SP-002 live evidence, SP-003, Probe 24, full BM-012, and iron mining remain unauthorized.
