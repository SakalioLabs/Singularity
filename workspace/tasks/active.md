...
- [ ] Run schema-v2 `plan-cache-report` hybrid guidance plus `plan-cache-runtime-report` on fresh M1/M2/M7 logs, then compare planner calls, token cost, verifier rejects, completion, and entry-scoped stage changes against baseline.
...
- [x] Implement fixed-ledger uniform versus information-weighted frontier allocation with typed prerequisite/verifier/novelty/risk signals, deterministic soft rounding, and explicit remaining-round intervals.
- [x] Add paired raw-session frontier-budget evidence gating with exact allocation replay, BAGEN-style interval coverage certificates, optimistic-miss checks, and hard prohibitions on automatic retry, branch execution, and budget extension.
- [ ] Collect at least 12 distinct matched uniform/information M1/M2 session pairs and link any claimed recovered rounds to an approved episode-abort gate plus the originating runtime credit event before advisory planner context.
