...
- Added an AgenticCache-style plan-transition cache path. `plan-cache-report` mines successful session-log plan transitions into approved cache artifacts with goal signatures, previous-plan signatures, compact state features, support, success rate, and promptware flags; runtime `--enable-plan-cache --plan-cache ...` can reuse accepted entries before an LLM planner call while preserving action verification, goal verification, visual grounding, and session hit/miss metrics. Runtime profiles now carry `enable_plan_cache` plus scanned `plan_cache_paths`.
