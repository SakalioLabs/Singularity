...
- Added MemTier-style `memory-attribution-report`, which groups generic `memory_read` events before each `plan`, hashes queries, and labels each retrieval as `supported`, `conflicting`, `review`, or `no_result` using downstream plan/action/goal outcomes before any retrieval-weight update is allowed.
- Added `memory-attribution-gate` as the retrieval-weight boundary: saved attribution reports must show enough supported outcome-attributed reads while keeping conflicting and no-result rates within configurable thresholds before a weighted memory retrieval profile can be enabled.
- Wired `memory-attribution-gate` into runtime configs and reusable runtime profiles. `--enable-weighted-memory-retrieval` now evaluates saved gate reports at Agent startup and remains ineffective unless the gate is approved.
- Closed the first generic retrieval-weight loop. Approved attribution gates now expose memory-id `retrieval_weight_hints`; `MemorySystem` applies those bounded deltas during durable memory and transfer-experience ranking, while reports continue to omit raw query text and memory content.
...
- Added `bounded-context-gate` so typed retrieval contracts can approve, review, or reject planner-context evidence before runtime profiles or live benchmark variants rely on long-horizon memory context.
...
- Wired approved memory promptware gates into runtime strict-memory enforcement. `--enforce-memory-write-gate` remains advisory-off unless an approved `--memory-promptware-gate` is supplied directly or through a runtime profile, so strict write suppression cannot be enabled from stale or missing memory-security evidence.
...
- Added an AgenticCache-style plan-transition cache path. `plan-cache-report` mines successful session-log plan transitions with goal signatures, previous-plan signatures, compact state features, support, success rate, and promptware flags; runtime profiles carry scanned `plan_cache_paths` plus approved `plan_cache_gate_paths`.
- Upgraded plan reuse to `progressive_workflow_crystallization_v1`, informed by Progressive Crystallization and EvoSOP. Offline schema-v2 entries can only become bounded hybrid planner guidance. `plan-cache-runtime-report` attributes complete executable-workflow signatures, distinct sessions, goal outcomes, action failures, and verifier rejects per entry; only complete `live_trace` reports participate in stage changes, and `plan-cache-gate` requires three matched successes across three sessions by default before that specific entry may bypass the LLM planner.
- Agent runtime loads only entry IDs named by an approved schema-v2 gate. Hybrid entries are injected as complete-line context under a 600-character cap, deterministic entries may be returned before the LLM call, and demoted entries are not loaded. Visual grounding plus action and goal verification remain authoritative after either path.
- Entry-scoped regression handling replaces whole-cache promotion. Excess action failures, verifier rejects, or failed goals demote only the affected workflow to agentic execution, retain its audit evidence, and leave unrelated hybrid or deterministic entries intact. Legacy schema-v1 gates are rejected rather than silently granting direct execution.
- Added `capability-evidence-report`, combining MineExplorer/MineNPC machine-checkable milestones, AgenticSTS evidence contracts, and systems-style memory evaluation into a project-level claim ledger. Source files and offline tests are reported separately from live observations and three-run verification, so stale M0-M7 status text cannot promote an unverified capability.
- Added WorldLines as the reference for visibility-aware, action-native state memory. Future M3/M5 acceptance should test state disappearance, revision, revisit, and grounded recovery rather than count generic memory reads.
- Added Agent Memory System Characterization as the reference for splitting memory construction, retrieval, maintenance, and generation costs before enabling a backend or profile in urgent Minecraft loops.
- Added SelfMem as the reference for feedback-driven memory-strategy optimization. Singularity keeps optimization offline and review-gated: runtime traces expose typed-layer usage and exact character cost, while held-out Minecraft transfer and downstream verifier outcomes must approve any budget or retrieval-policy change.
- Added MAGE execution-state memory as the reference for long-horizon branch isolation and checkpoint revision. The first runtime step is explicit, non-nested autonomous subgoal evidence; parent/revision provenance and automatic branch restoration remain future work.
- Added a schema-v2 bounded planning-memory contract to rule and LLM cycles. It enforces per-read and total packet limits, counts separators in the actual packet size, logs typed segment metrics without raw memory contents, and lets replay reject explicitly disabled or violated contracts. Advisory prompt sections outside typed memory are not yet covered by this budget, and deterministic rule selection explicitly reports that the attached memory has not changed its action choice.
- Repaired autonomous evidence boundaries. Autonomous sessions now log observations, plans, selected goals, and terminal outcomes as sequential subgoal events rather than nesting them inside an `AUTONOMOUS_MODE` goal; newly queued tasks remain scheduler candidates instead of silently changing the goal attached to an already generated plan.
- Added schema-v2 task execution-state lineage inspired by MAGE. Checkpoints carry execution/branch identity, parent/root IDs, depth, operation, validation evidence, terminal branch state, and revision provenance while schema-v1 JSONL records still load. Planner continuity context reconstructs a single root-to-current path; failed and proposed branches are clearly labeled as hints.
- Wired runtime operations to execution-state semantics: planning checkpoints use `grow`, post-action task updates use `maintain`, verifier-backed goal completion uses `compress`, and failed goal terminals close a failed branch. A later same-name attempt starts a new root instead of attaching beneath a closed ancestor.
- Added review-only revision proposals and CLI support. A proposal must identify a failed checkpoint and its nearest verified ancestor, creates a separate proposed branch, and always records `restoration_applied=false`; no Minecraft state, inventory, task status, or control flow is rolled back.
- Added MemGym as the evaluation reference for isolating memory value from planner, reasoning, and tool-use value. The next lineage experiment must hold planner/action backends fixed and compare completion, context cost, failed-branch contamination, verifier outcomes, and recovery efficiency before any restoration gate can approve runtime use.
- Implemented `task-continuity-lineage-ablation` with fixed planner, action backend, verifier, task stream, and seed identifiers. It compares the existing flat checkpoint ranking against one reconstructed execution path, reports active-leaf precision, failed-branch contamination, context characters, ambiguity, and lineage integrity, and marks synthetic built-ins as gate-ineligible.
- Implemented `task-continuity-restoration-report` as a shadow-only validator. It requires a genuine failed checkpoint, a verified ancestor whose proposal evidence matches, an isolated schema-v2 revision branch, bounded route evidence, distinct baseline/candidate sessions, non-regressing completion, and exact preservation of critical pre-action player state including inventory, position, health, hunger, XP, equipment, dimension, game mode, selected slot, oxygen, and effects when present.
- Implemented `task-continuity-restoration-gate`. The default requires live traces, at least three eligible lineage cases, three eligible restoration cases, and three distinct candidate sessions. It rechecks report schema, provenance, fixed controls, and case-level integrity; success can set only `shadow_revision_selection_allowed=true`, while `automatic_restore_allowed` is hard-coded false.
- The initial rich-lineage fixtures produced 2/3 review-ready cases, reduced failed-branch contamination from 2 to 0, and improved mean path precision by 0.5, while also exposing that rich candidate formatting was longer than the flat formatter. That measured regression motivated the budgeted capsule work below.
- Reviewed OpenClaw's memory/compaction contract: durable workspace files and full transcripts remain intact, while only the model-visible copy is budgeted or compacted. Reviewed Hermes Agent's official compression evaluator: compressed state is probed for recall, artifact trail, continuation, decision, completeness, and instruction following instead of being accepted from size alone.
- Added Minecraft Goal-Oriented Graphs as the procedural-retention reference. A compressed execution memory must preserve the requested goal and its unresolved prerequisite frontier as one chain, not a set of independently relevant entities or checkpoints.
- Implemented `MemorySystem.task_continuity_capsule()` and routed planner task-continuity reads through profile `goal_frontier_capsule_v1`. The capsule keeps leaf/path IDs, branch mode, active goal, ready/active/blocked tasks, compact missing preconditions, and next actions; alternate-branch details stay only in the durable ledger. It packs complete fields by priority under the planner's exact read allowance rather than blindly truncating a rich string. Sanitized `memory_read.context_trace` fields expose full/projected characters, truncation, required-line completeness, frontier/next inclusion, branch count, and path length without logging memory text.
- Extended lineage ablation to schema v3 with fair rich flat-baseline formatting, rich-lineage and capsule character counts, required/forbidden probe banks, budget checks, and per-case reductions. Built-ins retain every probe with zero failed-ID leakage and reduce context by 103, 74, and 86 characters (87.7 mean), but this is deterministic offline evidence only.
- Tightened the shadow-selection gate so eligible lineage evidence must use schema v3, pass capsule probes, stay within budget, and meet a configurable positive context-reduction threshold. The gate still cannot enable automatic restoration.
- Extended bounded-context replay to validate capsule trace schema and types, cross-check declared versus observed character counts, reject omitted available frontier or continuation fields, and treat a schema-valid empty capsule as normal before any checkpoint exists.
- Reviewed Task Decomposition-Guided Skill Reranking and the fixed-expert lightweight game-agent study. Together they suggest that skill selection should be evaluated as a state-transition matching problem against a fixed external yardstick, not as another free-form relevance prompt.
- Implemented `SkillLibrary.get_frontier_skill_route()` as `frontier_transition_skill_router_v1`. It reuses the machine-checkable task-readiness frontier, scores explicit assignment, unresolved precondition closure, target-state overlap, interval semantics, task family, contract readiness, governance, and Bayesian-smoothed use outcomes, while preserving the legacy ranker as a switchable baseline.
- Agent planning now reuses one readiness snapshot for both diagnosis and skill routing. The planner receives at most 600 complete characters of route lines, while `skill_frontier_route` logs only sanitized skill names, task IDs, scores, coverage, and reason codes. Action and goal verifiers remain authoritative.
- Added `skill-frontier-routing-ablation` with fixed planner/action/verifier/task-stream/seed identifiers. Three synthetic Minecraft fixtures route shelter logs, cobblestone acquisition, and assigned combat skills from 0/3 legacy top-1 hits to 3/3 with full frontier coverage and no regressions; built-ins are explicitly ineligible as live evidence.
- Reviewed Blind Curator, SkillCenter, and EvoSOP for the next lifecycle step. Rejected skills remain excluded by governance, while contribution-based soft retirement now has separate verifier calibration, no-skill contribution, and runtime gate artifacts.
- Implemented `skill-verifier-calibration-report`, which scores false-pass bias per judge from verifier-backed injected failures plus non-defect controls. One biased judge rejects the report instead of being hidden by aggregate accuracy; built-in and synthetic evidence remains runtime-ineligible.
- Implemented `skill-contribution-report`, which compares baseline and skill-enabled outcomes with planner, action backend, task stream, seed, and non-skill modules fixed. Candidate quarantine requires negative contribution, verifier-backed failures, complete provenance, and at least three distinct candidate sessions by default.
- Implemented `skill-retirement-gate` and Agent runtime loading. The gate revalidates live evidence and calibrated judge coverage, hard-codes `automatic_delete_allowed=false`, and can only add task-family exclusions to a `SkillLibrary` in-memory overlay. Built-in skills are immune and no skill storage file is edited or removed.
- Reviewed IGRPO, BAGEN, MineExplorer, and AgentTether as one budget/recovery design problem. Informativeness should bias a fixed rollout pool softly, remaining cost needs an explicit uncertainty interval, hidden prerequisites are first-class progress targets, and upstream dependency diagnosis must precede any retry.
- Implemented `frontier_information_budget_v1` plus `uniform_frontier_budget_v1`. Curriculum and task-readiness branches are projected into typed closure, verifier/no-progress, novelty/frontier, risk, attempts, and blocker signals. Deterministic largest-remainder rounding conserves every integer round and an exploration floor prevents all-or-nothing greedy suppression when the pool permits breadth.
- Added `frontier_budget_interval_v1` and BAGEN-style alerts. Raw intervals are always marked uncalibrated; paired successful outcomes provide actual remaining rounds, an exact one-sided Clopper-Pearson coverage lower bound, and an optimistic-miss rate before advisory planner context can be enabled.
- Added `frontier-rollout-budget-report` and runtime modes. Offline cases and three built-ins can validate shadow mechanics only. Advisory mode requires distinct paired uniform/information raw JSONL sessions, successful connection/terminal boundaries, identical controls and branch inputs, exact replay of recorded allocations, non-regressing completion/verifier/action outcomes, positive prerequisite-resolution targeting, and a whole-report integrity hash. Active episode-abort savings may become bounded credit only with a matching approved abort gate and event fingerprint; automatic retry, branch execution, and budget extension are hard-coded false.
- Reviewed AgentRx and TrajAudit alongside AgentTether. Minecraft failures now need guarded execution constraints, the first unrecovered transition rather than the last symptom, and compact dependency-linked evidence packets instead of raw trajectory injection.
- Added `minecraft_transition_unit_v1` and `minecraft_execution_dependency_graph_v1`. Actions and actionless planner responses become stable units with temporal, plan-order, shared-target, artifact, and normalized-error edges; raw planner reasoning is stored only as presence plus a fingerprint.
- Added `first_unrecovered_constraint_v1` and `critical-transition-report`. Deterministic schema, verifier, navigation, world-effect, grounding, repeated-failure, no-progress, and empty-plan constraints are marked recovered only by later matching state evidence. Synthetic controls match 5/5 critical units and categories versus 3/5 for recency, but typed Repair Memory candidates remain review-only with every behavior-changing authority disabled.
- Repaired the bridge navigation contract exposed by historical M1 traces. `move_to` now uses pathfinder `GoalNearXZ`/`GoalNear`, verifies final tolerance, and returns failure instead of false success; distant wood plans navigate in one cycle and re-observe before digging.
