...
- Added an AgenticCache-style plan-transition cache path. `plan-cache-report` mines successful session-log plan transitions into approved cache artifacts with goal signatures, previous-plan signatures, compact state features, support, success rate, and promptware flags; `plan-cache-runtime-report` audits cache-hit runs for verifier rejects and action failures; `plan-cache-gate` is now required before runtime `--enable-plan-cache --plan-cache ...` can reuse entries before an LLM planner call. Cached plans still pass through action verification, goal verification, visual grounding, and session hit/miss metrics, while runtime profiles carry scanned `plan_cache_paths` plus approved `plan_cache_gate_paths`.
