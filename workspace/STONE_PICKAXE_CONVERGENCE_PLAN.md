...
Current authorization: **exactly one `candidate/r11` live episode becomes authorized only after this ledger commit is pushed**. Candidates `r1`, `r4`, `r7`, and `r10` are consumed and cannot be retried; `r2/r3/r5/r6/r8/r9` remain excluded, while `r12` remains locked pending separate evidence review. R10 passed from authorization commit `5ad854d1`, its evidence is pushed at `90e4e230`, preflight fix `1f5a4061` is synchronized, and v4 is now 1/3. Another fixture session, support reruns, retries, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain locked.
...
- Recovery policy `stone-pickaxe-sp001-paired-evaluation-v4` is isolated under `workspace/evals/sp001_skill_evaluation_v4`. It binds route-scope fix commit `18525025`, exact `agent.py` hash `c7425d93...`, immutable v1-v3 policy/report identities, retained failed records `r1/r4/r7`, and unchanged v1 support arms. Only `r10/r11/r12` are eligible for one-arm authorizations; `r1..r9`, support reruns, retries, and normal runtime fail closed. Candidate `r10` passed exactly once, its eleven artifacts are frozen at evidence commit `90e4e230`, and the report is `retain_advisory` at 1/3 with inherited support 3/3 and 12/12 v4 cases passing. Preflight repair `1f5a4061` guarantees actionable remedies for every nonpassing local command probe. This separate commit authorizes exactly one fresh `candidate/r11` run after push; r12 remains locked.
