...
| SP-003 Composite Chain | 0 | One baseline, then 3 candidate successes | Phase 124 first table-tool-craft readiness passes offline; Phase 123 behaviorally closes the chain but remains strict-ineligible |

Current phase: **Phase 124 offline-verifies `sp003-first-table-tool-craft-readiness-v1` from pushed Phase 123 evidence commit `33441a18`. Phase 123 behaviorally completed the full empty-hand chain, but its first table-bound wooden-pickaxe action made one real craft call with no inventory delta; the next Planner cycle repeated the craft successfully, so strict baseline evidence remains false. The SP-003-only process-local preload now opens and validates the exact structured crafting table once before the first wooden- or stone-pickaxe craft, closes it, requires unchanged inventory, and then delegates to exactly one original craft call. A failed, timed-out, wrong-window, or mutating preflight is terminal and cannot reopen or craft; later tool craft reuses readiness, non-tool crafts are unchanged, and no retry or world mutation is introduced. Audit `136fbf31...`, stone-pickaxe `316/316`, full Python `1069/1069`, and seven Node suites with 85 internal cases pass. The repair awaits commit/push, and SP-003 remains 0/1 baseline and 0/3 candidate successes**.

Current authorization: **none**. All prior SP-001/SP-002 authorizations remain consumed or excluded, and all twenty-four SP-003 baseline authorizations through `sp003_baseline_20260720_040104_4676408a` cannot be reused. Phase 123 ran exactly once and cannot be retried. The next transaction is the Phase 124 offline repair commit and push. Only afterward may a fresh parent-bound one-use baseline authorization be committed and pushed. Automatic retry, authorization reuse, candidate execution before a passing baseline, full BM-012, Probe 24, and iron mining remain locked.
...
- Offline status: Phase 124 focused cases pass 3/3, all stone-pickaxe suites pass 316/316, TaskSystem/GoalVerifier/capability cases pass 140/140, selected M1/M2/M3 cases pass 72/72, the repository-wide Python gate passes 1069/1069, and seven Node suites pass 85/85 internal cases. The gate is rerun before each evidence or offline-fix commit.
...
| 8. SP-003 composite acceptance | Phase 124 first table-tool-craft readiness passes offline; twenty-four authorizations consumed; Phase 123 behavioral closure is strict-ineligible; baseline 0/1 and candidates 0/3; repair commit/push required before another authorization |
