...
| SP-003 Composite Chain | 0 | One baseline, then 3 candidate successes | Phase 127 live-exercised Phase 126 target semantics but failed on a stage-invalid craft request and a terminal reasoning envelope rejection |

Current phase: **Phase 127 consumed the one-use authorization from pushed commit `a9ead681` exactly once. Episode `sp003_baseline_20260720_064839_abb7c5cb` proves the Phase 126 literal target semantics live: the probe move and two surface-clearance digs use explicit machine targets, and no table reference is misread as clearance. The run remains strict-ineligible. Planner call 6 requested two extra planks during the exact crafting-table stage and the guard rejected it without invoking the backend; call 11 then returned a structurally bounded action with reasoning beyond the 320-character envelope, so the plan was rejected and the episode ended `empty_plan` before table placement. The immutable bundle is being retained with manifest `de2d72cd...`. SP-003 remains 0/1 baseline and 0/3 candidate successes**.

Current authorization: **none**. All prior SP-001/SP-002 authorizations remain consumed or excluded, and all twenty-six SP-003 baseline authorizations through `sp003_baseline_20260720_064839_abb7c5cb` cannot be reused. Phase 127 ran exactly once and cannot be retried. The next transactions are the Phase 127 evidence commit/push and a separate Phase 128 offline repair; only after both are pushed may a new parent-bound one-use baseline authorization be considered. Automatic retry, authorization reuse, candidate execution before a passing baseline, full BM-012, Probe 24, and iron mining remain locked.
