...
Current phase: **Phase 7 paired evaluation in progress with three immutable baseline arms, verified shadow/advisory support arms, and zero candidate arms; `learned:acquire_cobblestone@1.0.0` remains advisory and non-executable**.

Current authorization: **after the retained advisory evidence is committed and pushed, exactly one `fallback/fallback-1` SP-001 skill-evaluation arm may run**. The launcher requires clean synchronized `main`, writes a single-use pre-live authorization, and rejects a repeated arm/replicate. Candidate arms, another fixture session, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain locked until their preceding evidence is committed and reviewed.
...
- `workspace/evals/acquire_cobblestone_baseline_index.json` is 3/3. `workspace/evals/acquire_cobblestone_paired_evaluation.json` is intentionally `retain_advisory` at 0/3 candidate pairs with shadow and advisory verified while fallback remains unexercised. Both support runs completed in four verified actions with no direct skill selection or execution; shadow emitted three plans and advisory emitted three hints. It grants no normal runtime, capability, or M4 authority.
...
| 7. Paired promotion evaluations | In progress; baselines 3/3, candidates 0/3, support arms 2/3 (shadow/advisory verified) |
...
The retained fixture blockers plus all three SP-001 behavioral failures are reproduced and fixed; the two provider TLS EOF traces are retained as zero-action fail-closed failures. All source sessions remain immutable. The fixture snapshot still passes independent identity audit, and three eligible SP-001 successes establish 3/3. The acquire skill is advisory only. Shadow and advisory each ran exactly once from pushed predecessors, passed machine verification, matched baseline controls/state, and retained eleven hash-bound artifacts. Commit and push advisory evidence before running only `fallback/fallback-1`. Do not batch paired arms, retry a consumed replicate, run SP-002/SP-003 before their gates unlock, run full BM-012, run Probe 24, or begin iron mining.
