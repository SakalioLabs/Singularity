...
| SP-003 Composite Chain | 1 baseline / 0 candidate | One baseline, then 3 candidate successes | Phase 148 strict baseline passed; evidence push precedes candidate `r1` |

Current phase: **Phase 148 completed the strict empty-hand chain with one eligible baseline success. SP-003 is 1/1 baseline and 0/3 candidate successes; evidence `55c9026a...` awaits its own commit push before candidate `r1` can be authorized**.

Current authorization: **none**. All prior SP-001/SP-002 authorizations remain consumed or excluded, and all thirty-five SP-003 baseline authorizations through `sp003_baseline_20260720_211838_phase148` cannot be reused. Phases 139-143, 146, and 148 each ran exactly once and cannot be retried. Phases 145 and 147 each consumed their single permitted provider probe and cannot be rerun. Live authorization remains locked until Phase 148 evidence is pushed; then one separate parent-bound one-use candidate `r1` authorization may be created. Automatic retry, authorization reuse, candidate `r2/r3` before their sequential prerequisites, full BM-012, Probe 24, and iron mining remain locked.
...
Phase 145 binds JSONL observation line 270, source call `llm-47b868e32e5d4372`, and repaired request SHA `8b66dcaf...`. One real zero-retry provider request returned a schema-valid continuation in `2.858s` with exact `move_to(120.5,141,-36.5)`. The production Phase 122 effective guard allowed the raw action with no issues and normalized tolerance `1.0` plus `preserve_inventory=true`. LF-normalized evidence `3e116811...` is schema-valid. Focused checks pass 6/6, Phase 134-145 evidence checks pass 51/51, all stone-pickaxe checks pass 430/430, the full repository passes 1183/1183, seven Node suites pass 84/84, and all 1841 controlled JSON files parse; compilation, diff, and credential gates pass. The probe starts no Minecraft process, creates no authorization, and grants no baseline, skill, capability, or M4 credit. It must be committed and pushed before a separate parent-bound one-use baseline authorization can be created.

Phase 146 consumes exactly one separate authorization from pushed commit `d9d27969`. Reset, empty inventory, Paper, Bridge, and five observed log sources pass. Root Planner call 0 makes one transport attempt with zero retries, receives zero response bytes, and fails with `APIConnectionError -> ConnectError -> ConnectError -> SSLEOFError`; the episode ends `empty_plan` with zero actions. Manifest `f2bb7f56...` and derived classification `8fe9fc1b...` mark it `infrastructure_ineligible` and `intervention_not_exercised_new_blocker`. The Phase 144 step-up repair is not reached, no gameplay conclusion is drawn, and no baseline, skill, capability, or M4 credit is granted. Focused checks pass 4/4, the selected evidence chain passes 55/55, all stone-pickaxe checks pass 432/432, the full repository passes 1187/1187, seven Node suites pass 84/84, and all 1855 controlled JSON files parse; compilation, diff, credential, port, and owned-process cleanup gates pass. One bounded no-Minecraft root-provider recovery probe is required before any new authorization.

Phase 147 tooling commit `d64af344` binds the Phase 146 JSONL SHA `4e77402f...`, observation fingerprint `2cd05d40...`, failed root call `llm-3e890...`, and reconstructed request SHA `97f12aa6...`. Its one permitted provider chat call made one transport request with zero retries and returned a real schema-valid exact five-node root plan plus `move_to(121,142,-36)` in `5.375s`. Evidence `c78d6153...` passes all 20 criteria. Focused checks pass 5/5, the selected Phase 134-147 evidence chain passes 60/60, all stone-pickaxe checks pass 437/437, the full repository passes 1192/1192, seven Node suites pass 84/84, and all 1857 controlled JSON files parse; compilation, diff, and credential gates pass. Evidence commit `bcf35e6c` and separate Phase 148 authorization commit `3056719f` are pushed. The probe starts no Minecraft process and grants no success credit.

Phase 148 consumes authorization `3056719f` once and only once. Episode `sp003_baseline_20260720_211838_phase148` starts from a machine-audited empty inventory, uses 23 real schema-valid single-attempt Planner calls with zero retries, and completes 23/23 actions with no raw failures. It gathers exactly three independent logs, crafts and places one table, crafts and equips one wooden pickaxe, removes exactly three independent stone sources, and crafts one stone pickaxe at the same machine-proven table. Stable terminal inventory contains `stone_pickaxe:1`; all five graph nodes, both component verifiers, and all 51 strict criteria pass in `146.235s`. Manifest `55c9026a...` binds all twelve payloads and all thirteen run artifacts retain exact hashes. This counts as baseline 1/1 only, not candidate or capability/M4 evidence. Candidate `r1` stays locked until the Phase 148 evidence commit is pushed.
...
| 8. SP-003 composite acceptance | Phase 146 is retained after thirty-four consumed authorizations; baseline 0/1 and candidates 0/3; failure evidence push and one bounded no-Minecraft root-provider recovery probe are next |
