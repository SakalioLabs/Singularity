...
Current authorization: **no live episode is authorized**. Candidates `r1` and `r4` are consumed and cannot be retried; `r2/r3/r5/r6` remain outside all future authorization. Their retained failures are immutable, both root causes pass offline regression, and the provenance fix is pushed at `36f03ee7`. Recovery window v3 is offline-verified for fresh `r7/r8/r9` and must be committed and pushed before a separate one-arm `r7` authorization may exist. Another fixture session, support reruns, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain locked.
