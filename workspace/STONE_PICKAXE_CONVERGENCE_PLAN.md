...
| SP-003 Composite Chain | 0 | One baseline, then 3 candidate successes | Phase 147 root-provider recovery probe passed; evidence push and one parent-bound baseline authorization are next |

Current phase: **Phase 147 tooling commit `d64af344` is pushed and synchronized. Its one no-Minecraft root-provider recovery request passed and evidence `c78d6153...` awaits its own commit push. SP-003 remains 0/1 baseline and 0/3 candidate successes**.

Current authorization: **none**. All prior SP-001/SP-002 authorizations remain consumed or excluded, and all thirty-four SP-003 baseline authorizations through `sp003_baseline_20260720_194725_phase146` cannot be reused. Phases 139-143 and 146 each ran exactly once and cannot be retried. Phases 145 and 147 each consumed their single permitted provider probe and cannot be rerun. Live authorization remains locked until Phase 147 evidence is pushed; then one separate parent-bound one-use baseline authorization may be created. Automatic retry, authorization reuse, candidate execution before a passing baseline, full BM-012, Probe 24, and iron mining remain locked.
