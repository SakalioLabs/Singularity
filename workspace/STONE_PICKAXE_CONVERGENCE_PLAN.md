...
| SP-003 Composite Chain | 0 | One baseline, then 3 candidate successes | Phase 131 behaviorally completed the chain but one ungrounded direct stone dig was guard-rejected, so strict eligibility remains false |

Current phase: **Phase 130 repair commit `a7f8cc14` and separate authorization commit `67f44122` are pushed. Phase 131 episode `sp003_baseline_20260720_095823_dad3c456` completed the full empty-hand chain in 23 cycles: three logs, exact plank/stick/table crafts, one table placement, one wooden-pickaxe craft and equip, three distinct stone removals, and one stone-pickaxe craft. The terminal stable inventory contains both pickaxes and the five-node task graph is complete. It remains strict-ineligible because Planner call 18 explicitly saw `navigation_only=true` and `stone_pickup_approach=true` but incorrectly emitted a direct stone dig without `source_id`. The effective Phase 122 guard rejected it at zero duration and no mutation; the next cycle recovered with `move_to`, but the attempted fourth stone dig leaves one raw/unreconciled failure and breaks exact-three-action and SP-001 component checks. Manifest `2ae4db2d...` binds the run. SP-003 remains 0/1 baseline and 0/3 candidate successes**.

Current authorization: **none**. All prior SP-001/SP-002 authorizations remain consumed or excluded, and all twenty-eight SP-003 baseline authorizations through `sp003_baseline_20260720_095823_dad3c456` cannot be reused. Phase 131 ran exactly once and cannot be retried. Its evidence must be committed and pushed before a separate Phase 132 offline repair can extend the bounded pre-dispatch replan boundary for the exact machine-grounded approach issue. Only after that repair is tested, committed, and pushed may a new parent-bound one-use baseline authorization be created. Automatic retry, authorization reuse, candidate execution before a passing baseline, full BM-012, Probe 24, and iron mining remain locked.
