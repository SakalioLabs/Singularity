...
Current phase: **Phase 7 paired evaluation recovery with three immutable baseline arms, all three support arms verified, candidates `r1`, `r4`, and `r7` retained failed, and isolated v4 at 2/3 after `r10` and `r11` passed; `learned:acquire_cobblestone@1.0.0` remains advisory and non-executable**.

Current authorization: **exactly one `candidate/r12` live episode becomes authorized only after this ledger commit is pushed**. Candidates `r1`, `r4`, `r7`, `r10`, and `r11` are consumed and cannot be retried; `r2/r3/r5/r6/r8/r9` remain excluded. R11 passed from authorization commit `41ce0d14`, its evidence is pushed at `ef40eec7`, and v4 is now 2/3. Another fixture session, support reruns, retries, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain locked.
...
- Recovery policy `stone-pickaxe-sp001-paired-evaluation-v4` is isolated under `workspace/evals/sp001_skill_evaluation_v4`. It binds route-scope fix commit `18525025`, exact `agent.py` hash `c7425d93...`, immutable v1-v3 policy/report identities, retained failed records `r1/r4/r7`, and unchanged v1 support arms. Only `r10/r11/r12` are eligible for one-arm authorizations; `r1..r9`, support reruns, retries, and normal runtime fail closed. Candidates `r10` and `r11` each passed exactly once and are frozen at evidence commits `90e4e230` and `ef40eec7`; the report is `retain_advisory` at 2/3 with inherited support 3/3 and 12/12 v4 cases passing. This separate commit authorizes exactly one fresh `candidate/r12` run after push.
...
| 7. Paired promotion evaluations | In progress; v1 `r1`, v2 `r4`, and v3 `r7` are consumed failures, v4 `r10/r11` passed, and the fresh window is 2/3 |
