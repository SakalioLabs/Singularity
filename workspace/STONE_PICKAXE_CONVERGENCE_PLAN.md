...
Current authorization: **no live episode is authorized**. Candidate `r1` is consumed and cannot be retried. Its retained failure is pushed at `b423059`; the nearest-observed grounding disconnect is reproduced, fixed, and fully regression-tested offline. This fix and a new non-overlapping evaluation window must each be committed and pushed before any further candidate arm. Candidate `r2/r3` under the current window, another fixture session, SP-002/SP-003, Probe 24, full BM-012, and iron mining remain locked.
...
| 7. Paired promotion evaluations | In progress; v1 `r1` and v2 `r4` failed and are consumed, provenance fix passes offline, fresh non-overlapping window pending |
