...
| SP-003 Composite Chain | 0 | One baseline, then 3 candidate successes | Phase 126 offline-repairs explicit table-target semantics and SP-001 machine-reachable source ordering; fresh baseline still required |

Current phase: **Phase 126 offline repair starts from pushed Phase 125 evidence commit `41b484a1`. The immutable Phase 125 episode behaviorally completes all five stages and live-proves first-tool-craft readiness, but remains strict-ineligible. Policy `sp003-explicit-table-target-semantics-v1` makes every first-target boolean literal and marks a clear table reference with `machine_proven_placement=true`; a grass or dirt reference can no longer be inferred as clearance. Policy `sp003-sp001-machine-reachable-source-order-v1` preserves the action snapshot ground and a bounded fingerprinted order of only machine-proven pickup-access stones using exact Euclidean distance. A counterfactual replay of the unchanged Phase 125 full observations preserves all three selected sources and passes SP-001 with no issues, while missing-contract legacy evidence retains its original result. The repair adds no action rewrite, retry, protocol change, or evidence credit. SP-003 remains 0/1 baseline and 0/3 candidate successes until a fresh authorized live pass**.

Current authorization: **none**. All prior SP-001/SP-002 authorizations remain consumed or excluded, and all twenty-five SP-003 baseline authorizations through `sp003_baseline_20260720_051930_f39bab4c` cannot be reused. Phase 125 ran exactly once and cannot be retried. The next transaction is the Phase 126 offline repair commit and push; only afterward may a fresh parent-bound one-use baseline authorization be committed and pushed. Automatic retry, authorization reuse, candidate execution before a passing baseline, full BM-012, Probe 24, and iron mining remain locked.
...
- Offline status: Phase 126 focused tests pass 7/7, targeted SP-003 tests pass 96/96, all stone-pickaxe Python tests pass 324/324, and the repository-wide Python gate passes 1079/1079. Seven Node suites pass 84/84 machine-counted internal cases; 774 deliverable JSON files and all 765 `workspace/evals` evidence JSON files parse. Compilation, PowerShell parsing, diff checks, retained evidence hashes, and credential scanning also pass. Audit `5eecf4ad...` binds the repair.
...
| 8. SP-003 composite acceptance | Phase 125 behavior and first-tool readiness are retained; Phase 126 target/source repair is offline verified pending commit/push; twenty-five authorizations consumed; baseline 0/1 and candidates 0/3; fresh baseline authorization required afterward |
