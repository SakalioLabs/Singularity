...
| SP-003 Composite Chain | 0 | One baseline, then 3 candidate successes | Phase 125 live-proves first table-tool-craft readiness and behaviorally closes the chain again, but stage-target and nearest-source checks remain strict-ineligible |

Current phase: **Phase 125 live-proves pushed policy `sp003-first-table-tool-craft-readiness-v1`. Repair commit `19d5780e` and separate authorization `c524b0c7` produced exactly one episode, `sp003_baseline_20260720_051930_f39bab4c`. The Agent behaviorally completes the full empty-hand chain again: all five task nodes and goal verification pass, 23/24 actions succeed in 149.0 seconds, and terminal stable inventory contains both pickaxes. The first wooden-pickaxe action now makes exactly one successful backend call and attempt with zero retry and exact signed delta; the stone-pickaxe action uses the same table. Strict baseline evidence remains false because Planner call 13 invents a clearance flag for the table reference and requests one stage-forbidden grass dig; the guard rejects it without mutation, leaving one unreconciled failure and a six-request/five-proof surface mismatch. SP-001 also rejects the stone-stage initial movable state and transitions one and three under raw-nearest-source verification because runtime access rank precedes distance. Manifest `0ef3e451...` binds thirteen files. Phase 125 evidence must be committed and pushed before the bounded Phase 126 offline repair; SP-003 remains 0/1 baseline and 0/3 candidate successes**.

Current authorization: **none**. All prior SP-001/SP-002 authorizations remain consumed or excluded, and all twenty-five SP-003 baseline authorizations through `sp003_baseline_20260720_051930_f39bab4c` cannot be reused. Phase 125 ran exactly once and cannot be retried. The next transactions are the Phase 125 evidence commit/push and then the Phase 126 offline repair commit/push. Only afterward may a fresh parent-bound one-use baseline authorization be committed and pushed. Automatic retry, authorization reuse, candidate execution before a passing baseline, full BM-012, Probe 24, and iron mining remain locked.
...
- Offline status: Phase 125 evidence replay passes 3/3, all stone-pickaxe Python suites pass 317/317, the repository-wide Python gate passes 1072/1072, seven Node suites pass 85/85 internal cases, and 1726 JSON files parse. Compilation, PowerShell parsing, diff checks, retained evidence hashes, and credential scanning also pass. The gate is rerun before each evidence or offline-fix commit.
...
| 8. SP-003 composite acceptance | Phase 125 first table-tool-craft readiness is live-proven; twenty-five authorizations consumed; behavioral closure remains strict-ineligible on stage-target and nearest-source checks; baseline 0/1 and candidates 0/3; evidence plus Phase 126 repair commits required before another authorization |
