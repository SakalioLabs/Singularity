...
Current authorization: **none**. Replicates `r1..r15` from SP-001, all nine SP-002 fixture/live authorizations, v1 `shadow/shadow-1`, v2 support `shadow-2/advisory-2/fallback-2`, and candidates `r4/r5` are consumed and cannot be reused. V1 `advisory-1`, `fallback-1`, and candidate `r1..r3` are excluded from recovery. After the Phase 68 evidence commit is pushed and `main` is synchronized, the next transaction is one separate parent-bound `candidate/r6` authorization. Executable craft promotion, automatic retry, SP-003, Probe 24, full BM-012, and iron mining remain locked.
...
| 6. Craft candidate/advisory | Advisory `learned:craft_stone_pickaxe@1.0.0` created; v2 support 3/3, candidate pairs 2/3 |
...
The retained fixture blockers, controlled SP-001 failures, first two SP-002 source failures, and v1 `shadow-1` failure remain immutable. Three eligible SP-001 successes and three eligible SP-002 successes establish both extraction gates; v5 remains frozen at 3/3, and the append-only acquire 1.1.0 executable promotion remains complete. Do not retry consumed candidates including `r4/r5`, `shadow-2`, `advisory-2`, or `fallback-2`; run excluded v1 IDs; reuse prior pair IDs; run more than the separately authorized next v2 arm; skip the craft candidate/advisory lifecycle; run SP-003 before both skills are executable; run full BM-012; run Probe 24; or begin iron mining.
