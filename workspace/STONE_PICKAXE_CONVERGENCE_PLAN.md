...
| SP-003 Composite Chain | 0 | One baseline, then 3 candidate successes | Phase 132 offline-replans the sole Phase 131 ungrounded direct stone dig; a fresh strict live baseline is still required |

Current phase: **Phase 131 evidence commit `00ee5271` is pushed and immutable. Phase 132 offline policy `sp003-grounded-approach-pre-dispatch-replan-v1` extends the existing bounded pre-dispatch recovery only for `sp003_stone_grounded_approach_required_before_dig`. It suppresses that non-executed action before action counting or backend dispatch, requires a fresh observation, permits one replan per fingerprint within the unchanged four-per-episode ceiling, and neither rewrites the action nor retries the same call. Exact Phase 131 counterfactual replay leaves 22/22 successful actions and exactly three grounded stone digs. This is offline verification only: SP-003 remains 0/1 baseline and 0/3 candidate successes until a fresh live run passes strictly**.

Current authorization: **none**. All prior SP-001/SP-002 authorizations remain consumed or excluded, and all twenty-eight SP-003 baseline authorizations through `sp003_baseline_20260720_095823_dad3c456` cannot be reused. Phase 131 ran exactly once and cannot be retried. The Phase 132 repair must be committed and pushed before one new parent-bound, one-use baseline authorization is created in a separate commit. Automatic retry, authorization reuse, candidate execution before a passing baseline, full BM-012, Probe 24, and iron mining remain locked.
