...
| SP-003 Composite Chain | 0 | One baseline, then 3 candidate successes | Phase 130 offline repair routes both dispatch gates through the effective guard and bounds exact actionless-planning replans; live validation remains pending |

Current phase: **Phase 129 evidence is committed and pushed at `f9bd709c`. Phase 130 implements two offline-only corrections. Both pre-dispatch and execution verification now resolve the current effective SP-003 guard instead of falling back to the base guard. The exact real-LLM failure shape `status=planning`, zero actions, and sole issue `planning_action_count_must_equal_one` can request a fresh-observation next-cycle replan once per state fingerprint and at most twice per episode. Duplicate, malformed, non-SP-003, or exhausted cases retain the original `empty_plan` fail-closed behavior. No action is rewritten, no backend call or action budget is consumed, and no transport retry or world mutation is introduced. Phase 123 and Phase 125 previously completed the behavioral chain and produced a stone pickaxe, but each retained an in-run failure and therefore remained strict-ineligible. SP-003 is still 0/1 baseline and 0/3 candidate successes**.

Current authorization: **none**. All prior SP-001/SP-002 authorizations remain consumed or excluded, and all twenty-seven SP-003 baseline authorizations through `sp003_baseline_20260720_081826_aeafcafc` cannot be reused. Phase 129 ran exactly once and cannot be retried. Phase 130 has passed full offline regression and must be committed and pushed before a separate parent-bound one-use baseline authorization can be created. Automatic retry, authorization reuse, candidate execution before a passing baseline, full BM-012, Probe 24, and iron mining remain locked.
