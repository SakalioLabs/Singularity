...
| SP-003 Composite Chain | 0 | One baseline, then 3 candidate successes | Phase 103 retained at the pre-stone navigation schema boundary; offline fix pending |

Current phase: **Phase 103 exercised the pushed bounded-state repair in one authorized baseline. Calls 0-11 were schema-valid and all 12 emitted actions were machine-accepted: three moves, three log digs, four crafts, one table placement, and one wooden-pickaxe equip. The terminal machine state has `wooden_pickaxe:1`, but call 12 stopped before stone removal because the SP-003 prompt requires navigation-only `move_to` with `x/z` while the generic envelope validator also requires `y`. The run remains ineligible at 0/1 baseline and 0/3 candidate successes; its 13-file bundle and exact replay are being retained before an offline mode-specific schema repair**.

Current authorization: **none**. All prior SP-001/SP-002 authorizations remain consumed or excluded, and all fourteen SP-003 baseline IDs through `sp003_baseline_20260719_154642_73c81d37` cannot be reused. The next transactions are to push Phase 103 evidence, implement and push the offline `move_to` contract repair, then create and push one separate fresh parent-bound baseline authorization-only commit. Automatic retry, authorization reuse, candidate execution before a passing baseline, full BM-012, Probe 24, and iron mining remain locked.
...
- Offline status: SP-003 dedicated cases pass 70/70, all stone-pickaxe suites pass 251/251, the repository-wide Python gate passes 1006/1006, and seven Node suites retain 67/67 internal cases. The gate is rerun before each evidence or offline-fix commit.
...
| 8. SP-003 composite acceptance | Phase 103 retained; fourteen authorizations consumed; baseline 0/1 and candidates 0/3; mode-specific move schema fix pending |
