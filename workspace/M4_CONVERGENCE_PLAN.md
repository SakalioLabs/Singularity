...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after twenty-eight consumed baseline authorizations. Phase 131 evidence commit `00ee5271` retains a behaviorally complete empty-hand-to-stone-pickaxe chain plus one pre-mutation guard rejection. Phase 132 offline policy `sp003-grounded-approach-pre-dispatch-replan-v1` now turns only that exact navigation-only direct-dig issue into a bounded fresh-observation replan before action counting or backend dispatch. Counterfactual replay leaves 22/22 successful actions and exactly three grounded stone digs, but it creates no live or capability evidence. Phase 132 must be committed and pushed before a separate parent-bound one-use baseline authorization. Capability counts and M4 remain unchanged.
