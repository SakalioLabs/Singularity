...
The separate stone-pickaxe skill project remains non-capability research and does not authorize another M4 probe. SP-001 is 3/3 with executable `learned:acquire_cobblestone@1.1.0`; SP-002 is 2/3 and SP-003 remains locked. A sealed 48-file SP-002 fixture has exact three cobblestone, two sticks, no target, and an interactive table. Four live SP-002 authorizations are permanently consumed: the first two retain their root-graph and stale-inventory failures, while the last two independently validate the Phase 51 repair. Episodes `sp002_live_20260718_193656_a6ce837e` and `sp002_live_20260718_201122_e277e1cb` each used one schema-valid real Planner call and one Mineflayer craft attempt, completed both task nodes, and proved exact `cobblestone:-3`, `stick:-2`, and `stone_pickaxe:+1` through the authoritative crafting-window refresh. They passed machine verification and evidence eligibility in 5.453 and 5.515 seconds with no retry or continuation call. All 18 SP-002 cases, 46 non-live Python suites, and six Node suites pass; another parent-bound authorization remains false until this second-success evidence commit is pushed. M4 status and all capability counts are unchanged.
