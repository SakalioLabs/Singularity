...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty-four baseline authorizations, including five infrastructure closures. Phase 144 repair commit `c30ff138` preserves the machine-proven step-up marker and makes navigation-only table staging explicit. Phase 146 consumed one parent-bound authorization, passed Minecraft preflight, and stopped before any action when root Planner call 0 returned zero bytes after one zero-retry TLS EOF transport failure; manifest `f2bb7f56...` and derived classification `8fe9fc1b...` mark it `infrastructure_ineligible`. Evidence commit `3242b4b0` and Phase 147 tooling commit `d64af344` are pushed. The single no-Minecraft Phase 147 recovery probe then returned a real schema-valid exact five-node root plan and expected first move in `5.375s` with one request and zero retries. Evidence `c78d6153...` passes its schema and all 20 criteria; repository regression passes 1192/1192 and grants no capability or M4 credit. No further SP-003 live run is permitted until that probe evidence is independently committed and pushed and one separate parent-bound one-use authorization is created. Capability counts and M4 remain unchanged.
