...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty-three baseline authorizations, including four infrastructure closures. Phase 142 evidence is pushed at `6bd489f2`; Phase 143 authorization `5b82e7bb` was consumed once. The retained run reached exact table materials and two bounded surface clearances, then exposed a compact Planner-state omission for the machine-proven step-up egress marker and ended without table placement. No further SP-003 run is permitted until Phase 143 evidence and a Phase 144 offline Planner-contract repair are each committed and pushed. Capability counts and M4 remain unchanged.
