...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after twenty-four consumed baseline authorizations: twenty-two gameplay failures and two infrastructure failures. Phase 123 evidence is pushed at `33441a18`; episode `sp003_baseline_20260720_040104_4676408a` behaviorally completes the empty-hand chain in one live goal, with all five task nodes and Agent goal verification passing and both wooden and stone pickaxes present in terminal stable inventory. Strict evidence remains false because the first table-bound wooden-pickaxe call produces no inventory delta, the next cycle repeats it successfully, and the run retains one unreconciled action failure plus two wooden-pickaxe action attempts. Phase 124 offline-verifies `sp003-first-table-tool-craft-readiness-v1`, a one-shot non-mutating exact-table preflight before the first tool craft. It preserves one original craft call per action, adds no retry, fails terminally on timeout, wrong window, or inventory mutation, reuses readiness for the stone-pickaxe craft, and leaves non-tool crafts, the shared Bridge, protocol, Planner, and action budget unchanged. There is no live authorization. The next gate is the Phase 124 fix commit and push followed by a separate one-use SP-003 baseline authorization; capability counts and M4 remain unchanged.
