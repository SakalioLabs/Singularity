...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after twenty-three consumed baseline authorizations: twenty-one gameplay failures and two infrastructure failures. Phase 121 evidence is pushed at `5bd4210f`. Phase 122 now offline-verifies process-local policy `sp003-partial-clearance-shaft-step-up-egress-v1`: only after same-level egress is machine-proven unavailable, one complete-scan-proven cardinal step may rise exactly one level onto allowed solid ground with air feet/head, inventory preservation, no world mutation, and one attempt per proof fingerprint. Retained replay selects west stand `(123.5,142,-36.5)`; the post-step replay retains support `stone:124:139:-37` and selects remaining `dirt:124:140:-37`. Same-level egress stays first, and malformed or exhausted proofs fail closed. No live authorization exists. The next gate is the Phase 122 commit and push followed by one separate parent-bound baseline authorization; capability counts and M4 remain unchanged.
