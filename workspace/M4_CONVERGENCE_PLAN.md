...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty-two baseline authorizations, including four infrastructure closures. Phase 141 evidence is pushed at `00e08cd8`. Phase 142 tooling commit `ac945428` replays the exact provider action through production guard `stone-pickaxe-sp003-action-guard-v2`; audit `d5f1f7c0...` obtains the exact expected normalized action and rejects four negative controls without a provider request or Minecraft process. No further SP-003 run is permitted until this reclassification evidence and a separate one-use authorization are each pushed. Capability counts and M4 remain unchanged.
