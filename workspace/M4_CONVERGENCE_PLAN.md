...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after twenty-seven consumed baseline authorizations. Phase 129 completed ten actions without an action failure, reaching three removed logs, exact plank/stick/table crafts, and two surface-clearance removals, but did not place the table or acquire either pickaxe or stone. It exposed a base-guard/effective-guard pre-dispatch mismatch and then ended `empty_plan` when a real planning response contained zero actions despite an available machine target. The immutable bundle uses manifest `32e7c5c6...`; no retry or evidence credit is granted. No live authorization exists, and Phase 129 evidence plus a separate Phase 130 offline repair must be pushed before any new baseline authorization. Capability counts and M4 remain unchanged.
