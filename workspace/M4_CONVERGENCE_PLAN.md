...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty-two baseline authorizations, including four infrastructure closures. Phase 136 policy `sp003-observation-delayed-log-pickup-reconciliation-v1` is pushed at `9af7db44` and passes its retained replay. Phase 138 root-provider probe evidence is pushed at `21bd9764`; Phase 139 authorization `45aa6a8d` was then consumed exactly once. Planner calls 0-2 were real and schema-valid, and the existing action-result policy fully reconciled one raw pickup failure across two distinct log removals. The Phase 136 observation-only branch was not needed. Continuation call 3 received zero bytes and failed after one zero-retry attempt with `APIConnectionError -> ConnectError -> ConnectError -> SSLEOFError`. Manifest `1ead5fe3...` and derived record `80abdfa5...` classify the run as `infrastructure_ineligible` with `intervention_not_exercised_new_blocker`. No further SP-003 run is permitted until this evidence is pushed, a bounded no-Minecraft continuation-provider probe passes, and a separate one-use authorization is pushed. Capability counts and M4 remain unchanged.
