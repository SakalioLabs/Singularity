...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty-two baseline authorizations, including four infrastructure closures. Phase 140 evidence is pushed at `c38e09a9`. Phase 141 tooling commit `009a3cfe` then issued one exact-state no-Minecraft continuation request with zero retries. The provider returned a real schema-valid exact-coordinate log dig in `4.108s`; raw artifact `2c67ac6a...` reports false only because it expected `source_id` before the real SP-003 guard injected the exact machine target ID. Retained-state guard replay is allowed with zero issues and produces the exact expected normalized action. No further SP-003 run is permitted until Phase 141 evidence and the offline guard-normalized evaluator repair are each pushed, followed by a separate one-use authorization. Capability counts and M4 remain unchanged.
