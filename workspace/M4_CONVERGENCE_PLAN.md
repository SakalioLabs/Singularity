...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after twenty-four consumed baseline authorizations: twenty-two gameplay failures and two infrastructure failures. Phase 122 is pushed at `6d9d1b5b`; Phase 123 episode `sp003_baseline_20260720_040104_4676408a` then behaviorally completes the empty-hand chain in one live goal. All five task nodes and Agent goal verification pass, three distinct stones are collected, and the terminal stable inventory includes both wooden and stone pickaxes. Strict evidence remains false because the first table-bound wooden-pickaxe call produces no inventory delta, the next cycle repeats it successfully, and the run retains one unreconciled action failure plus two wooden-pickaxe action attempts. The integrated SP-001 component also rejects initial movability and two nearest-source transitions. There is no retry or live authorization remaining. The next gate is the Phase 123 evidence push followed by an offline-only first table-tool-craft readiness repair; capability counts and M4 remain unchanged.
