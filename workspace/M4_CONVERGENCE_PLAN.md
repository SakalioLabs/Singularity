...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after twenty-five consumed baseline authorizations: twenty-three gameplay failures and two infrastructure failures. Phase 124 repair commit `19d5780e` is pushed and its separate authorization `c524b0c7` was consumed once by Phase 125 episode `sp003_baseline_20260720_051930_f39bab4c`. The full empty-hand chain closes behaviorally again with all five task nodes and Agent goal verification passing. The repaired first table-tool craft now succeeds in one call and one attempt with zero retry and exact signed inventory delta; the stone-pickaxe craft uses the same table. Strict evidence remains false because one schema-valid Planner response hallucinates a clearance flag on the table reference and requests a stage-forbidden dig, which the guard rejects without mutation. That leaves one unreconciled failure and a six-request/five-proof surface-clearance mismatch. The integrated SP-001 verifier also rejects the initial movable state and nearest-source transitions one and three because runtime access-rank sorting precedes raw distance. Manifest `0ef3e451...` binds all thirteen files. There is no live authorization; the next gate is the Phase 125 evidence commit and push followed by a bounded Phase 126 offline repair. Capability counts and M4 remain unchanged.
