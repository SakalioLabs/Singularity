...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after twenty-seven consumed baseline authorizations. Phase 123 and Phase 125 behaviorally produced a stone pickaxe from empty hand, but each retained an in-run failure and stayed strict-ineligible. Phase 129 later exposed a base-guard/effective-guard dispatch mismatch and an exact actionless-planning `empty_plan` termination; its evidence commit `f9bd709c` is pushed. Phase 130 now applies the current effective guard consistently and adds a bounded fresh-observation replan for only that exact zero-action schema failure. Full offline regression passes 1098/1098 Python tests and seven Node suites with 84/84 assertions. No live authorization, retry, evidence credit, capability credit, or M4 credit exists; a separate parent-bound authorization remains locked until Phase 130 is committed and pushed. Capability counts and M4 remain unchanged.
