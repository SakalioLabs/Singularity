...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty-one baseline authorizations, including three infrastructure closures. Phase 136 policy `sp003-observation-delayed-log-pickup-reconciliation-v1` is pushed at `9af7db44` and passes its retained replay. Phase 137 authorization `5aa70afa` was consumed exactly once, but the first root Planner request received zero bytes and failed with `APIConnectionError -> ConnectError -> ConnectError -> SSLEOFError`; no action executed and Phase 136 was not exercised. Manifest `553e8731...` and derived record `21ac9a4c...` classify the run as `infrastructure_ineligible` with `intervention_not_exercised_new_blocker`. Phase 138 then made one zero-retry no-Minecraft provider request from the exact failed root state and returned the schema-valid five-node graph plus expected first move in `5.625s`; artifact `8cd727c1...` grants no credit. No further SP-003 run is permitted until that probe evidence and a separate one-use authorization are each pushed. Capability counts and M4 remain unchanged.
