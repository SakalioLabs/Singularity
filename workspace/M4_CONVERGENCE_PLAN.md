...
- Current target: BM-014 Craft iron pickaxe; Probe 51 and Probe 53 are retained as two ineligible attempts, Probe 52 is the first independently eligible success, and the parent-bound Probe 54 authorization artifact is staged pending its own commit/push/readback
...
- Next gate: the Probe 54 authorization artifact is staged but remains unusable until its own commit is pushed and independently read back; Probe 55 remains unauthorized

## Probe 54 One-Use Authorization (Staged)
...
- Authorization artifacts: `workspace/evals/m4_probe54_authorization.json` and `tests/test_m4_probe54_authorization.py` are staged in the current transaction
- Scope: exactly one fresh BM-014 episode, fixed 300-second task limit, skills off, one Planner transport attempt, zero client/proxy retries, and credential/provider preflight before Minecraft
- Prior state: three BM-014 attempts, two retained failures, one independently eligible success, and two eligible successes still required; authorization issuance adds no episode, attempt, success, or capability credit
- Publication gate: status is `authorization_pending_its_own_push_readback`; the staged artifact is not usable until its own commit is pushed and independently read back
- Sequential lock: Probe 55 remains unauthorized
