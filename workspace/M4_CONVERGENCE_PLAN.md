...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after twenty-five consumed baseline authorizations. Phase 125 behaviorally completed the full empty-hand chain and live-proved first-tool-craft readiness, but its strict failure remains immutable under pushed evidence commit `41b484a1`. Phase 126 is offline verified: table references now expose literal placement/clearance booleans, and SP-001 evidence now preserves ground state plus fingerprinted pre-action machine-reachable stone order. The unchanged Phase 125 full observations pass a derived SP-001 counterfactual with no verifier issues, while the retained episode remains failed. No live authorization exists; Phase 126 must be committed and pushed before one separate baseline authorization. Capability counts and M4 remain unchanged.
