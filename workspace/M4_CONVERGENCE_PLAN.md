...
The separate stone-pickaxe skill project remains non-capability research and does not authorize another M4 probe. SP-001 is 3/3 with executable `learned:acquire_cobblestone@1.1.0`; SP-002 remains 0/3 and SP-003 remains locked. A sealed 48-file SP-002 fixture has exact three cobblestone, two sticks, no target, and an interactive table. Two live SP-002 authorizations are permanently consumed: the first failed before action on the root-graph contract; the second executed one verified Mineflayer craft and created a real stone pickaxe but remained ineligible on a ghost stick count and an invalid continuation. Persisted Paper playerdata proves exact server state `cobblestone:0`, `stick:0`, `stone_pickaxe:1`. The Phase 51 offline repair now performs one bounded authoritative crafting-window inventory refresh, records signed deltas, binds the consumptive action after pre-action task reconciliation, and terminates only from the exact single-attempt machine transition. It adds no craft retry and relaxes neither material criteria nor Planner validation. All 18 SP-002 cases, 46 non-live Python suites, and six Node suites pass; a fresh parent-bound authorization remains false until the fix commit is pushed. M4 status and all capability counts are unchanged.
