...
BM-011 is closed at 3/3 independently eligible fresh live successes. BM-012 Probes 1 through 27 remain ineligible at 0/3. The active provider is `grok-4.5` under revision `m4-grok-4.5-openai-compatible-v2`. Probe 27 made 57 real zero-retry calls, all schema-valid, and completed 19/19 actions. From empty inventory it produced a crafting table and wooden pickaxe, then mined coal and 2 cobblestone. The episode stalled because `Equip wooden pickaxe` had no deterministic equipment verifier: 36 complete plans were rejected as `unknown` despite exact machine hand-state evidence. Policy `m4-machine-equipment-goal-verifier-v1` now requires exact explicit equip intent, target item, and machine hand slot; inventory-only, action-only, wrong-item, and malformed evidence fail closed. Probe 28 is not authorized pending a separately committed gate. BM-013/BM-014 remain sequentially locked.
