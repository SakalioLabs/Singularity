...
- Current target: BM-014 Craft iron pickaxe; Probe 51 is retained as the first ineligible attempt, its 30-blob repair/evidence commit is pushed and read back at `0d97e431` / `cae2c41d`, and one separate zero-retry Probe 52 authorization has been issued pending its own commit/push/readback
...
- Consumption state: `consumed=false`; no consumption fields exist, no automatic retry is permitted, and Probe 53 remains unauthorized
- Transaction gate: Probe 52 remains unusable until this authorization and its test/docs/ledger are committed separately, pushed, and independently read back
