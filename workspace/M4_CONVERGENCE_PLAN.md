...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty baseline authorizations, including two infrastructure closures. Phase 134 probe commit `d21ca537` reestablished fixed-provider throughput, and Phase 135 consumed one authorization at `cf32589d`. All 15 Planner calls were real and schema-valid with zero retries and only `39.010s` cumulative provider time. A failed-but-removed log created one valid pending pickup; event 96 then machine-observed `oak_log:3` while progress remained at two verified sources plus one pending source. Because delayed pickup reconciliation exists only on a later successful dig result, Planner call 4 selected a fourth log, terminal inventory became `oak_log:4`, and ten exact plank crafts failed closed. Manifest `9ed16b45...` grants no evidence credit. The next gate is evidence push followed by one bounded observation-state delayed-pickup reconciliation fix; live authorization remains false. Capability counts and M4 remain unchanged.
