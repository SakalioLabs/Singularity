...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty-four baseline authorizations, including five infrastructure closures. Phase 144 repair commit `c30ff138` preserves the machine-proven step-up marker and makes navigation-only table staging explicit. Phase 145 made exactly one no-Minecraft, zero-retry provider request: a real schema-valid response returned exact `move_to(120.5,141,-36.5)` in `2.858s`, and the production guard normalized it with inventory preservation. Phase 146 then consumed one separate parent-bound authorization, passed all Minecraft preflight gates, and stopped before any executable action when root Planner call 0 returned zero bytes after one zero-retry TLS EOF transport failure. Manifest `f2bb7f56...` and derived classification `8fe9fc1b...` mark the run `infrastructure_ineligible`; the Phase 144 intervention was not exercised. Evidence commit `3242b4b0` is pushed. Phase 147 tooling now binds that exact root state and permits only one no-Minecraft, zero-retry provider recovery request after the tooling commit is pushed. No further SP-003 live run is permitted until the probe passes and its evidence is independently committed and pushed. Capability counts and M4 remain unchanged.
