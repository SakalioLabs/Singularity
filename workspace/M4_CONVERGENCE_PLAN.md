...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty baseline authorizations, including two infrastructure closures. Phase 135 evidence is pushed at `0e66e22d`; its valid pending removed-log proof was not reconciled when event 96 machine-observed exact `oak_log:3`, so one fourth log was gathered and ten plank crafts failed closed. Phase 136 offline policy `sp003-observation-delayed-log-pickup-reconciliation-v1` now performs one bounded pre-stage reconciliation only when fresh inventory is exact, same-family, at most three, and backed by oldest valid pending proof fingerprints. The retained replay reaches three verified and zero pending sources in one observation, removes the fourth-log target, and permits exact plank crafting; overshoot, stale, wrong-family, mixed-family, forged, repeated, action-invoking, backend-invoking, and world-mutating controls reject. The raw action failure remains visible to the episode verifier. No live process or credit occurred; the Phase 136 repair must be committed and pushed before a separate one-use SP-003 authorization. Capability counts and M4 remain unchanged.
