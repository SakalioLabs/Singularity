...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty-three baseline authorizations, including four infrastructure closures. Phase 143 evidence is pushed at `1ebbb35a`. Phase 144 now preserves the machine-proven step-up marker, makes navigation-only table staging explicit, and suppresses the retained wrong `place` once before dispatch while allowing the exact inventory-preserving move. Offline audit `77f94ee9...` is schema-valid and starts no provider or Minecraft process. No further SP-003 live run is permitted until the repair and one bounded no-Minecraft provider-probe result are each committed and pushed. Capability counts and M4 remain unchanged.
