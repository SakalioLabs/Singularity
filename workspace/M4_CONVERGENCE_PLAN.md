...
The separate stone-pickaxe project remains non-capability research and does not authorize another M4 probe. SP-001 and SP-002 are each 3/3 with executable versions `learned:acquire_cobblestone@1.1.0` and `learned:craft_stone_pickaxe@1.0.1`. SP-003 remains 0/1 baseline and 0/3 candidate after thirty-three baseline authorizations, including four infrastructure closures. Phase 144 repair commit `c30ff138` preserves the machine-proven step-up marker and makes navigation-only table staging explicit. Phase 145 then made exactly one no-Minecraft, zero-retry provider request: a real schema-valid response returned exact `move_to(120.5,141,-36.5)` in `2.858s`, and the production guard normalized it with inventory preservation. LF-normalized evidence `3e116811...` creates no authorization or capability credit. No further SP-003 live run is permitted until that probe evidence is committed and pushed and a separate parent-bound one-use authorization is created. Capability counts and M4 remain unchanged.
