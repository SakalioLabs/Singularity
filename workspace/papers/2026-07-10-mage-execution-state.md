...
**Project Action**: Schema-v2 `TaskContinuityRecord` now captures execution branches, parent/root checkpoints, depth, validation, terminal state, and revision provenance. Planner context follows one selected path and isolates failed/proposed branches as hints. `task-continuity-lineage-ablation` measures this choice against flat retrieval, while the shadow restoration report verifies failed-point provenance, ancestor reachability, isolated revision branches, and unchanged critical player state. `task-continuity-revision` remains proposal-only and automatic Minecraft restoration stays disabled.
