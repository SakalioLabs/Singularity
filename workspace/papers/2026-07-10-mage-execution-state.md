...
**Project Action**: Schema-v2 `TaskContinuityRecord` captures execution branches, parent/root checkpoints, depth, validation, terminal state, and revision provenance. The full ledger remains durable, while planner context now receives a budgeted Goal Frontier Capsule for one selected path; alternate branch details are omitted from injection. `task-continuity-lineage-ablation` measures selection, probe retention, and context cost, while shadow restoration verifies failed-point provenance, ancestor reachability, branch isolation, and unchanged critical player state. Automatic Minecraft restoration stays disabled.
