...
The prior `m4-place-target-player-occupancy-v1` gate is now live-validated. The next bounded gate is `dig_backend_required_tool_equip_grounding` and is not yet implemented: strict M4 must select a block-compatible harvest tool, equip it, and machine-confirm the held item before any dig mutation; missing tools, equip failures, and held-item mismatch must fail closed while the existing expected-drop postcondition remains authoritative. Probe 14 was the only live episode in this round. No second episode is authorized until the offline gate passes and its commit is pushed.
