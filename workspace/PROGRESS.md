...
BM-012 remains 0/3 after nineteen attempts. Probe 19 ran exactly once from committed and pushed gate commit `692cf77`. The Agent selected 15 goals, completed ten, and failed five across 89 cycles; it executed 41/66 actions successfully, made 89 real Planner calls with 85 schema-valid responses, acquired no iron, and passed 64/74 eligibility checks.

The prior hypothesis was not live-exercised. Health and food remained 20 throughout all 155 complete observations, no action entered the critical-health branch, and `m4-critical-health-survival-action-precondition-v1` emitted zero live reports. The Probe 18 deadlock did not recur, but the policy remains passed offline rather than live-validated.

Independent review overrides the Runner's coal `empty_plan` at event 424. Goal 8 subsequently machine-verified `coal:+1` through dig event 574, and later goals obtained twelve cobblestone and crafted a stone pickaxe. The earliest unrecovered transition is instead `post_place_crafting_table_machine_observation_grounding`. Plan event 611 selected a table placement; action event 621 verified the target changed from air to `crafting_table` at `(106,136,-36)`. Post-action observation event 623 did not include that block, so task reconciliation left the placement prerequisite open. Even craft actions 642 and 664 directly reported the same table as found, but the next planning observation still exposed neither that evidence nor the table.

Planner event 677 therefore asserted that the table remained in inventory and action event 686 failed at zero duration because the target was already occupied. Fourteen identical occupied-target failures followed. All 40 Planner calls in the torch goal were real and schema-valid; the goal ended at event 1419 after 40 cycles, 209.688 wall-clock seconds, and 4080 world ticks. Goal 10 then moved adjacent, observation event 1437 exposed the original table, and event 1445 machine-verified placement completion. The machine objective recovered, but the elapsed dusk budget did not.

Stone-pickaxe progression completed at event 1858. Shelter preparation did not start until world time 10039 with 103.641 seconds remaining. Two bounded shelter commands timed out; both bridge recoveries confirmed fresh machine state, but a final move ended at event 2003 after the deadline. Agent duration was 600.015 seconds and evidence duration 600.031 seconds. The last complete observation at event 1987 still showed health/food 20, a connected zero-death lifecycle, coal, cobblestone, and a stone pickaxe, but the result terminal snapshot failed closed after recovery had no remaining budget.

The bounded offline fix now passes under `m4-post-place-crafting-table-machine-observation-v1`. It is strict-M4 and crafting-table only. The exact Probe 19 action at reference `(106,135,-36)` is accepted only because the successful result reports the same requested item, reference, placed target `(106,136,-36)`, air-to-table name change, and matching integer before/after coordinates. That machine evidence is projected into exactly two observations: the immediate post-action state used by TaskSystem and GoalVerifier, and the next planner-facing state used by pre-plan reconciliation. It then expires. Unsuccessful results, result-item mismatch, placed-target drift, after-name drift, after-position drift, malformed coordinates, non-table actions, and fixed M1/M2 controls produce no projection. Observer radius and truncation, bridge placement, ActionVerifier, protocol/task-contract hashes, deadlines, success thresholds, skills, vision, and multi-agent behavior are unchanged. Two exact gate cases, 43 M4 deadline definitions, 735 full Python regression definitions, all 35 non-live Python scripts, and six Node suites with 52 internal PASS cases succeed. No live episode ran in this gate round; exactly one fresh Probe 20 is authorized only after this commit is pushed.
