...
BM-012 remains 0/3 after fifteen attempts. Probe 15 live-exercises `m4-dig-required-tool-equip-v1`: all 19 dig actions emitted policy evidence, all 13 stone digs required and confirmed `wooden_pickaxe`, and 12 successful stone actions acquired 12 cobblestone. Event 914 removed one stone without an expected drop, but event 936 then acquired the twelfth cobblestone and event 940 machine-verified the goal, so that failure recovered. The Agent selected seven goals, completed five, executed 39/61 actions successfully, and emitted 81 real schema-valid Planner calls plus one deadline-time transport failure. The first unrecovered transition is event 981 / monotonic 90944.203 / cycle 41: `move_to(108,130,-34)` exhausted its 60-second action budget while approaching observed coal, returned `command_replayed=false` and `bridge_reconnected=false`, and left 281.859 seconds in the episode. The next 16 actions had zero successes, including 15 `Not connected to bot bridge` failures; no iron was acquired.

The required-tool equip gate is passed offline and live-validated, and the bounded `m4-deadline-bound-bridge-recovery-v1` gate now passes offline. The Probe 15 timeout replay remains one command with no synchronous reconnect or replay. Before the next strict-M4 observation, the bridge now uses only the remaining absolute episode budget to establish a fresh socket, obtains a valid `get_player_state` response, and hands that confirmed state to Observer exactly once; expired reconnects make zero attempts, retry/backoff cannot cross the deadline, and failed confirmation reaches neither Observer nor Planner. The non-deadline single-shot reconnect path and fixed M1/M2 behavior remain unchanged. Validation covers five exact gate tests, 30 focused bridge/deadline tests, 727 full Python tests, and all six Node suites with 50 internal cases. No live episode ran in this offline round, so BM-012 remains at fifteen failed attempts and 0/3; exactly one fresh Probe 16 is authorized only after this gate commit is pushed.
