...
The dedicated stone-pickaxe project is paused in Phase 2 under `stone-pickaxe-skill-fixed-v1`. SP-001 Acquire Cobblestone is 0/3, SP-002 Craft Stone Pickaxe is 0/3, and SP-003 Composite Stone Pickaxe Chain is 0/3 and locked. Four separately authorized non-counting fixture-preparation sessions ran and failed machine audit; all four blockers are fixed offline, no SP-001 live episode ran, and `learned:acquire_cobblestone` plus `learned:craft_stone_pickaxe` remain uncreated.
...
- Offline validation passes 30/30 protocol cases and 21/21 runtime cases. The repository-wide non-live gate covers M1/M2/M3, GoalVerifier, TaskSystem, Skill runtime, ActionController, M4 deadlines, fixed Node suites, and Python compilation.
...
- All four terminal states failed exact wooden-pickaxe and reachable-stone checks. No forbidden intervention, result injection, stone dig, snapshot, SP-001 episode, or automatic retry occurred. Paper and Bridge stopped, ports `25565` and `30000` are clear, and `server.properties` was restored. Sixteen local evidence hashes are frozen in the failure ledger, all four authorizations are consumed, the conditional SP-001 authorization never activated, and next live authorization is false. The protocol JSON and SHA-256 remain unchanged.
