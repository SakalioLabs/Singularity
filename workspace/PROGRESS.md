...
BM-012 remains 0/3 after twenty attempts. Probe 20 ran exactly once from committed and pushed gate commit `06957bd`. The Agent selected 24 goals, completed 23 and failed one across 63 cycles; it executed 39/51 actions successfully, made 63 real Planner calls with 62 schema-valid responses, acquired `raw_iron:1`, and passed 68/74 eligibility checks.

The prior gate was live-activated twice. Event 269 accepted the first exact placement result, observation event 271 carried `m4-post-place-crafting-table-machine-observation-v1`, event 273 completed the task, event 278 machine-verified the goal, and next planner observation event 288 consumed the second bounded observation before wooden-pickaxe planning. Nine unsuccessful placement results produced fail-closed policy reports, and a later exact placement activated the gate again at event 1063. Observer directly saw both successful tables, so `projected=false` in the live observations and the exact omission-projection branch remains offline-only. The Probe 19 same-target loop did not recur.

Independent review overrides the Runner's event-820 `empty_plan`. The malformed torch response failed one goal, but goal 8 later placed a second table, goal 9 crafted a stone pickaxe, and action event 1172 machine-verified an iron-ore dig with `raw_iron:+1`. That failure is recovered and not causal.

The earliest unrecovered transition is `m4_ready_task_goal_verifier_success_criteria_bypass`. Goal 11 was selected from a ready `Mine iron ore` task. Readiness event 1186 reported one ready and one blocked task; real schema-valid plan event 1189 supplied a valid nearby iron dig and required `raw_iron:2`. Before that action could execute, pre-plan GoalVerifier event 1193 ignored the selected task criterion and accepted the generic root from existing `raw_iron:1`, with no inventory delta. The same mismatch repeated through event 1440: fourteen real schema-valid plans each proposed the next `raw_iron:2` dig, fourteen verifier calls accepted `raw_iron:1/1`, and no action executed after event 1172.

Those fourteen one-cycle false completions consumed goal indices 11 through 24. The episode stopped at `max_goals_or_stopped` after 409.484 seconds with 190.516 seconds remaining, health/food 20, a connected zero-death lifecycle, four nearby iron-ore blocks, and one raw iron. Five task-deadline interrupts all recovered and are not causal.

The bounded offline fix now passes under `m4-ready-task-goal-verifier-success-criteria-v1`. Strict M4 captures the exact task ID selected by `ready_task_selected` plus a deep-copied success-criteria snapshot. Pre-plan, Planner-complete, and post-action root verification may finish only after that exact task reaches `completed` through existing `machine_state` reconciliation or `action_result` feedback; generic GoalVerifier output cannot complete or retarget the task. Every applied decision logs the task ID, criteria, status, selection reason, verifier result, deadline state, suppression decision, and machine completion source.

The exact Probe 20 replay begins with `raw_iron:1`, a bound `raw_iron:2` task, and a generic achieved verifier result. The first report suppresses completion, the planned iron dig executes, action feedback completes the bound task at `raw_iron:2`, and the second report permits goal completion. Accepted and active tasks remain blocked; a completed same-title replacement, criteria mutation, status-only completion without a machine source, missing or malformed binding, invalid deadline, and expired deadline all fail closed. Non-ready M4 goals and fixed M1/M2 controls are unchanged. Four exact gate definitions, 47 M4 deadline definitions, 739 Python regression definitions, all 35 non-live Python scripts, six Node suites with 52 internal PASS cases, Python compilation, Node syntax, JSON parsing, capability consistency, and repository checks pass. No live episode ran in this offline round. Exactly one fresh BM-012 Probe 21 is authorized only after this gate commit is pushed; BM-013 and BM-014 remain locked.
