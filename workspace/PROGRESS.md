...
The dedicated stone-pickaxe project remains in Phase 2 under `stone-pickaxe-skill-fixed-v1`. SP-001 Acquire Cobblestone is 2/3, SP-002 Craft Stone Pickaxe is 0/3, and SP-003 Composite Stone Pickaxe Chain is 0/3 and locked. Four non-counting fixture-preparation failures, one sealed fixture, four failed SP-001 episodes, and two eligible SP-001 successes remain retained. The three behavioral root causes are fixed offline; the zero-action provider TLS failure required no runtime change. Neither prospective learned skill exists.
...
Offline fixtures and failed SP runs are explicitly non-capability evidence. The canonical capability report and custom skill library are unchanged; M1/M2/M3 remain `repeat_verified`, BM-012 remains 0/3 after twenty-three failed attempts, and M4 remains `failing`. One conditional SP-001 authorization becomes runnable only after the retained-failure/offline-fix commit is pushed.
...
- Sealed-fixture commit `aa8b7e6` remains synchronized in history; SP-001 is 2/3, SP-002 is 0/3, SP-003 is locked, and neither prospective learned skill exists.
...
- Offline validation passes 30/30 protocol cases and 29/29 runtime cases. The repository-wide non-live gate covers M1/M2/M3, GoalVerifier, TaskSystem, Skill runtime, ActionController, M4 deadlines, fixed Node suites, and Python compilation.
...
- All four failed preparations, the successful non-counting preparation, all four failed SP-001 episodes, and both successful SP-001 episodes remain immutable. Paper and Bridge stopped, ports `25565` and `30000` are clear, the fixture tree still audits to `996b2a1f989626e9c44ddca5c24f81ae55a5dca03b246f0d72723c46fd6a7636`, and all sixty SP-001 artifacts are hash-bound. One conditional SP-001 authorization becomes consumable only after the same-cell pickup fix and retained evidence commit is pushed.
