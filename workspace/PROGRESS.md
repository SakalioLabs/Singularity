...
BM-012 remains 0/3. Probe 10 still records the first schema-accepted object trigger at call event 262 / task event 265 / plan event 266 and the later `AttributeError` after event 1185; its 67/74 postmortem result remains ineligible and unchanged. The bounded offline fix now runs `m4-subtask-opportunity-trigger-type-grounding-v1` before strict-M4 task creation. It rejects a non-array trigger field, any non-string element, and empty strings; the exact Probe 10 object yields one deterministic schema issue with zero task/action acceptance. Valid string triggers remain unchanged, and TaskSystem skips legacy malformed containers and non-string values without making them match. Two focused gate tests, 27 M4 deadline tests, 95 TaskSystem tests, 76 fixed M4 Python tests, 716 full Python tests, six Node suites with 38 assertions, and Python compilation pass. This gate round ran no live episode; exactly one fresh BM-012 Probe 11 is authorized only after the gate commit is pushed.
