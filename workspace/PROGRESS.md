...
BM-012 remains 0/3 after seventeen attempts. Probe 17 ran once from committed and pushed gate commit `8ebacf9`. The Agent selected nine goals, completed five, failed three, and had one interrupted across 95 cycles; it executed 22/46 actions successfully, made 95 real Planner calls with 94 schema-valid responses, acquired no iron, and passed 55/74 eligibility checks.

The prior hypothesis was live-exercised. Event 296 rejected placement reference `(103,135,-31)` and supplied four adjacent candidates. Replan call event 311 repeated the rejected reference, so `m4-place-replan-feedback-grounding-v1` made the response schema-invalid and exposed zero tasks and actions. A later placement root repeated the initial collision once, then replan event 354 selected candidate `(104,135,-31)` and action event 367 placed the crafting table. The machine objective recovered in 13.906 seconds, so the runner's event-316 `empty_plan` candidate is not the earliest unrecovered transition.

The earliest unrecovered layer is `deadline_bound_bridge_recovery_pathfinder_readiness`. A navigation consumed its 60-second action budget while moving from `(103.5,139,-29.5)` to `(109.5,136,-33.5)`. Recovery event 550 reported `bridge_reconnected=true` and confirmed `get_player_state`, but the next navigation at event 578 failed immediately with `Path was stopped before it could be completed`. All nineteen later `move_to` actions failed with the same error and no movement, including event 1801 toward the previously reachable crafting-table coordinate. Non-navigation craft, bounded shelter, and wait actions still worked, isolating the failure to pathfinder readiness rather than bridge transport generally.

One dusk interrupt suspended the mining root, preserved its frontier, built a 9/9 machine-verified sealed shelter at event 1845, and selected night maintenance. The episode still ended 0.016 seconds after the absolute deadline; one in-flight wait result was logged post-deadline, and the independent gate failed closed.

The bounded offline fix now passes under `m4-deadline-bound-pathfinder-readiness-v1`. Strict M4 drains Mineflayer's deferred stop state after a navigation failure. If the Python bridge times out, recovery reconnects without replay, requires a policy-bound `recover_navigation` readiness response, then confirms player machine state; pending recovery clears only after both checks pass. M1/M2 behavior, protocol/task-contract hashes, deadlines, and success thresholds are unchanged. Three exact Python gate cases, two exact Node cases, 730 Python regression definitions across all 35 non-live scripts, and six Node suites with 52 internal PASS cases succeed. No live episode ran in this offline round; exactly one fresh Probe 18 is authorized after this gate commit is pushed.
