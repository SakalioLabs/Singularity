...
BM-012 remains 0/3. Probe 8 ran exactly once from committed and pushed gate `27d9a54`. `m4-place-target-occupancy-v1` rejected the observed `dark_oak_log` target at zero execution duration, requested a target-aware replan, and the next air target accepted a successful crafting-table placement; Probe 7's occupied-target timeout did not recur. The earliest failure is now `m4_task_world_state_reconciliation_grounding`: a duplicate `nearby_block_present=crafting_table` task remained accepted after machine state satisfied it because strict-M4 pre-goal reconciliation allowed only inventory criteria. The stale task was selected as a root, its plan created more descendants before GoalVerifier immediately completed it, and two dirt roots then consumed 80 cycles despite 27 successful dirt digs. Probe 8 finished ineligible at 55/74 checks with no iron-source action. This was the round's only live episode; no further run is authorized until the bounded offline reconciliation gate passes.
