...
BM-012 remains 0/3. Probe 11 live-validates the prior opportunity-trigger gate: 110/110 real responses were schema-valid, every one exposed a passing trigger-grounding report, all 117 trigger values were valid strings, and neither malformed task acceptance nor the Probe 10 `AttributeError` recurred. The rejection branch remained unexercised. Independent event review moved the first unrecovered transition ahead of the runner's navigation candidate: action event 81 / monotonic 74061.859 removed `oak_log` at `(93,139,-36)` but reported success despite no pickup, no inventory delta, and no dropped-item collection attempt. The bounded `m4-expected-drop-pickup-postcondition-v1` fix now passes offline. Strict M4 waits up to 1.5 seconds for the entity, navigates with `GoalNear(...,1)` under the existing 6-second collection bound, and returns machine-readable failure unless the expected drop reaches inventory; the same no-pickup fixture retains legacy success without the M4 flag. Full regression passes 718 Python tests and six Node suites with 40 internal PASS cases. No live episode ran in this offline round; exactly one fresh Probe 12 is authorized only after the gate commit is pushed.
