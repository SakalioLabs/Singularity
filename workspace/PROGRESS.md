...
BM-012 remains 0/3 after fourteen attempts. Probe 14 live-exercises the prior player-occupancy gate: 21 attempts against reference `(106,135,-29)` were rejected at zero duration because target `(106,136,-29)` intersected the player's collision cells, and each rejection produced a new Planner call. Action event 1554 then used adjacent reference `(107,135,-29)`, placed the crafting table at `(107,136,-29)` in 46 ms, and did not repeat Probe 13's `blockUpdate` timeout. The Agent autonomously selected seven goals, completed four, crafted a wooden pickaxe, executed 20/50 actions successfully, and made 98 real schema-valid Planner calls, but acquired no iron. Independent review overrides the runner's earlier max-cycle candidate because the placement objective later recovered. The first unrecovered action is event 1724 / monotonic 87571.656 / cycle 90: ActionVerifier accepted a stone dig because `wooden_pickaxe:1` existed, but the backend kept `oak_planks` selected, removed the stone at `(114,133,-29)`, and acquired no cobblestone. Seven stones were irreversibly removed under the same held-item mismatch, with zero successful stone digs and zero cobblestone delta. The run passed 54/74 eligibility checks and ended at the absolute deadline with no post-deadline Planner call, plan, or action.

The prior `m4-place-target-player-occupancy-v1` gate is live-validated, and the bounded `m4-dig-required-tool-equip-v1` gate now passes offline. Strict M4 forwards an explicit `require_tool_equip` transport flag, reads the target block's Mineflayer harvest metadata, selects a compatible positive inventory stack, equips it to hand, and confirms exact held name/type compatibility before `bot.dig`. The exact Probe 14 ineffective-equip replay performs one equip attempt, zero dig calls, and zero world mutations; missing-tool and equip-error controls also fail before mutation. Positive controls enforce `equip:wooden_pickaxe -> dig:stone -> cobblestone:+1` and reject a wooden pickaxe in favor of a stone pickaxe for `iron_ore -> raw_iron:+1`. Hand-harvestable blocks and the non-M4 path remain compatible, while the existing expected-drop postcondition is unchanged. Validation covers 40 focused Python tests, 722 full Python tests, and all six Node suites with 50 internal cases. No live episode ran in this offline round, so BM-012 remains at fourteen attempts and 0/3; exactly one fresh Probe 15 is authorized only after this gate commit is pushed.
