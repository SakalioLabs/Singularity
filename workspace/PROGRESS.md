...
BM-012 remains 0/3 after twenty-one attempts. Probe 21 ran exactly once from committed and pushed gate commit `84ca601`. The Agent attempted nine goals, completed three, failed five, and interrupted one across 77 cycles; it executed 38/64 actions successfully and made 77 real Planner calls with 73 schema-valid responses and 278025 total tokens. It ended at `episode_deadline`, passed 64/74 eligibility checks, and acquired no iron.

The prior gate live-activated exactly as designed. Sixty-one `m4_ready_task_goal_verifier_binding` reports contained 37 `retain_unverified_goal`, 24 `suppress_until_bound_task_machine_completion`, and zero allow decisions. Bound task `7790c984` became active at event 814, expired at event 851, and remained failed. Event 970 later placed a crafting table and event 973 achieved the generic GoalVerifier rule, but event 974 correctly suppressed completion because the exact bound task lacked an approved machine-completion state.

Independent review overrides Runner event 318. Its placement-replan `empty_plan` recovered: event 970 successfully placed the table, events 963-968 reconciled seven sibling placement tasks, event 1491 crafted a wooden pickaxe, and event 1495 completed that root. The stricter ready-task binding added churn around the expired task but did not prevent this broader recovery.

The earliest unrecovered transition is `m4_readiness_recovery_inventory_family_root_completion_disconnect`. Event 1504 selected `Acquire 4 oak_log for Craft oak_planks from oak_logs` for stale blocked task `d37f41ea`, preempting fallback `Gather 6 oak logs for iron-tool progression`. Observation event 1509 already held `dark_oak_log:4`, a wooden pickaxe, and a nearby crafting table, with iron ore visible at `(100,132,-41)`.

Event 1511 created recovery task `25fa2b53`; events 1512/1513 immediately machine-completed it because `m4-task-inventory-family-grounding-v1` canonicalized `dark_oak_log:4` to `oak_log:4`. Plan event 1519 nevertheless reasoned `Currently have 0 oak_log`, created another exact-oak collection chain, and event 1532 executed an unnecessary move. The root consumed nine cycles without closing. Dusk event 1721 suspended it with six stale accepted `Craft oak_planks from oak_logs` tasks still in the frontier; no stone-pickaxe or iron progression followed.
