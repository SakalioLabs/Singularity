...
The required decision enum is recorded as `infrastructure_ineligible` because it has no inconclusive or intervention-not-exercised value. This is causal-test ineligibility, not a preflight failure: preflight, reset, fixed controls, protocol identity, runtime health, and deadline checks passed. Probe 22 consumed its sole authorization at `autonomous_start` monotonic 78162.171 (`2026-07-14T11:53:37.544782Z`). No Probe 23, retry, code fix, or second episode ran, and next authorization is false. BM-013 and BM-014 remain locked.
