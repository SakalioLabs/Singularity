...
Those fourteen one-cycle false completions consumed goal indices 11 through 24. The episode stopped at `max_goals_or_stopped` after 409.484 seconds with 190.516 seconds remaining, health/food 20, a connected zero-death lifecycle, four nearby iron-ore blocks, and one raw iron. Five task-deadline interrupts all recovered and are not causal. The exact next gate must bind strict-M4 pre-plan acceptance of a `ready_task_selected` root to that active task's machine success criteria, while preserving ordinary GoalVerifier semantics and M1/M2 behavior. No code fix, second live episode, or new authorization exists in this round.
