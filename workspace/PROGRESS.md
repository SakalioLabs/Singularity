...
BM-012 remains 0/3 after eighteen attempts. Probe 18 ran once from committed and pushed gate commit `e78ccfc`. The Agent selected 24 goals, completed seven, failed sixteen, and interrupted one across 46 cycles; it executed 28/47 actions successfully, made 46 real Planner calls with 44 schema-valid responses, acquired no iron, and passed 67/74 eligibility checks.

The prior hypothesis was not live-exercised. There was no deadline-bound action failure, bridge recovery, `recover_navigation` report, or `PathStopped` error. Before health became critical, 8/10 moves succeeded; event 30's bounded completion failure recovered at event 51, and event 546's failure recovered at event 699. The exact Probe 17 stopped-path cascade did not recur, but the policy branch remains live-unverified.

Independent review also overrides the Runner's event-303 placement `empty_plan`: the next selected task moved adjacent, event 379 placed the crafting table at `(104,136,-30)`, and event 383 machine-verified completion. The earliest unrecovered transition is instead `critical_health_survival_action_precondition_deadlock`. Event 781 interrupted the placement root for a zombie at 2.4 blocks and preserved its frontier; emergency move event 785 succeeded, but health fell from 20 to 2.33. Goal event 796 selected immediate-threat flight, real schema-valid plan event 807 emitted a retreat `move_to`, and ActionVerifier event 812 accepted it. Observation event 814 showed health 3.33, food 17, and the zombie still 5.3 blocks away; action event 817 then returned `Pre-condition failed: Health critical` at zero duration without moving.

That blanket precondition repeated fourteen times across thirteen moves and one craft. Goal indices 12 through 24 repeatedly selected the aligned `Find food and avoid danger until health can recover` priority-2 goal; twelve failed through the same blocked action and one later malformed Planner response, exhausting the 24-goal limit with 244.437 seconds still available. The terminal bot remained connected and death-free at health 3.33, food 17, and world time 7230.

The bounded offline fix now passes under `m4-critical-health-survival-action-precondition-v1`. Only strict M4 may cross the blanket health guard, and only for a finite `move_to` while no known food is available or `use_item` naming available known food. Probe 18's `craft oak_planks`, movement while food is available, non-food use, malformed coordinates, and all fixed M1/M2 controls remain blocked before bot execution. Each activated decision emits machine-readable policy evidence in the action result. Three exact gate cases, 41 M4 deadline definitions, 733 full Python regression definitions, all 35 non-live Python scripts, and six Node suites with 52 internal PASS cases succeed. No live episode ran in this gate round; exactly one fresh Probe 19 is authorized only after this commit is pushed.
