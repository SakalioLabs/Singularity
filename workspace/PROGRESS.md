...
BM-012 remains 0/3. Probe 8 ran exactly once from committed and pushed gate `27d9a54`. `m4-place-target-occupancy-v1` rejected the observed `dark_oak_log` target at zero execution duration, requested a target-aware replan, and the next air target accepted a successful crafting-table placement; Probe 7's occupied-target timeout did not recur. The earliest failure is `m4_task_world_state_reconciliation_grounding`: a duplicate `nearby_block_present=crafting_table` task remained accepted after machine state satisfied it because strict-M4 pre-goal reconciliation allowed only inventory criteria. The bounded offline gate now passes under `m4-task-world-state-reconciliation-v1`. The exact Probe 8 task is completed from `observation.nearby_blocks` before ready-task selection; inventory, entity, landmark, missing, and malformed controls cannot satisfy the block criterion; urgent survival precedence and non-M4 pre-goal reconciliation remain unchanged. The offline-only round ran no live episode, and exactly one fresh Probe 9 is authorized after the gate commit is pushed.
