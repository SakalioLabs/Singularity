...
- Offline validation passes 30/30 protocol cases and 19/19 runtime cases. The repository-wide non-live gate covers M1/M2/M3, GoalVerifier, TaskSystem, Skill runtime, ActionController, M4 deadlines, fixed Node suites, and Python compilation.
...
- All three terminal states failed exact wooden-pickaxe and reachable-stone checks; the second additionally failed on truncated Planner output, while the third failed only Planner schema eligibility. No forbidden intervention, result injection, stone dig, snapshot, SP-001 episode, or automatic retry occurred. Paper and Bridge stopped, ports `25565` and `30000` are clear, and `server.properties` was restored. Twelve local evidence hashes are frozen in the failure ledger, all three authorizations are consumed, the conditional SP-001 authorization never activated, and next live authorization is false. The protocol JSON and SHA-256 remain unchanged.
