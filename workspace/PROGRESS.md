...
- Both terminal states failed exact wooden-pickaxe and reachable-stone checks; the second also failed Planner request eligibility because its final response was truncated. No forbidden intervention, result injection, stone dig, snapshot, SP-001 episode, or automatic retry occurred. Paper and Bridge stopped, ports `25565` and `30000` are clear, and `server.properties` was restored. Eight local evidence hashes are frozen in the failure ledger, both authorizations are consumed, the conditional SP-001 authorization never activated, and next live authorization is false. The protocol JSON and SHA-256 remain unchanged.
