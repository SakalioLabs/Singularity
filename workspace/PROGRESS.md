...
The dedicated stone-pickaxe project remains in Phase 2 under `stone-pickaxe-skill-fixed-v1`. SP-001 Acquire Cobblestone is 0/3, SP-002 Craft Stone Pickaxe is 0/3, and SP-003 Composite Stone Pickaxe Chain is 0/3 and locked. Four non-counting fixture-preparation failures, one sealed fixture, and two failed SP-001 episodes remain retained. The first SP run exposed repeated equip; the second reached terminal `cobblestone:4` but failed strict per-transition pickup provenance. Both root causes are fixed offline, and neither prospective learned skill exists.
...
- Offline validation passes 30/30 protocol cases and 28/28 runtime cases. The repository-wide non-live gate covers M1/M2/M3, GoalVerifier, TaskSystem, Skill runtime, ActionController, M4 deadlines, fixed Node suites, and Python compilation.
...
- All four failed preparations, the successful non-counting preparation, all three failed SP-001 episodes, and both successful SP-001 episodes remain immutable. Paper and Bridge stopped, ports `25565` and `30000` are clear, the fixture tree still audits to `996b2a1f989626e9c44ddca5c24f81ae55a5dca03b246f0d72723c46fd6a7636`, and one conditional SP-001 authorization becomes consumable only after the transport-failure evidence commit is pushed.
