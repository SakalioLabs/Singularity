...
BM-012 remains 0/3 after sixteen attempts. Probe 16 ran from the committed and pushed bridge-recovery gate and did not reproduce Probe 15's transport failure: deadline-bound action failures, `m4_deadline_bound_bridge_recovery` events, and `Not connected to bot bridge` action errors were all zero; `move_to` completed 10/10 and the terminal bot was connected. The gate therefore remains passed offline but was not live-exercised. The Agent selected 14 goals, completed 12, failed two, executed 30/53 actions successfully, and emitted 101 real schema-valid Planner calls plus two deadline-time transport failures. It acquired no iron and passed 66/74 eligibility checks.

The earliest causal blocker is `planner_place_replan_feedback_grounding`. Event 283 / monotonic 8932.328 rejected `place(crafting_table,106,135,-29)` because target `(106,136,-29)` intersected the player, supplied four adjacent reference candidates, and explicitly said not to retry the rejected reference. The first replan at events 292/294 was real and schema-valid but repeated that exact reference; event 304 / monotonic 8936.656 rejected it again with unchanged position and inventory. The rejected reference was attempted 20 times. The original root made 40 real schema-valid Planner calls, including 17 replans and 17 rejected actions, before failing at event 1024. Adjacent reference `(107,135,-29)` eventually succeeded at event 1413, 268.812 seconds after the first violating retry, but the failed root and elapsed budget did not recover and no iron progression followed.

The bounded offline fix now passes under `m4-place-replan-feedback-grounding-v1`. On a strict-M4 player-collision rejection, Agent sends Planner the rejected reference and the verifier's one-to-four adjacent candidates as structured, one-shot feedback. Only the next `replan` consumes it. Planner requires exactly one place action to select one supplied candidate; repeating the rejected reference, omitting place, emitting multiple place actions, selecting an external coordinate, or receiving malformed/duplicate/unbounded feedback makes the whole response schema-invalid before task creation and action execution. The exact Probe 16 response now produces zero tasks and zero actions, each of the four adjacent candidates passes independently, and legacy/non-M4 behavior is unchanged. The focused M4 deadline harness, all 35 non-live Python scripts, Python compilation, and six Node suites with 50 internal cases pass; the repository contains 729 Python regression definitions, with the real M2 integration test skipped because `OPENAI_API_KEY` is absent. No live episode ran in this offline round, so BM-012 remains at sixteen failed attempts and 0/3; exactly one fresh Probe 17 is authorized only after this gate commit is pushed.
