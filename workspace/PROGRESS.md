...
The earliest causal blocker is `planner_place_replan_feedback_grounding`. Event 283 / monotonic 8932.328 rejected `place(crafting_table,106,135,-29)` because target `(106,136,-29)` intersected the player, supplied four adjacent reference candidates, and explicitly said not to retry the rejected reference. The first replan at events 292/294 was real and schema-valid but repeated that exact reference; event 304 / monotonic 8936.656 rejected it again with unchanged position and inventory. The rejected reference was attempted 20 times. The original root made 40 real schema-valid Planner calls, including 17 replans and 17 rejected actions, before failing at event 1024. Adjacent reference `(107,135,-29)` eventually succeeded at event 1413, 268.812 seconds after the first violating retry, but the failed root and elapsed budget did not recover and no iron progression followed. The next bounded offline gate must require a strict-M4 place replan to exclude the rejected reference, choose one supplied adjacent candidate, and fail closed before action execution if the rejected reference repeats. No second live episode is authorized in this round.
