...
The dedicated stone-pickaxe project remains in Phase 2 under `stone-pickaxe-skill-fixed-v1`. SP-001 Acquire Cobblestone is 0/3, SP-002 Craft Stone Pickaxe is 0/3, and SP-003 Composite Stone Pickaxe Chain is 0/3 and locked. Four separately authorized non-counting fixture-preparation failures remain retained; the fifth preparation passed, sealed an independently audited immutable fixture, and activated one conditional SP-001 episode after the manifest/audit-fix commit is pushed. No SP-001 live episode has run, and `learned:acquire_cobblestone` plus `learned:craft_stone_pickaxe` remain uncreated.
...
- Offline validation passes 30/30 protocol cases and 22/22 runtime cases. The repository-wide non-live gate covers M1/M2/M3, GoalVerifier, TaskSystem, Skill runtime, ActionController, M4 deadlines, fixed Node suites, and Python compilation.
...
- The first independent audit command exposed host compatibility with `Path.GetRelativePath`; the runtime now uses a repository-bounded legacy-compatible helper for both audit and SP-001. Post-action reconciliation no longer requires consumed crafting inputs to remain present before closing a machine-satisfied task. Runtime tests pass 22/22 and the real fixture audit rerun passes.
- All four failed preparations remain immutable and non-counting; the fifth successful preparation also contributes no skill or capability evidence. No forbidden intervention, result injection, stone dig, SP-001 episode, or automatic retry occurred. Paper and Bridge stopped, ports `25565` and `30000` are clear, and `server.properties` was restored. Twenty local preparation evidence hashes and the tracked manifest are frozen, the conditional single SP-001 authorization is pending this commit push, and the protocol JSON/SHA-256 remain unchanged.
