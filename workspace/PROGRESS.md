...
BM-012 remains 0/3. Probe 12 live-validates the prior expected-drop gate: all five strict digs emitted `m4-expected-drop-pickup-postcondition-v1`, three acquired the expected item, two returned machine-readable failure when no inventory delta appeared, and zero strict digs falsely succeeded. The first missing-pickup failure at event 72 recovered through successful digs at events 97 and 116. Independent review overrides two earlier movement-tolerance candidates because their target blocks were subsequently removed, and fixes the first unrecovered transition at event 169 / monotonic 77764.312: pickup collection reported inner pathfinder success while ending 2.092 blocks from the dropped `oak_log`, outside the 1-block goal, with inventory unchanged at `dark_oak_log:3`. A downstream 60-second move deadline at event 212 disconnected the bridge, after which 39 actions could not connect and 147 observations were incomplete. Planner controls remained real and schema-valid for 115/115 responses, one transport error recovered, and the final request timed out at the absolute deadline. The run passed 54/74 eligibility checks, ended 0.031 seconds after the evidence boundary, acquired no iron, and does not count. The next hypothesis is restricted to `pickup_collection_pathfinder_completion_grounding`; no live run is authorized until its offline gate passes and is pushed.
