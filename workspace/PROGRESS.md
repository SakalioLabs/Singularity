...
BM-012 remains 0/3. Probe 9 ran exactly once from committed and pushed gate `9d370bf`. The new reconciliation policy appeared in all six reconciliation events and event 1797 completed five `nearby_block_present=oak_planks` shelter tasks from machine block state; no already-satisfied world-state task was reselected, but the exact Probe 8 crafting-table boundary was not reached. Its earliest unrecovered transition was `m4_planner_schema_rejection_recovery_scope`: real continuation call `llm-3bbb8772f5f0473e` at event 115 was correctly rejected for `subtask[2]:preconditions_inventory_count_invalid:oak_log`, yet the Agent had no bounded recovery for that typed issue, so events 120 and 122 emitted `empty_plan` and failed the root. The offline gate now passes under `m4-typed-schema-recovery-v1`. An exact three-subtask/six-requirement replay proves one next-cycle replan with the same goal, root, and hash-identical nonterminal task frontier, with zero invalid task acceptance or action execution; a second typed rejection emits one exhausted event and fails closed. Frontier drift, malformed evidence metrics, unrelated schema issues, deadline exhaustion, and M2 controls reject recovery. Full regression passes 714 Python tests and six Node suites with 38 internal assertions. No live episode ran in this gate round; exactly one fresh Probe 10 is authorized only after the gate commit is pushed.
