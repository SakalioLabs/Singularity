...
The dedicated stone-pickaxe project remains in Phase 2 under `stone-pickaxe-skill-fixed-v1`. SP-001 Acquire Cobblestone is 0/3, SP-002 Craft Stone Pickaxe is 0/3, and SP-003 Composite Stone Pickaxe Chain is 0/3 and locked. Four non-counting fixture-preparation failures and one successful sealed fixture remain retained. The first SP-001 episode then ran once and failed after eight successful equip actions without a dig; its evidence is frozen and the machine-state disconnect is fixed offline. `learned:acquire_cobblestone` and `learned:craft_stone_pickaxe` remain uncreated.
...
- Offline validation passes 30/30 protocol cases and 25/25 runtime cases. The repository-wide non-live gate covers M1/M2/M3, GoalVerifier, TaskSystem, Skill runtime, ActionController, M4 deadlines, fixed Node suites, and Python compilation.
