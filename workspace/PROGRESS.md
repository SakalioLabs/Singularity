...
BM-012 remains 0/3 after thirteen attempts. Probe 13 live-exercises the prior pickup-completion gate: event 96 saw `GoalNear` resolve at 1.014387 blocks against the one-block envelope, marked it ungrounded, selected the machine-validated standable cell `(93,138,-36)`, and used one `GoalBlock` fallback with 5317 ms of the shared 6000 ms budget remaining. The fallback reached 0.875760 blocks and the outer postcondition subsequently observed `oak_log:+1`, so the Probe 12 failure did not recur. The Agent autonomously completed seven goals, crafted a wooden pickaxe and four torches, observed `iron_ore` at event 428 only five blocks away, and executed 37/51 actions successfully, but acquired no iron. Independent review agrees with the runner's new first unrecovered action at event 974 / monotonic 82047.437 / cycle 40: reference block `(103,135,-31)` was grass, target `(103,136,-31)` was air, the crafting table was equipped, and both existing place policies accepted; however, the target exactly matched the player's floored feet cell. `placeBlock` therefore produced no mutation and timed out after 5000 ms. Five retries under the same goal and one later shelter retry repeated the timeout, while four unrelated later actions did not complete the interrupted placement frontier. The run used 60 real schema-valid Planner calls, passed 54/74 eligibility checks, and ended at the absolute deadline with no post-deadline Planner call, plan, or action.

The next bounded gate now passes offline as `m4-place-target-player-occupancy-v1`. Strict M4 computes player-intersected cells from a fixed 0.6-by-1.8 collision box, rejects feet/head and horizontal-boundary targets before equip or `placeBlock`, fails closed without a valid machine position, and emits at most four adjacent references for one next-cycle replan. The exact Probe 13 replay rejects at zero duration with zero bridge calls or mutation; adjacent strict placement and the non-M4 compatibility path both pass. Validation covers 117 cross-module and 722 full Python tests plus all six Node suites with 45 internal cases. No live episode ran in this offline round, so counts stay at thirteen attempts and 0/3; exactly one fresh Probe 14 is authorized only after this gate commit is pushed.
