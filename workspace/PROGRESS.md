...
The dedicated stone-pickaxe project is paused in Phase 2 under `stone-pickaxe-skill-fixed-v1`. SP-001 Acquire Cobblestone is 0/3, SP-002 Craft Stone Pickaxe is 0/3, and SP-003 Composite Stone Pickaxe Chain is 0/3 and locked. One authorized non-counting fixture-preparation session ran and failed its machine audit; its Planner blocker is fixed offline, no SP-001 live episode ran, and `learned:acquire_cobblestone` plus `learned:craft_stone_pickaxe` remain uncreated.
...
- Offline validation passes 30/30 protocol cases and 15/15 runtime cases. All 39 non-live Python scripts, six fixed Node suites, and Python compilation pass, including M1/M2/M3, GoalVerifier, TaskSystem, Skill runtime, ActionController, and M4 deadline coverage.
- The one authorized non-counting fixture-preparation session `sp_fixture_prep_20260715_143222_b0e58483` ran with Paper, Bridge, and the normal Agent/Planner. Planner call 0 ended at the 4096-token completion limit with 15685 reasoning bytes but zero response bytes, so the Agent returned `empty_plan` after one cycle and zero actions.
- The terminal survival state remained at the dark-oak-canopy spawn with an empty inventory and no reachable observed stone source. Machine audit failed `wooden_pickaxe_exact` and `three_reachable_observed_stone_sources`; no forbidden intervention, target-result injection, stone dig, snapshot, SP-001 episode, or automatic retry occurred.
- The failure was reproduced offline: `stone-pickaxe-skill-fixed-v1` was absent from the Planner fixed-request policy branch, so the declared non-thinking payload and episode-bound timeout were omitted and an empty response remained schema-valid. The repair now sends exact `extra_body={"thinking":{"type":"disabled"}}`, uses one zero-retry request bounded by remaining episode time, rejects empty output before execution, and makes no post-deadline Planner call. Fixture sealing and SP-001 eligibility also fail closed on Planner control drift. The protocol JSON and SHA-256 remain unchanged.
- Paper and Bridge stopped, controlled ports `25565` and `30000` are clear, and `server.properties` was restored to `level-name=world`. The four local evidence hashes are frozen in the failure ledger. Fixture-preparation authorization is consumed, the conditional SP-001 authorization never activated, and next live authorization is false.
