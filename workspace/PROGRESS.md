...
BM-012 remains 0/3. Probe 9 ran exactly once from committed and pushed gate `9d370bf`. The new reconciliation policy appeared in all six reconciliation events and event 1797 completed five `nearby_block_present=oak_planks` shelter tasks from machine block state; no already-satisfied world-state task was reselected, but the exact Probe 8 crafting-table boundary was not reached. The earliest unrecovered transition is now `m4_planner_schema_rejection_recovery_scope`: real continuation call `llm-3bbb8772f5f0473e` at event 115 was correctly rejected for `subtask[2]:preconditions_inventory_count_invalid:oak_log`, yet the Agent's M4 output recovery covers only `planning_actions_missing`. Event 120 therefore emitted `empty_plan` and event 122 failed the root after two successful log digs. Downstream task roots, 56 deadline recoveries, dusk shelter construction, and the terminal deadline produced no iron-source action. Probe 9 was this round's only live episode; no further run is authorized until a bounded typed schema-rejection recovery gate passes offline.
