...
BM-012 remains 0/3. Probe 10 ran exactly once from committed and pushed gate `d11c633`. All 50 Planner calls were real and schema-valid, the prior typed inventory-count rejection did not recur, and zero `m4_planner_output_recovery` events were emitted, so `m4-typed-schema-recovery-v1` remains offline-passed but was not exercised live. At event 262, call `llm-0cd847107eb747db` returned a `Mine coal ore` subtask whose `opportunity_triggers` contained an object; schema validation passed, task event 265 accepted it, and plan event 266 exposed the malformed value. Nine such plans entered the frontier. Planning-context readiness calls warned and continued, but after the second autonomous goal exhausted 40 cycles, the direct goal-selection readiness call raised `'dict' object has no attribute 'lower'` after event 1185 and aborted the runner. The last canonical event was 275.766 seconds before the absolute deadline; 30 actions included six successful oak-log digs but no iron-source action. Canonical postmortem eligibility passes 67/74 checks and rejects the run on seven terminal/resource conditions. No second episode ran, and no further live authorization exists until an offline opportunity-trigger type gate passes and is pushed.
