...
The required-tool equip gate is now passed offline and live-validated. The next bounded hypothesis is `deadline_bound_navigation_bridge_recovery`: after a deadline-bound navigation timeout, restore the bridge for the next cycle without replaying the timed-out command or weakening either deadline. Probe 15 passed 54/74 eligibility checks, ended 0.016 seconds beyond the absolute deadline with no post-deadline Planner call, plan, or action, and lacked terminal machine/lifecycle evidence after bridge loss. This was the round's only live episode; no second run is authorized before the offline recovery gate passes and is pushed.
