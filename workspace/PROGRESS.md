...
BM-012 remains 0/3. Probe 12 live-validates the prior expected-drop gate: all five strict digs emitted `m4-expected-drop-pickup-postcondition-v1`, three acquired the expected item, two returned machine-readable failure when no inventory delta appeared, and zero strict digs falsely succeeded. The first missing-pickup failure at event 72 recovered through successful digs at events 97 and 116. Independent review fixes the next blocker at event 169 / monotonic 77764.312: pickup collection reported inner pathfinder success while ending 2.092 blocks from the dropped `oak_log`, outside the 1-block goal, with inventory unchanged at `dark_oak_log:3`; the later bridge loss remains downstream. The bounded `m4-pickup-collection-completion-grounding-v1` gate now replays the exact 2.114-to-2.092 transition as ungrounded, validates the air/air/solid standable cell `(93,138,-36)`, and uses one exact `GoalBlock` fallback. The deterministic replay spends 2000 ms on direct navigation and receives only the remaining 4000 ms of the shared 6000 ms budget before acquiring `oak_log:+1`. A still-distant fallback performs no third navigation, an unsupported cell is rejected before navigation, and the same false completion retains legacy behavior without strict M4. Full regression passes 718 Python tests, 36 focused Python tests, six Node suites with 44 internal PASS cases, and Python compilation. No live episode ran in this offline round; exactly one fresh BM-012 Probe 13 is authorized only after the gate commit is pushed.
