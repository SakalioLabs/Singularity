"""Fail-closed paired evaluation for the SP-002 crafting skill.

The fixed stone-pickaxe protocol and retained SP-002 runs are immutable. This
module binds the advisory craft skill to a supplemental evaluation policy,
adapts three retained skill-off successes as baseline arms, and emits review
evidence without granting normal runtime or capability authority.
"""
...
POLICY_RELATIVE_PATH = (
    "workspace/evals/sp002_skill_evaluation/"
    "stone_pickaxe_sp002_paired_evaluation_policy.json"
)
POLICY_PATH = REPOSITORY_ROOT / POLICY_RELATIVE_PATH
POLICY = read_json(POLICY_PATH)
POLICY_SHA256 = file_sha256(POLICY_PATH)
RUN_TYPE = "stone_pickaxe_sp002_skill_evaluation_run"
REPORT_TYPE = "stone_pickaxe_sp002_skill_paired_evaluation"
AUTHORIZATION_ROOT = REPOSITORY_ROOT / "workspace/evals/sp002_skill_evaluation_runs"
...
def policy_identity_report(policy: dict | None = None) -> dict:
    ...
    issues: list[str] = []
    base = _mapping(value.get("base_protocol"))
    fixture_spec = _mapping(value.get("fixture"))
    skill_spec = _mapping(value.get("target_skill"))
    implementation = _mapping(value.get("implementation"))

    _check(issues, "policy_type", value.get("type") == "stone_pickaxe_sp002_paired_evaluation_policy")
    _check(issues, "policy_schema", value.get("schema_version") == 1)
    _check(issues, "policy_task", value.get("task_id") == "SP-002")
    _check(issues, "base_protocol_id", base.get("id") == PROTOCOL["id"])
    _check(issues, "base_protocol_hash", base.get("sha256") == PROTOCOL_SHA256)
    ...
    fixture = _read_repository_json(fixture_spec.get("path"), issues, "fixture")
    fixture_snapshot = _mapping(fixture.get("snapshot"))
    _check(issues, "fixture_id", fixture.get("fixture_id") == fixture_spec.get("id"))
    _check(issues, "fixture_protocol", fixture.get("protocol_sha256") == PROTOCOL_SHA256)
    _check(issues, "fixture_tree", fixture_snapshot.get("tree_sha256") == fixture_spec.get("tree_sha256"))
    ...
    skills = _jsonl_records(skill_spec.get("record_path"), issues, "skill_records")
    exact_skills = [
        record
    ...
    _check(
        ...
        "skill_record_hash",
        bool(skill)
        and canonical_record_sha256(skill) == skill_spec.get("record_canonical_sha256"),
    )
    skill_gate = _mapping(skill.get("gate"))
    _check(issues, "skill_executable_gate_absent", not skill_gate.get("executable_promotion"))

    candidates = _jsonl_records(skill_spec.get("queue_path"), issues, "candidate_queue")
    exact_candidates = [
        record for record in candidates if record.get("id") == skill_spec.get("candidate_id")
    ]
    _check(issues, "candidate_record_count", len(exact_candidates) == 1)
    ...
    _check(
        ...
        bool(candidate)
        and canonical_record_sha256(candidate)
        == skill_spec.get("queue_record_canonical_sha256"),
    )

    for label, raw_record in sorted(implementation.items()):
        record = _mapping(raw_record)
        _check_file(issues, f"implementation_{label}", record.get("path"), record.get("sha256"))

    task = next(item for item in PROTOCOL["tasks"] if item["id"] == "SP-002")
    fixed = _mapping(value.get("fixed_controls"))
    _check(issues, "fixed_goal", fixed.get("goal") == SP002_GOAL)
    ...
    _check(
        issues,
        "fixed_action_timeout",
        fixed.get("per_action_timeout_s") == PROTOCOL["deadline_policy"]["per_action_timeout_s"],
    )
    _check(issues, "fixed_runtime_policy", fixed.get("runtime_policy_id") == SP002_RUNTIME_POLICY_ID)
    ...
    bindings = value.get("pair_bindings") if isinstance(value.get("pair_bindings"), list) else []
    _check(issues, "three_pair_bindings", len(bindings) == 3)
    for index, binding in enumerate(bindings, 1):
        item = _mapping(binding)
        prefix = f"baseline_{index}"
        for label in ("episode", "verification", "manifest", "session"):
            _check_file(
                issues,
                f"{prefix}_{label}_file",
                item.get(f"{label}_path"),
                item.get(f"{label}_sha256"),
            )
        episode = _read_repository_json(item.get("episode_path"), issues, f"{prefix}_episode")
        verification = _read_repository_json(
            item.get("verification_path"), issues, f"{prefix}_verification"
        )
        manifest = _read_repository_json(item.get("manifest_path"), issues, f"{prefix}_manifest")
        baseline_sessions.append(str(episode.get("session_id") or ""))
        baseline_episodes.append(str(episode.get("episode_id") or ""))
        _check(issues, f"{prefix}_task", episode.get("task_id") == "SP-002")
        _check(issues, f"{prefix}_episode_id", episode.get("episode_id") == item.get("baseline_episode_id"))
        _check(issues, f"{prefix}_session_id", episode.get("session_id") == item.get("baseline_session_id"))
        _check(issues, f"{prefix}_selected_skills", episode.get("selected_skills") == [])
        _check(issues, f"{prefix}_verification_passed", verification.get("passed") is True)
        _check(issues, f"{prefix}_manifest_passed", manifest.get("passed") is True)
    ...
    return {
        "type": "stone_pickaxe_sp002_paired_policy_identity",
        "schema_version": 1,
...
def build_evaluation_authorization(
    *,
    ...
    if not identity["passed"]:
        raise ValueError(f"SP-002 evaluation policy identity failed: {identity['issues']}")
    spec = arm_spec(normalized_arm)
    if normalized_replicate not in spec.get("replicate_ids", []):
        raise ValueError(
            f"replicate {normalized_replicate!r} is not allowed for arm {normalized_arm!r}"
        )
    if not normalized_episode or len(normalized_episode) > 96:
        raise ValueError("episode_id must be a bounded identifier")
    predecessor = str(authorization_predecessor or "").strip().lower()
    if not _full_commit(predecessor):
        raise ValueError("authorization_predecessor must be a full lowercase commit")
    duplicates = _duplicate_run_ids(
        normalized_arm,
    ...
    return {
        ...
        "schema_version": 1,
        "authorization_id": (
            f"sp002-eval:{normalized_arm}:{normalized_replicate}:{normalized_episode}"
        ),
        ...
        "runtime_scope": "controlled_live_sp002_skill_evaluation_only",
...
def validate_evaluation_authorization(
    ...
    *,
    ...
    _check(issues, "authorization_replicate", replicate in spec.get("replicate_ids", []))
    _check(issues, "authorization_mode", value.get("skill_execution_mode") == spec.get("skill_execution_mode"))
    _check(
        issues,
        "authorization_direct_execution",
        value.get("direct_skill_execution_allowed")
        is spec.get("direct_skill_execution_allowed"),
    )
    _check(issues, "authorization_policy", value.get("policy_id") == POLICY["id"])
    ...
    _check(issues, "authorization_task", value.get("task_id") == "SP-002")
    ...
    _check(issues, "authorization_skill_name", value.get("skill_name") == target["name"])
    _check(issues, "authorization_skill_version", value.get("skill_version") == target["version"])
    _check(issues, "authorization_skill_status", value.get("skill_status") == target["required_status"])
    _check(issues, "authorization_candidate", value.get("candidate_id") == target["candidate_id"])
    _check(
        issues,
        "authorization_skill_hash",
        value.get("skill_record_canonical_sha256") == target["record_canonical_sha256"],
    )
    _check(issues, "authorization_promotion_hash", value.get("promotion_sha256") == target["promotion_sha256"])
    _check(issues, "authorization_fixture", value.get("fixture_tree_sha256") == POLICY["fixture"]["tree_sha256"])
    _check(issues, "authorization_runtime_forbidden", value.get("normal_runtime_permission") is False)
    ...
    if expected_replicate_id:
        _check(
            issues,
            "authorization_expected_replicate",
            replicate == str(expected_replicate_id).lower(),
        )
    if expected_episode_id:
        _check(issues, "authorization_expected_episode", value.get("episode_id") == expected_episode_id)
    if current_head or parent_head:
        current = str(current_head or "").lower()
        parent = str(parent_head or "").lower()
        _check(issues, "authorization_current_head", _full_commit(current))
        _check(issues, "authorization_parent_head", _full_commit(parent))
        _check(issues, "authorization_commit_is_new", current != parent)
        _check(
            issues,
            "authorization_predecessor",
            value.get("authorization_predecessor") == parent,
        )
    if arm == "candidate":
        binding = pair_binding(replicate)
        _check(issues, "authorization_pair_binding", value.get("baseline_binding") == binding)
        _check(issues, "authorization_pair_id", value.get("pair_id") == binding.get("pair_id"))
    else:
        _check(issues, "authorization_support_binding", value.get("baseline_binding") == {})
        _check(issues, "authorization_support_pair", value.get("pair_id") == "")
    return {
        "type": "stone_pickaxe_sp002_skill_evaluation_authorization_audit",
        "schema_version": 1,
        "passed": not issues,
        "issues": sorted(set(issues)),
        "arm": arm,
...
def build_skill_evaluation_runtime_config(
    *,
    ...
    if not audit["passed"]:
        raise ValueError(f"SP-002 evaluation authorization rejected: {audit['issues']}")
...
class StonePickaxeSP002SkillEvaluationAgent(StonePickaxeRuntimeAgent):
    """SP-002 agent whose learned-skill artifacts remain read-only."""
...
def build_skill_evaluation_episode(*, authorization: dict, **kwargs: Any) -> dict:
    episode = build_sp002_episode(**kwargs)
    ...
    expected = POLICY["target_skill"]
    exact_selected = [
        item
        for item in selected
        if isinstance(item, dict)
        and item.get("skill_id") == expected["skill_id"]
        and item.get("version") == expected["version"]
        and item.get("status") == expected["required_status"]
    ]
    selection_allowed = (
    ...
    eligibility["passed"] = bool(
        ...
        and eligibility.get("backend_single_craft_attempt") is True
...
def build_skill_evaluation_run(
    *,
    ...
    arm = str(authorization.get("arm") or "")
    metrics = evaluation_metrics(events, episode, verification)
    ...
    checks = {
        ...
        "policy_identity_passed": identity["passed"],
        "source_evidence_integrity": _source_evidence_valid(source_evidence),
        ...
        "skill_artifact_read_only": (
            identity.get("skill_record_canonical_sha256")
    ...
    if arm == "shadow":
        ...
        checks.update({
            ...
            "single_skill_action_executed": metrics["skill_executed_count"] == 1,
            "single_skill_completion": metrics["skill_completion_count"] == 1,
...
def build_baseline_index() -> dict:
    ...
    return {
        "type": "stone_pickaxe_sp002_skill_baseline_index",
        "schema_version": 1,
        "policy_id": POLICY["id"],
        "policy_sha256": POLICY_SHA256,
        "task_id": "SP-002",
...
def build_paired_evaluation_report(run_paths: Iterable[str | Path]) -> dict:
    ...
    for raw_path in run_paths:
        ...
        if not integrity["passed"]:
            errors.extend(
                f"{run.get('run_id', repo_relative(path))}:{issue}"
    ...
    for binding in POLICY["pair_bindings"]:
        ...
        pair = {
            ...
            "candidate_integrity": bool(candidate)
            and verify_run_record(candidate)["passed"],
    ...
    baseline_sessions = sorted({
        pair["baseline_session_id"] for pair in valid_pairs if pair["baseline_session_id"]
    })
    candidate_sessions = sorted({
        pair["candidate_session_id"] for pair in valid_pairs if pair["candidate_session_id"]
    })
    ...
    no_action_failure_regression = (
        aggregate["candidate_failed_actions"] <= aggregate["baseline_failed_actions"]
    )
    no_verifier_regression = (
        aggregate["candidate_verifier_rejects"] <= aggregate["baseline_verifier_rejects"]
    )
    no_no_progress_regression = (
        aggregate["candidate_no_progress_loops"] <= aggregate["baseline_no_progress_loops"]
    )
...
def verify_run_record(run: Any) -> dict:
    ...
    _check(issues, "run_task", value.get("task_id") == "SP-002")
    _check(
        issues,
        "run_arm",
        value.get("arm") in {"baseline", "shadow", "advisory", "fallback", "candidate"},
    )
    ...
    _check(
        ...
        value.get("record_payload_sha256") == _record_payload_sha256(value),
    )
...
def arm_spec(arm: str) -> dict:
    ...
    spec = _mapping(POLICY.get("arms", {}).get(normalized))
    if not spec:
        raise ValueError(f"unsupported SP-002 evaluation arm: {arm}")
    return spec


def pair_binding(replicate_id: str) -> dict:
    normalized = str(replicate_id or "").strip().lower()
    matches = [
        binding
        for binding in POLICY.get("pair_bindings", [])
        if binding.get("replicate_id") == normalized
    ]
    if len(matches) != 1:
        raise ValueError(f"exactly one SP-002 baseline binding is required for {normalized!r}")
    return dict(matches[0])


def fixed_control_profile(episode: dict) -> dict:
    fixed = POLICY["fixed_controls"]
    fixture = _mapping(episode.get("fixture"))
    return {
        "protocol_id": PROTOCOL["id"],
        "protocol_sha256": PROTOCOL_SHA256,
        "task_id": "SP-002",
...
def initial_state_fingerprint(observation: Any) -> str:
    ...
    tables = []
    for block in value.get("nearby_blocks", []) if isinstance(value.get("nearby_blocks"), list) else []:
        if isinstance(block, dict) and block.get("name") == "crafting_table":
            tables.append({
                "name": "crafting_table",
    ...
    payload = {
        ...
        "crafting_tables": sorted(
            tables,
            key=lambda item: json.dumps(item["position"], sort_keys=True),
        ),
    }
    return canonical_record_sha256(payload)


def discover_evaluation_run_paths(
...
def evaluation_metrics(events: list[dict], episode: dict, verification: dict) -> dict:
    values = events if isinstance(events, list) else []
    action_events = [
        event for event in values if isinstance(event, dict) and event.get("type") == "action"
    ]
    skill_actions = []
    for event in action_events:
        data = _mapping(event.get("data"))
        action = _mapping(data.get("action"))
        context = _mapping(action.get("skill_context"))
    ...
    verifier_rejects = sum(
        ...
        and _mapping(_mapping(event.get("data")).get("verification")).get("status") == "reject"
    )
    steps_verified = bool(skill_actions) and all(
        _mapping(data.get("result")).get("success") is True
        and _mapping(_mapping(data.get("result")).get("action_verification")).get("status")
        == "accept"
    ...
    exact_context = bool(skill_actions) and all(
        _mapping(_mapping(data.get("action")).get("skill_context")).get("skill_id")
        == exact["skill_id"]
        and _mapping(_mapping(data.get("action")).get("skill_context")).get("version")
        == exact["version"]
        and _mapping(_mapping(data.get("action")).get("skill_context")).get("status")
        == exact["required_status"]
        for data in skill_actions
    )
    result = _mapping(episode.get("goal_result"))
    return {
        ...
        "successful_actions": sum(
            1
            for event in action_events
            if _mapping(_mapping(event.get("data")).get("result")).get("success") is True
        ),
        ...
        "planner_calls": sum(
            1
            for event in values
            if isinstance(event, dict) and event.get("type") == "llm_planner_call"
        ),
        ...
        "skill_selected_count": sum(
            1
            for event in values
            if isinstance(event, dict) and event.get("type") == "skill_selected"
        ),
        ...
        "skill_fallback_count": sum(
            1
            for event in values
            if isinstance(event, dict) and event.get("type") == "skill_fallback"
        ),
        "skill_shadow_plan_count": sum(
            1
            for event in values
            if isinstance(event, dict) and event.get("type") == "skill_shadow_plan"
        ),
        "skill_advisory_hint_count": sum(
            1
            for event in values
            if isinstance(event, dict) and event.get("type") == "skill_advisory_hint"
        ),
...
def _baseline_run_record(binding: dict) -> dict:
    ...
    metrics = evaluation_metrics(events, episode, verification)
    checks = {
        "retained_episode_hash": file_sha256(REPOSITORY_ROOT / binding["episode_path"])
        == binding["episode_sha256"],
        "retained_verification_hash": file_sha256(
            REPOSITORY_ROOT / binding["verification_path"]
        ) == binding["verification_sha256"],
        "retained_manifest_hash": file_sha256(REPOSITORY_ROOT / binding["manifest_path"])
        == binding["manifest_sha256"],
        "retained_session_hash": file_sha256(REPOSITORY_ROOT / binding["session_path"])
        == binding["session_sha256"],
...
def _promotion_gate(report: dict, promotable: bool) -> dict:
    ...
    gate = {
        ...
        "transfer_scope": {
            ...
            "quantity_range": {"minimum": 1, "default": 1, "maximum": 1},
            "recipe_inputs": {"cobblestone": 3, "stick": 2},
            "nearby_block_required": "crafting_table",
        },
...
def _aggregate_metrics(pairs: list[dict]) -> dict:
    ...
    result["environment_step_gain"] = (
        result["baseline_environment_steps"] - result["candidate_environment_steps"]
    )
    result["planner_call_gain"] = (
        result["baseline_planner_calls"] - result["candidate_planner_calls"]
    )
    return result
...
def _duplicate_run_ids(
    ...
    for raw_path in candidates:
        ...
        try:
            ...
            continue
        if (
            str(record.get("arm") or "") == arm
            and str(record.get("replicate_id") or "") == replicate_id
        ):
            duplicates.append(repo_relative(path))
    return sorted(set(duplicates))


def _source_evidence_valid(source_evidence: Any) -> bool:
    ...
    for item in source_evidence:
        ...
        if not raw_path or raw_path in seen or not _sha256(expected_hash):
            return False
        seen.add(raw_path)
        try:
