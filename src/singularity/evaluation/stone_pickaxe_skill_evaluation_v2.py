"""Isolated recovery window for the SP-001 learned-skill evaluation.

The original v1 policy, harness, report, and retained runs are immutable. This
adapter evaluates only fresh candidate replicates while importing the three v1
...
POLICY_RELATIVE_PATH = "workspace/evals/stone_pickaxe_paired_evaluation_policy_v2.json"
...
def policy_identity_report(policy: dict | None = None) -> dict:
    ...
    failed = window.get("retained_failed_run", {}) if isinstance(window.get("retained_failed_run"), dict) else {}
    supports = value.get("inherited_support_bindings", [])
    supports = supports if isinstance(supports, list) else []

    _check(issues, "recovery_policy_id", value.get("id") == "stone-pickaxe-sp001-paired-evaluation-v2")
    _check(issues, "recovery_window_id", window.get("id") == "sp001-acquire-runtime-fix-recovery-v2")
    ...
    _check(issues, "runtime_fix_commit", window.get("runtime_fix_commit") == "473b05a93440293c1e06fbfeaed98d44002e2e95")
    _check(
        issues,
        "candidate_replicates_exact",
        value.get("arms", {}).get("candidate", {}).get("replicate_ids") == ["r4", "r5", "r6"],
    )
    _check(issues, "excluded_replicates_exact", window.get("excluded_replicate_ids") == ["r1", "r2", "r3"])
    ...
    _check(issues, "failed_run_replicate", failed.get("replicate_id") == "r1")
    ...
    _check(issues, "candidate_evidence_fresh", candidate_spec.get("evidence_source") == "fresh_v2_live")
...
class StonePickaxeSkillEvaluationAgent(_v1.StonePickaxeSkillEvaluationAgent):
    """Evaluation agent pinned to the isolated v2 recovery policy."""
