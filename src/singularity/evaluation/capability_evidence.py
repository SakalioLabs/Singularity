...
def _normalize_m4_episode_bundle(
    ...
    bundle_protocol_sha256 = str(manifest.get("protocol_sha256") or "").lower().strip()
    independent = evaluate_m4_episode_for_protocol_hash(
        events,
        result,
        preflight,
        manifest,
    ...
    supported_protocols = supported_protocol_sha256s()
    if eligibility.get("protocol_sha256") not in supported_protocols:
        issues.append("m4_eligibility_protocol_mismatch")
    if manifest.get("protocol_sha256") not in supported_protocols:
        issues.append("m4_manifest_protocol_mismatch")
    if eligibility.get("protocol_sha256") != bundle_protocol_sha256:
        issues.append("m4_bundle_protocol_mismatch")
