"""Second isolated recovery window for the SP-001 learned-skill evaluation.

The v1 and v2 policies, reports, and retained failures are immutable. This
adapter evaluates only fresh r7-r9 candidates while importing the three v1
support arms through exact path and hash bindings.
"""
...
POLICY_RELATIVE_PATH = (
    "workspace/evals/sp001_skill_evaluation_v3/"
    "stone_pickaxe_paired_evaluation_policy_v3.json"
)
...
_PREVIOUS_POLICY_RELATIVE_PATH = _v2.POLICY_RELATIVE_PATH
_PREVIOUS_POLICY = _v2.POLICY
_PREVIOUS_POLICY_SHA256 = _v2.POLICY_SHA256
_PREVIOUS_POLICY_IDENTITY_REPORT = _v2.policy_identity_report
...
def policy_identity_report(policy: dict | None = None) -> dict:
    ...
    _check(issues, "recovery_policy_id", value.get("id") == "stone-pickaxe-sp001-paired-evaluation-v3")
    _check(issues, "recovery_window_id", window.get("id") == "sp001-acquire-skill-attribution-recovery-v3")
    ...
    _check(
        issues,
        "runtime_fix_commit",
        window.get("runtime_fix_commit") == "36f03ee71b2bae25df60f9d4a966fccb5e28e81f",
    )
    _check(
        issues,
        "candidate_replicates_exact",
        value.get("arms", {}).get("candidate", {}).get("replicate_ids") == ["r7", "r8", "r9"],
    )
    _check(
        issues,
        "excluded_replicates_exact",
        window.get("excluded_replicate_ids") == ["r1", "r2", "r3", "r4", "r5", "r6"],
    )
    ...
    _check(issues, "two_retained_failed_runs", len(failed_runs) == 2)
    expected_failures = [
        ("r1", _BASE_POLICY.get("id"), _BASE_POLICY_SHA256),
        ("r4", _PREVIOUS_POLICY.get("id"), _PREVIOUS_POLICY_SHA256),
    ]
    actual_failures = [
        (
    ...
    _check(issues, "recovery_pair_replicates", pair_replicates == ["r7", "r8", "r9"])
    ...
    _check(issues, "candidate_evidence_fresh", candidate_spec.get("evidence_source") == "fresh_v3_live")
...
class StonePickaxeSkillEvaluationAgent(_v1.StonePickaxeSkillEvaluationAgent):
    """Evaluation agent pinned to the isolated v3 recovery policy."""
    ...
    for raw_path in run_paths:
        ...
        if run.get("policy_id") == POLICY["id"] or run.get("policy_sha256") == POLICY_SHA256:
            if run.get("policy_id") != POLICY["id"] or run.get("policy_sha256") != POLICY_SHA256:
                ...
                selected[relative] = path
            continue
        if run.get("policy_id") in {_BASE_POLICY.get("id"), _PREVIOUS_POLICY.get("id")}:
            excluded.append({
