"""Isolated supplemental runtime for SP-002 stone-pickaxe crafting.

The SP-001 runtime is evidence-frozen, so this module retains its proven helper
surface while adding SP-002-only guards, observations, and evidence adaptation.
"""
...
SP002_RUNTIME_POLICY_ID = "stone-pickaxe-sp002-controlled-runtime-v1"
...
SP002_GOAL = (
    "Craft exactly 1 stone_pickaxe from 3 cobblestone and 2 sticks at the "
    "observed crafting table"
)
FIXTURE_GOAL = (
...
SP002_FIXTURE_GOAL = (
    "Prepare the SP-002 fixture using only normal survival actions: keep exactly "
    "two sticks, acquire exactly three cobblestone with the wooden pickaxe, place "
    "or approach one crafting table until it is observed within interaction range, "
    "and keep stone_pickaxe at zero. Do not craft a stone pickaxe."
)
...
def build_sp002_authorization(
    *,
    ...
    harness_policy_path: str,
    harness_policy_sha256: str,
) -> dict:
    normalized_scope = str(scope or "").strip().lower()
    if normalized_scope not in {"fixture_preparation", "live_episode"}:
        raise ValueError("scope must be fixture_preparation or live_episode")
    normalized_episode_id = str(episode_id or "").strip().lower()
    if not re.fullmatch(r"[a-z0-9][a-z0-9_-]{0,95}", normalized_episode_id):
        raise ValueError("episode_id must be a bounded lowercase ASCII identifier")
    identity = {
        "scope": normalized_scope,
        "episode_id": normalized_episode_id,
        "authorization_predecessor": str(authorization_predecessor).lower(),
        "fixture_path": str(fixture_path).replace("\\", "/"),
        "fixture_sha256": str(fixture_sha256).lower(),
        "fixture_id": str(fixture_id),
        "fixture_tree_sha256": str(fixture_tree_sha256).lower(),
        "harness_policy_path": str(harness_policy_path).replace("\\", "/"),
        "harness_policy_sha256": str(harness_policy_sha256).lower(),
        "protocol_sha256": PROTOCOL_SHA256,
    }
    return {
        "type": "stone_pickaxe_sp002_one_time_authorization",
        "schema_version": 1,
...
def verify_sp002_authorization(
    authorization: Any,
    *,
    expected_scope: str,
    current_head: str,
    ...
    expected_identity = {
        "scope": str(expected_scope or "").strip().lower(),
        "episode_id": str(value.get("episode_id") or ""),
        "authorization_predecessor": str(value.get("authorization_predecessor") or "").lower(),
        "fixture_path": str(fixture_path).replace("\\", "/"),
        "fixture_sha256": str(fixture_sha256).lower(),
        "fixture_id": str(fixture_manifest.get("fixture_id") or ""),
        "fixture_tree_sha256": str(
            (fixture_manifest.get("snapshot") or {}).get("tree_sha256") or ""
        ).lower(),
        "harness_policy_path": str(harness_policy_path).replace("\\", "/"),
        "harness_policy_sha256": str(harness_policy_sha256).lower(),
        "protocol_sha256": PROTOCOL_SHA256,
    ...
    checks = {
        "authorization_type": value.get("type") == "stone_pickaxe_sp002_one_time_authorization",
        "authorization_schema": value.get("schema_version") == 1,
        "authorization_status": value.get("status") == "active",
        "task_id": value.get("task_id") == "SP-002",
        "scope": value.get("scope") == expected_identity["scope"],
        "episode_id": bool(expected_identity["episode_id"]),
        ...
        "automatic_retry_disabled": value.get("automatic_retry_allowed") is False,
        "support_rerun_disabled": value.get("support_rerun_allowed") is False,
        "sp003_disabled": value.get("sp003_allowed") is False,
        ...
        "authorization_predecessor": (
            str(parent_head).lower() == expected_identity["authorization_predecessor"]
        ),
        "fixture_path": value.get("fixture_path") == expected_identity["fixture_path"],
        "fixture_sha256": value.get("fixture_sha256") == expected_identity["fixture_sha256"],
        "fixture_id": value.get("fixture_id") == expected_identity["fixture_id"],
        "fixture_tree_sha256": (
            value.get("fixture_tree_sha256")
            == expected_identity["fixture_tree_sha256"]
        ),
        "harness_policy_path": (
            value.get("harness_policy_path")
            == expected_identity["harness_policy_path"]
        ),
        "harness_policy_sha256": (
            value.get("harness_policy_sha256")
            == expected_identity["harness_policy_sha256"]
        ),
        "authorization_fingerprint": (
            value.get("authorization_id") == canonical_sha256(expected_identity)
        ),
    }
    return {
        "type": "stone_pickaxe_sp002_authorization_preflight",
        "schema_version": 1,
...
def audit_sp002_bridge_protocol_status(
    status: Any,
    *,
    episode_id: str,
    level_name: str,
    ...
    runtime = (
        value.get("runtime_controls")
        if isinstance(value.get("runtime_controls"), dict)
        else {}
    )
    checks = {
        "command_success": value.get("success") is True,
        "configured": value.get("configured") is True,
        "profile": value.get("profile") == "m4-fixed-v1",
        "minecraft_version": (
            value.get("minecraft_version")
            == PROTOCOL["environment"]["minecraft_version"]
            and value.get("observed_minecraft_version")
            == PROTOCOL["environment"]["minecraft_version"]
        ),
        "seed": (
            str(value.get("seed") or "")
            == str(PROTOCOL["environment"]["world_seed"])
        ),
        "episode_id": value.get("episode_id") == episode_id,
        "level_name": value.get("level_name") == level_name,
        "server_jar": (
            str(value.get("server_jar_sha256") or "").lower()
            == PROTOCOL["environment"]["server_jar_sha256"]
        ),
        "skill_execution_off": runtime.get("skill_execution_mode") == "off",
        "no_protocol_errors": value.get("errors") == [],
    }
    return {
        "type": "stone_pickaxe_sp002_bridge_protocol_audit",
        "schema_version": 1,
        "passed": all(checks.values()),
        "checks": checks,
        "issues": sorted(key for key, passed in checks.items() if not passed),
...
def _position_distance(left: Any, right: Any) -> float | None:
    first = left if isinstance(left, dict) else {}
    second = right if isinstance(right, dict) else {}
    deltas = []
    for axis in ("x", "y", "z"):
        left_value = _finite(first.get(axis))
        right_value = _finite(second.get(axis))
...
def evidence_observation(
    ...
    *,
    ...
    include_crafting_table: bool = False,
) -> dict:
    ...
    if include_crafting_table:
        ...
        for block in raw.get("nearby_blocks", []) if isinstance(raw.get("nearby_blocks"), list) else []:
            ...
            identifier = source_id("crafting_table", block_position)
            distance = _finite(block.get("distance"))
            if distance is None:
                distance = _position_distance(position, block_position)
            if not identifier or distance is None:
                continue
            tables.append({
                "block_id": identifier,
                "name": "crafting_table",
                ...
                "distance": round(distance, 6),
                "observed": True,
                "interactive": distance <= table_range,
            })
        tables.sort(key=lambda item: (item["distance"], item["block_id"]))
        compact["nearby_blocks"] = tables
...
def audit_sp002_fixture(observation: Any) -> dict:
    normalized = evidence_observation(
        observation,
        role="fixture_audit",
        ordinal=0,
    ...
    tables = [
        block
        for block in normalized.get("nearby_blocks", [])
        if isinstance(block, dict)
        and block.get("name") == "crafting_table"
        and block.get("observed") is True
        and block.get("interactive") is True
    ]
    checks = {
        "cobblestone_exact": inventory.get("cobblestone", 0) == 3,
        "stick_exact": inventory.get("stick", 0) == 2,
        "stone_pickaxe_absent": inventory.get("stone_pickaxe", 0) == 0,
        "survival_mode": normalized.get("game_mode") == "survival",
        "safe": normalized.get("safe") is True,
        "movable": normalized.get("movable") is True,
        "interactive_crafting_table_observed": bool(tables),
    }
    return {
        "type": "stone_pickaxe_sp002_fixture_machine_audit",
        "schema_version": 1,
        "policy_id": FIXTURE_POLICY_ID,
        "passed": all(checks.values()),
        "checks": checks,
...
def guard_runtime_action(mode: str, action: Any, observation: Any) -> dict:
    ...
    if normalized_mode in {"sp001", "prepare_sp002_fixture"}:
        if action_type == "dig":
            if params.get("block") != "stone":
                issues.append(f"{normalized_mode}_dig_requires_exact_stone")
            ...
            if not target_id or not matching:
                issues.append(
                    f"{normalized_mode}_dig_target_must_be_reachable_and_observed"
                )
            elif candidates[0]["source_id"] != target_id:
                issues.append(
                    f"{normalized_mode}_dig_target_must_be_nearest_observed"
                )
            else:
            ...
            if _count(inventory.get("wooden_pickaxe")) != 1:
                issues.append(f"{normalized_mode}_exact_wooden_pickaxe_required")
            if (
                normalized_mode == "prepare_sp002_fixture"
                and _count(inventory.get("cobblestone")) >= 3
            ):
                issues.append("prepare_sp002_fixture_cobblestone_limit_reached")
        elif action_type == "equip":
            if params.get("item") != "wooden_pickaxe":
                issues.append(
                    f"{normalized_mode}_only_wooden_pickaxe_may_be_equipped"
                )
            elif _main_hand_item(observation) == "wooden_pickaxe":
                issues.append(f"{normalized_mode}_redundant_wooden_pickaxe_equip")
        elif normalized_mode == "prepare_sp002_fixture" and action_type == "craft":
            if params.get("item") != "crafting_table" or params.get("count") != 1:
                issues.append("prepare_sp002_fixture_only_one_table_craft_allowed")
            if _nearby_crafting_table_observed(observation):
                issues.append("prepare_sp002_fixture_duplicate_table_craft_forbidden")
        elif normalized_mode == "prepare_sp002_fixture" and action_type == "place":
            if params.get("item") != "crafting_table":
                issues.append("prepare_sp002_fixture_only_table_placement_allowed")
            if _nearby_crafting_table_observed(observation):
                issues.append("prepare_sp002_fixture_duplicate_table_placement_forbidden")
            ...
            issues.append(
                f"{normalized_mode}_action_type_forbidden:{action_type or 'missing'}"
            )
    elif normalized_mode == "sp002":
        inventory = observation.get("inventory", {}) if isinstance(observation, dict) else {}
        if action_type != "craft":
            issues.append(f"sp002_action_type_forbidden:{action_type or 'missing'}")
        else:
            if set(params) != {"item", "count"}:
                issues.append("sp002_exact_craft_parameters_required")
            if params.get("item") != "stone_pickaxe":
                issues.append("sp002_craft_requires_exact_stone_pickaxe")
            if params.get("count") != 1:
                issues.append("sp002_craft_count_must_equal_one")
            if _count(inventory.get("cobblestone")) != 3:
                issues.append("sp002_exact_three_cobblestone_required")
            if _count(inventory.get("stick")) != 2:
                issues.append("sp002_exact_two_sticks_required")
            if _count(inventory.get("stone_pickaxe")) != 0:
                issues.append("sp002_stone_pickaxe_must_be_absent")
            if not _nearby_crafting_table_observed(observation):
                issues.append("sp002_observed_interactive_crafting_table_required")
...
class StonePickaxeRuntimeAgent(Agent):
    ...
    def _observe(self) -> dict:
        ...
        local_scan = []
        if self.stone_pickaxe_runtime_mode in {"sp002", "prepare_sp002_fixture"}:
            local_scan = self.bot.get_nearby_blocks(
        ...
        if isinstance(scan, list) or isinstance(local_scan, list):
            merged = []
            seen = set()
            for block in list(local_scan or []) + list(scan or []):
                if not isinstance(block, dict):
                    continue
                position = block.get("position") if isinstance(block.get("position"), dict) else {}
                key = (
                    str(block.get("name") or ""),
                    _finite(position.get("x")),
                    _finite(position.get("y")),
                    _finite(position.get("z")),
                )
                if not key[0] or key in seen:
                    continue
                seen.add(key)
                merged.append(block)
            observation["nearby_blocks"] = merged[:100]
        return observation
    ...
    def _goal_is_verified(
        ...
        sp002_fixture = self.stone_pickaxe_runtime_mode == "prepare_sp002_fixture"
        audit = (
            audit_sp002_fixture(observation)
            if sp002_fixture
            else audit_sp001_fixture(observation)
        )
        evidence = (
            [
                "exact cobblestone=3, stick=2, and stone_pickaxe=0 verified",
                f"interactive crafting tables={len(audit['crafting_tables'])}",
            ]
            if sp002_fixture
            else [
        ...
        target_inventory = (
            {"cobblestone": 3, "stick": 2, "stone_pickaxe": 0}
            if sp002_fixture
            else {"wooden_pickaxe": 1, "cobblestone": 0, "stone_pickaxe": 0}
        )
        verification = GoalVerification(
            ...
            confidence=1.0,
            evidence=evidence if audit["passed"] else [],
...
def build_sp002_fixture_artifact(
    preparation: dict,
    snapshot_report: dict,
    *,
    snapshot_path: str,
) -> dict:
    value = preparation if isinstance(preparation, dict) else {}
    audit = value.get("fixture_audit") if isinstance(value.get("fixture_audit"), dict) else {}
    planner_audit = (
        value.get("planner_request_controls")
        if isinstance(value.get("planner_request_controls"), dict)
        else {}
    )
    source_fixture = (
        value.get("source_fixture")
        if isinstance(value.get("source_fixture"), dict)
        else {}
    )
    protocol_status_audit = (
        value.get("protocol_status_audit")
        if isinstance(value.get("protocol_status_audit"), dict)
        else {}
    )
    checks = {
        "protocol_identity": value.get("protocol_sha256") == PROTOCOL_SHA256,
        "preparation_machine_audit": audit.get("passed") is True,
        "preparation_planner_request_controls": planner_audit.get("passed") is True,
        "bridge_protocol_status": protocol_status_audit.get("passed") is True,
        "source_sp001_fixture_verified": (
            source_fixture.get("fixture_id") == "sp001-acquire-cobblestone-v1"
            and source_fixture.get("snapshot_identity_verified") is True
            and len(str(source_fixture.get("tree_sha256") or "")) == 64
        ),
        "survival_preparation": value.get("game_mode") == "survival",
        "normal_agent_actions": value.get("external_step_script") is False,
        "no_forbidden_intervention": value.get("forbidden_interventions") == [],
        "no_target_result_injection": value.get("target_result_injection") is False,
        "no_active_episode_reset": value.get("active_benchmark_reset") is False,
        "snapshot_tree": snapshot_report.get("passed") is True,
    }
    return {
        "type": "stone_pickaxe_fixture_manifest",
        ...
        "preparation": {
            ...
            "episode_id": str(value.get("episode_id") or ""),
            "level_name": str(value.get("level_name") or ""),
            ...
            "administrative_commands": list(value.get("administrative_commands", [])),
        },
        "checks": checks,
        "snapshot_identity_verified": all(checks.values()),
        "issues": sorted(key for key, passed in checks.items() if not passed),
...
def build_sp002_episode(
    *,
    ...
    every_craft_single_attempt = True
    for index, event in enumerate(action_events, start=1):
        data = event["data"]
        action = data.get("action") if isinstance(data.get("action"), dict) else {}
        params = action.get("parameters") if isinstance(action.get("parameters"), dict) else {}
        ...
        if finished is None:
            elapsed = _finite(event.get("elapsed_s")) or 0.0
            finished = float(
                goal_result.get("episode_started_monotonic") or initial_monotonic
            ) + elapsed
        if started is None:
            started = finished - max(0, _count(result.get("duration_ms"))) / 1000.0
        if (
        ...
        if action_type == "chat":
            ...
            if token in FORBIDDEN_COMMAND_TOKENS:
                forbidden_interventions.append({"index": index, "command": token})
        if result.get("success") is True and action_type != "craft":
            non_target_mutations.append({"index": index, "action": action})
        if action_type != "craft":
            continue
        ...
        table_position = (
            ...
            if isinstance(result.get("crafting_table_position"), dict)
            else {}
        )
        table_id = source_id("crafting_table", table_position)
        observed_tables = [
            block
            for block in pre.get("nearby_blocks", [])
            if isinstance(block, dict) and block.get("block_id") == table_id
        ]
        observed_table = observed_tables[0] if observed_tables else {}
        verification = (
        ...
        every_craft_single_attempt = every_craft_single_attempt and single_attempt
        transitions.append({
            "action": {
                "type": "craft",
                "parameters": {
            ...
            "crafting_table_interaction": {
                ...
                "distance": observed_table.get("distance"),
                "observed": bool(observed_table),
                "interactive": bool(
                    result.get("crafting_table_found") is True
                    and observed_table.get("interactive") is True
                ),
            ...
            "backend_result": {
                "success": result.get("success") is True,
                "item": str(result.get("item") or params.get("item") or ""),
    ...
    terminal = evidence_observation(
        stable_observation,
        role="terminal",
        ordinal=len(action_events) + 2,
        monotonic_s=stable_monotonic,
        include_crafting_table=True,
    )
    fixture_snapshot = (
        fixture_manifest.get("snapshot", {})
        if isinstance(fixture_manifest, dict)
        else {}
    )
    fixture_verified = fixture_manifest.get("snapshot_identity_verified") is True
    protocol_match = fixture_manifest.get("protocol_sha256") == PROTOCOL_SHA256
    fixture_scope = fixture_manifest.get("fixture_id") == "sp002-craft-stone-pickaxe-v1"
    planner_audit = planner_request_controls_audit(events)
    backend_single_craft_attempt = (
        craft_action_count == 1 and every_craft_single_attempt
    )
    eligibility = {
        "passed": bool(
            fixture_verified
            ...
            and protocol_match
            and planner_audit["passed"]
            and backend_single_craft_attempt
            and not forbidden_interventions
            and not post_deadline_actions
            and not selected_skills
        ),
        "protocol_match": protocol_match,
        "planner_request_controls": planner_audit["passed"],
        "reset_clean": fixture_verified and fixture_scope,
        "backend_single_craft_attempt": backend_single_craft_attempt,
        "no_forbidden_intervention": not forbidden_interventions,
        "no_post_deadline_action": not post_deadline_actions,
        "fixture_snapshot_tree_sha256": fixture_snapshot.get("tree_sha256", ""),
    }
...
def build_sp002_run_audit(
    ...
    if not verification.get("passed"):
        ...
        if "fixture" in joined or "initial_" in joined or "crafting_table" in joined:
            next_fix = "offline_sp002_fixture_or_table_grounding"
        elif "consumption" in joined or "inventory_delta" in joined:
            next_fix = "offline_sp002_exact_material_delta_binding"
        elif "stable" in joined or "ghost" in joined:
            next_fix = "offline_sp002_stable_reobservation_binding"
        elif "attempt" in joined or "retry" in joined:
            next_fix = "offline_sp002_single_attempt_backend_policy"
        elif "deadline" in joined:
            next_fix = "offline_sp002_deadline_budgeting"
        else:
            next_fix = "offline_sp002_first_failed_transition_reproduction"
    return {
        "type": "stone_pickaxe_run_audit",
        "schema_version": 1,
        "task_id": "SP-002",
...
def verify_sp002_fixture_manifest(manifest: Any, snapshot_root: str | Path) -> dict:
    value = manifest if isinstance(manifest, dict) else {}
    report = verify_fixture_manifest(value, snapshot_root)
    preparation = (
        value.get("preparation")
        if isinstance(value.get("preparation"), dict)
        else {}
    )
    audit = (
        preparation.get("machine_audit")
        if isinstance(preparation.get("machine_audit"), dict)
        else {}
    )
    checks = {
        **dict(report.get("checks", {})),
        "fixture_id": value.get("fixture_id") == "sp002-craft-stone-pickaxe-v1",
        "fixture_policy": (
            value.get("policy_id")
            == "stone-pickaxe-sp002-survival-fixture-preparation-v1"
        ),
        "preparation_audit": audit.get("passed") is True,
        "preparation_exact_inventory": (
            audit.get("checks", {}).get("cobblestone_exact") is True
            and audit.get("checks", {}).get("stick_exact") is True
            and audit.get("checks", {}).get("stone_pickaxe_absent") is True
        ),
        "preparation_table": (
            audit.get("checks", {}).get("interactive_crafting_table_observed")
            is True
        ),
    }
    return {
        **report,
        "type": "stone_pickaxe_sp002_fixture_preflight",
        "passed": all(checks.values()),
        "checks": checks,
        "issues": sorted(key for key, passed in checks.items() if not passed),
    }
...
def verify_sp002_runtime_episode(episode: dict) -> dict:
    report = verify_sp002_episode(episode)
    eligibility = (
        episode.get("eligibility")
        if isinstance(episode.get("eligibility"), dict)
        else {}
    )
    if eligibility.get("backend_single_craft_attempt") is not True:
        issues = sorted(
            set(list(report.get("eligibility_issues", [])) + ["backend_single_craft_attempt"])
        )
        report = {
            **report,
