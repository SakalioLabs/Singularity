...
_STONE_PICKAXE_SP003_STAGE_INSTRUCTIONS = {
    ...
    "place_crafting_table": "Read every first-target boolean literally. When navigation_only=true, emit exactly one move_to: use exact stand_position x/y/z when present, otherwise use position x/z. This includes stone_clearance_probe, stone_pickup_approach, stone_clearance_shaft_egress, and stone_clearance_shaft_step_up_egress. Never place, dig, or wait on a navigation-only target. Dig only for stone_surface_clearance=true, and place exactly one crafting_table only for machine_proven_placement=true. A false marker is forbidden to infer as true.",
...
- During place_crafting_table, navigation_only=true requires move_to and forbids place, dig, or wait. Use exact stand_position x/y/z when present, otherwise position x/z. This includes stone_clearance_probe, stone_pickup_approach, stone_clearance_shaft_egress, and stone_clearance_shaft_step_up_egress.
- stone_clearance_probe=true or stone_pickup_approach=true are navigation-only examples; the shaft-egress markers follow the same navigation_only rule.
- During place_crafting_table, dig only for literal stone_surface_clearance=true.
...
If completion_ready=true and no action is needed, return status=complete with actions=[]."""
                root_graph_required = (
                    self._expected_plan_kind == "root" and not completion_ready
                )
                if root_graph_required:
                    required_root_graph = [
                        {
                            "id": "acquire_cobblestone",
                    ...
                    root_graph_gate = f"""
SP-002 fixture root graph gate: root_graph_required=true.
...
The second node must keep depends_on=[\"acquire_cobblestone\"]. Never return subtasks=[] on this root planning call.
This graph does not authorize extra actions; actions must still contain only the one next grounded action."""
                else:
                    root_graph_gate = """
...
The root response must use status=planning and contain exactly craft stone_pickaxe count=1."""
                    ...
                    root_graph_gate = f"""
SP-002 live root graph gate: root_graph_required=true.
Copy this exact JSON array into subtasks without omitting, adding, renaming, or reordering nodes:
{json.dumps(required_root_graph, sort_keys=True, separators=(',', ':'))}
