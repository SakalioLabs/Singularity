...
from singularity.evaluation.stone_pickaxe_skill_evaluation_v3 import (
    POLICY,
    POLICY_PATH,
    POLICY_SHA256,
...
def test_v3_policy_is_isolated_and_prior_identities_remain_valid():
    report = policy_identity_report()
    assert report["passed"], report["issues"]
    assert file_sha256(POLICY_PATH) == POLICY_SHA256
    assert POLICY["arms"]["candidate"]["replicate_ids"] == ["r7", "r8", "r9"]
    assert POLICY["recovery_window"]["excluded_replicate_ids"] == [
        "r1", "r2", "r3", "r4", "r5", "r6"
    ]
    for relative, expected in {**V1_FROZEN_FILES, **V2_FROZEN_FILES}.items():
...
def test_v3_authorizes_only_fresh_candidate_replicates():
    authorization = _authorization("r7", "v3-auth-r7")
    audit = validate_evaluation_authorization(
        authorization,
        expected_arm="candidate",
        expected_replicate_id="r7",
        expected_episode_id="v3-auth-r7",
        expected_git_head="a" * 40,
    )
    assert audit["passed"], audit["issues"]
    assert authorization["pair_id"] == "sp001-acquire-recovery-v3-r7"
    assert authorization["evaluation_window_id"] == "sp001-acquire-skill-attribution-recovery-v3"
    assert authorization["prior_window_policy_sha256"] == V2_FROZEN_FILES[
        "workspace/evals/stone_pickaxe_paired_evaluation_policy_v2.json"
    ]
    for replicate in ("r1", "r2", "r3", "r4", "r5", "r6"):
        with _raises(ValueError):
            _authorization(replicate, f"v3-reject-{replicate}")
    with _raises(ValueError):
        build_evaluation_authorization(
            arm="shadow",
            replicate_id="shadow-1",
            episode_id="v3-reject-shadow",
            git_head="a" * 40,
            existing_run_paths=[],
        )


def test_v3_duplicate_candidate_authorization_fails_closed(tmp_path):
    consumed = tmp_path / "authorization.json"
    consumed.write_text(
        json.dumps({"arm": "candidate", "replicate_id": "r7", "authorization_id": "used-r7"}),
        encoding="utf-8",
    )
    with _raises(ValueError, match="already consumed"):
        build_evaluation_authorization(
            arm="candidate",
            replicate_id="r7",
            episode_id="v3-duplicate-r7",
            git_head="a" * 40,
            existing_run_paths=[consumed],
        )


def test_initial_v3_report_inherits_support_and_excludes_both_failed_candidates():
...
def test_v3_policy_rejects_inherited_support_hash_tampering():
...
def test_v3_policy_rejects_prior_window_and_retained_failure_tampering():
    tampered = copy.deepcopy(POLICY)
    ...
    tampered = copy.deepcopy(POLICY)
    tampered["recovery_window"]["retained_failed_runs"][1]["sha256"] = "0" * 64
    report = policy_identity_report(tampered)
    assert not report["passed"]
    assert "failed_r4_file" in report["issues"]


def test_v3_report_requires_three_new_candidates_before_review():
    with _repository_tempdir() as directory:
        two = [_materialize_candidate(directory, replicate, index) for index, replicate in enumerate(("r7", "r8"), 1)]
        report = build_paired_evaluation_report(two)
        assert report["valid_pair_count"] == 2
        assert report["decision"] == "retain_advisory"
        third = _materialize_candidate(directory, "r9", 3)
...
def test_v3_report_rejects_duplicate_and_missing_window_identity():
    with _repository_tempdir() as directory:
        first = _materialize_candidate(directory, "r7", 1)
        duplicate = _materialize_candidate(directory, "r7", 2)
        report = build_paired_evaluation_report([first, duplicate])
        assert "duplicate_arm_replicate:candidate:r7" in report["errors"]
        assert report["decision"] == "retain_advisory"
...
def test_v3_context_does_not_leak_into_prior_policies_after_report_build():
    build_paired_evaluation_report([])
...
def test_v3_launcher_is_candidate_only_and_uses_isolated_cli():
    launcher = (REPOSITORY_ROOT / "scripts/stone-pickaxe-skill-window-v3.ps1").read_text(encoding="utf-8")
    assert '"candidate" = @("r7", "r8", "r9")' in launcher
    assert "stone_pickaxe_skill_evaluation_v3.py write-authorization" in launcher
    assert "stone_pickaxe_skill_evaluation_v3.py run-arm" in launcher
    assert "stone_pickaxe_skill_evaluation_v3.py refresh-report" in launcher
    assert '"shadow" = @("shadow-1")' not in launcher
    assert '"candidate" = @("r1", "r2", "r3")' not in launcher
    assert '"candidate" = @("r4", "r5", "r6")' not in launcher
    assert "workspace\\evals\\sp001_skill_evaluation_v3" in launcher


def test_v3_generated_reports_are_byte_preserved_without_changing_v1_attributes():
    attributes = (
        REPOSITORY_ROOT / "workspace/evals/sp001_skill_evaluation_v3/.gitattributes"
    ).read_text(encoding="utf-8").splitlines()
    assert attributes == [
        "acquire_cobblestone_baseline_index_v3.json binary",
        "acquire_cobblestone_paired_evaluation_v3.json binary",
        "stone_pickaxe_paired_evaluation_policy_v3.json binary",
    ]
...
def _materialize_candidate(root: Path, replicate: str, ordinal: int) -> Path:
    ...
    episode_id = f"offline-v3-candidate-{replicate}-{ordinal}"
    session_id = f"offline-v3-session-{replicate}-{ordinal}"
    ...
    for event in events:
        ...
        action["skill_context"] = {
            ...
            "goal_fingerprint": "offline-v3-goal",
        }
    events.extend([
        {
            "type": "skill_selected",
...
class _repository_tempdir:
    def __enter__(self):
        self._temporary = tempfile.TemporaryDirectory(prefix="stone-skill-eval-v3-test-", dir=REPOSITORY_ROOT / "workspace")
